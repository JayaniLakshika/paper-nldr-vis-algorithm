...
@fig-comp-metric-pbmc and @fig-comp-metric-mnist compare RMSE with commonly used evaluation metrics such as RTA, ARNX, sc, and RGS across multiple NLDR layouts. These visual comparisons highlight that RMSE behaves differently from these existing metrics due to the different settings involved.
...
#| fig-cap: "Comparison of evaluation metrics across eight NLDR layouts for the PBMC3k data. Each line represents a layout evaluated using RTA, ARNX, sc, RGS, and RMSE. The metrics show different patterns, meaning no single measure tells the full story of embedding quality. Interestingly, the green layout, rated worst by ARNX and RGS, is seen as the best by RMSE and sc, while the brown layout, rated best by ARNX, is seen as the worst by RMSE showing that RMSE captures a different view of how well the data structure is represented."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 10
#| fig-width: 10
