...
which given $s_1 < 0$ and $a_1 > 0$ will *always* be true. The same logic follows for @eq-equation4 and substituting $a_2 = \sqrt{3}a_1/{2}$, and $s_2 = -q \times r_2$ to @eq-equation4 can be written as,

$$
a_1 > -\frac{4}{\sqrt{3}}qr_2
$$

Also, substituting $a_2 = \sqrt{3}a_1/{2}$, $s_2 = -q \times r_2$ and rearranging @eq-equation5 gives:
...
tr_from_to_df_pbmc <- tr_from_to_df_pbmc <- update_trimesh_index(trimesh_data = tr_from_to_df_pbmc)
