...
## Compare RMSE with existing evaluation metrics

@fig-comp-metric-pbmc and @fig-comp-metric-mnist compare RMSE with commonly used evaluation metrics such as RTA, AUC(R_NX), sc, and RGS across multiple NLDR layouts. These visual comparisons highlight that RMSE behaves differently from these existing metrics due to the different settings involved.
...
#| fig-cap: "Comparison of evaluation metrics across eight NLDR layouts for the PBMC3k data. Each line represents a layout evaluated using RTA, AUC(R_NX), sc, RGS, and RMSE. The metrics show different patterns, meaning no single measure tells the full story of embedding quality. Interestingly, the green layout, rated worst by AUC(R_NX) and RGS, is seen as the best by RMSE and sc, while the brown layout, rated best by AUC(R_NX), is seen as the worst by RMSE showing that RMSE captures a different view of how well the data structure is represented."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 10
#| fig-width: 10
...
#| fig-cap: "Comparison of evaluation metrics across eight NLDR layouts for the MNIST digit 1 data. Each line represents a layout evaluated using RTA, AUC(R_NX), sc, RGS, and RMSE. The metrics show different patterns, meaning no single measure tells the full story of embedding quality. Interestingly, the brown layout, rated the best by AUC(R_NX), sc, and RGS, is seen as the worst by RMSE, while the green layout, rated best by RTA, is seen as the best by RMSE as well."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 10
#| fig-width: 10
