...
comp_metrics_pbmc_scdeed <- ggparcoord(
  data = nldr_metrics,
  columns = c(8,3,5,2,7), #c(5,8,3,2,7),           # numeric columns to plot
...
#| fig-pos: H
#| out-width: 100%
#| fig-cap: "Comparing the published layout (a) with what would be suggested to be optimal by scDEED (b), using HBE vs $a_1$, on a subset of PBMC3k data. Color represents NLDR layouts. HBE would corroborate that the scDEED optimised layout is better than what was originally published. Comparison of scaled evaluation metrics (ARNX, RTA, SC, GS, and HBE using $a_1=0.04$) for two NLDR layouts of the PBMC3k data the originally published layout (a) and the scDEED optimised layout (b). Each line represents a layout, with color matching the corresponding scatterplots. Most metrics (ARNX, RTA, GS, and HBE) consistently indicate that the optimised layout (b) provides a better representation, while SC slightly favors the published layout. "
#| fig-width: 6
#| fig-height: 4
...
final_plot <- (error_umap_plot_pbmc | bottom_row) /comp_metrics_pbmc_scdeed + plot_layout(heights = c(0.7, 1))
...
comp_metrics_pbmc <- ggparcoord(
  data = nldr_metrics,
  columns = c(8,3,5,2,7), #c(5,3,8,2,7),           # numeric columns to plot
  groupColumn = 1,         # color by method
  scale = "uniminmax"      # normalize between 0 and 1 for fair comparison
) +
  scale_color_manual(
    values=c('#999999','#a65628','#e41a1c','#377eb8','#4daf4a','#ff7f00','#984ea3','#f781bf')) +
  theme_minimal() +
  labs(
    y = "scaled metric value",
    x = ""
...
#| fig-cap: "Comparison of scaled evaluation metrics (ARNX, RTA, SC, GS, and HBE using $a_1=0.06$) for the eight NLDR layouts computed on the PBMC3k data using a parallel coordinate plot. Color of the line indicates NLDR layout. ARNX is reversed so that lower is best. RTA, SC, and GS should also be reversed, but they align well with ARNX and HBE without doing so. There is general agreement across metrics, with some variation in ranking. All metrics identify layout f as poor, and layouts a, c, g, and h as suboptimal due to excessive cluster separation. Layout d performs well only at very fine binwidths but deteriorates as binwidth increases. The best overall agreement is between layouts b and e, with layout e ultimately chosen as the most reasonable representation."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 6
#| fig-width: 6
...
comp_metrics_mnist <- ggparcoord(
  data = nldr_metrics,
  columns = c(8,3,5,2,7), #c(5,8,2,3,7),           # numeric columns to plot
  groupColumn = 1,         # color by method
  scale = "uniminmax"      # normalize between 0 and 1 for fair comparison
) +
  scale_color_manual(
