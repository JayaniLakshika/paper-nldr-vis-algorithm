...
kable(fig_param_df, 
  ...
  row_spec(45, hline_after = TRUE) |>
  row_spec(46, hline_after = TRUE) |>
  row_spec(48, hline_after = TRUE) |>
  row_spec(50, hline_after = TRUE) |>
  row_spec(52, hline_after = TRUE) |>
  row_spec(60, hline_after = TRUE) 
```
...
kable(fig_param_df, 
  ...
  kable_styling(position = "center", 
                ...
                latex_options = c("repeat_header")) |>
  row_spec(0, bold = TRUE) |>
  column_spec(1:2, width = c("5cm", "5cm", "10cm")) |>
...
#| fig-pos: H
#| out-width: 100%
#| fig-cap: "Assessing which of the 2 NLDR layouts on the PBMC3k data is the better representation using HBE for varying binwidth ($a_1$). Color used for the lines and points in the left plot and in the scatterplots represents NLDR layouts (a, b). Of the two, layout b is optimal across all binwidths making it the best choice."
...
The HBE vs binwidth ($a_1$) plot (@fig-pbmc-mse-umap) illustrates that our approach would suggest that scDEED is correct here, that layout b is more accurately reflecting the cluster structure in the PBMC data.
...
```{r}
#| label: fig-comp-metric-pbmc-scdeed
#| fig-cap: "Comparison of evaluation metrics across eight NLDR layouts for the MNIST digit 1 data. Each line represents a layout evaluated using RTA, ARNX, sc, RGS, and HBE. The metrics show different patterns, meaning no single measure tells the full story of embedding quality. Interestingly, the brown layout, rated the best by ARNX, sc, and RGS, is seen as the worst by HBE, while the green layout, rated best by RTA, is seen as the best by HBE as well."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 6
#| fig-width: 6
...
# Combine the plots using the layout
(comp_metrics_pbmc_scdeed + nldr1c + nldr2c) +
  plot_layout(design = layout_design)
```
...
@fig-comp-metric-pbmc and @fig-comp-metric-mnist compare HBE with commonly used evaluation metrics such as RTA, ARNX, sc, and RGS across multiple NLDR layouts. These visual comparisons highlight that HBE behaves differently from these existing metrics due to the different settings involved.
...
#| fig-cap: "Comparison of evaluation metrics across eight NLDR layouts for the PBMC3k data. Each line represents a layout evaluated using RTA, ARNX, sc, RGS, and HBE. The metrics show different patterns, meaning no single measure tells the full story of embedding quality. Interestingly, the green layout, rated worst by ARNX and RGS, is seen as the best by HBE and sc, while the brown layout, rated best by ARNX, is seen as the worst by HBE showing that HBE captures a different view of how well the data structure is represented."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 6
#| fig-width: 6
