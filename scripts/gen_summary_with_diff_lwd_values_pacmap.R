...
PaCMAP_data <- read_rds("data/s_curve/s_curve_pacmap.rds")

shape_value_curve <- calculate_effective_shape_value(.data = PaCMAP_data,
                                                     x = PaCMAP1, y = PaCMAP2)

num_bins_x <- 10 ## Number of bins along the x-axis (looking at mse plot)
...
pred_df_training_object <- predict_hex_id(training_data = training_data, nldr_df = PaCMAP_data, nldr_df_test = PaCMAP_data, num_bins = num_bins_x, shape_val = shape_value_curve, x = "PaCMAP1", y = "PaCMAP2", col_start = "x")
...
for (i in 1:length(benchmark_rm_hex_vec)) {
  ...
  PaCMAP_data_with_hb_id <- PaCMAP_data |>
    mutate(hb_id = hexbin_data_object_training$hb_data@cID)

  ## To generate a data set with high-D and 2D training data
  df_all <- dplyr::bind_cols(training_data |> dplyr::select(-ID), PaCMAP_data_with_hb_id)
...
write_rds(MSE_df_2, "data/s_curve/s_curve_summary_lwd_pacmap.rds")
