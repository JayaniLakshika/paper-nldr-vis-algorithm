...
PaCMAP_two_nonlinear_clusters <- read_rds("data/one_curvy_one_gau_clust/one_curvy_one_gau_clust_pacmap_n-neighbors_10_init_random_MN-ratio_0.5_FP-ratio_2.rds")

two_nonlinear_clusters_scaled_obj <- gen_scaled_data(
  data = PaCMAP_two_nonlinear_clusters)
PaCMAP_two_nonlinear_clusters_scaled <- two_nonlinear_clusters_scaled_obj$scaled_nldr |>
  mutate(ID = 1:NROW(PaCMAP_two_nonlinear_clusters))
...
trimesh_removed_two_nonlinear_clusters <- ggplot() +
  geom_segment(data = tr_from_to_df_two_nonlinear_clusters,
               ...
               colour = "#33a02c",
               linewidth = 1) +
  geom_point(data = PaCMAP_two_nonlinear_clusters_scaled,
             aes(
               x = PaCMAP1,
               y = PaCMAP2
             ),
...
hb_obj_two_nonlinear_clusters <- hex_binning(
  data = PaCMAP_two_nonlinear_clusters_scaled,
  bin1 = num_bins_x_two_nonlinear_clusters,
  r2 = r2_two_nonlinear_clusters)

PaCMAP_data_with_hb_id <- hb_obj_two_nonlinear_clusters$data_hb_id
