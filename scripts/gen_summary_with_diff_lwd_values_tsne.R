...
pred_df_training_object <- predict_hex_id(training_data = training_data, nldr_df = tSNE_data, nldr_df_test = tSNE_data, num_bins = num_bins_x, shape_val = shape_value_curve, x = "tSNE1", y = "tSNE2", col_start = "x")
...
for (i in 1:length(benchmark_rm_hex_vec)) {
  ...
  df_all <- dplyr::bind_cols(training_data |> dplyr::select(-ID), tSNE_data_with_hb_id)
...
write_rds(MSE_df_2, "data/s_curve/s_curve_summary_lwd_tsne.rds")
