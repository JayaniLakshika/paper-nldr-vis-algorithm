...
shape_value_s_curve <- calculate_effective_shape_value(.data = PHATE_s_curve,
                                                       x = PHATE1, y = PHATE2) ## 1.259938

## To extract bin centroids
hexbin_data_object_s_curve <- extract_hexbin_centroids(nldr_df = PHATE_s_curve,
                                                       num_bins = num_bins_s_curve,
                                                       shape_val = shape_value_s_curve, x = PHATE1, y = PHATE2)

df_bin_centroids_s_curve <- hexbin_data_object_s_curve$hexdf_data

## Identify bins with low-density
identify_rm_bins <- find_low_density_hexagons(df_bin_centroids_s_curve, num_bins_s_curve, benchmark_rm_hex = 0.06)

df_bin_centroids_s_curve <- df_bin_centroids_s_curve |>
  filter(!(hexID %in% identify_rm_bins))

PHATE_s_curve_with_hb_id <- PHATE_s_curve |>
  dplyr::mutate(hb_id = hexbin_data_object_s_curve$hb_data@cID)

## To generate a data set with high-D and 2D training data
df_all_s_curve <- dplyr::bind_cols(training_data_s_curve |> dplyr::select(-ID), PHATE_s_curve_with_hb_id)
...
for(i in 1:length(benchmark_dist_vec)) {
  ...
  pred_df_training_object <- predict_hex_id(training_data = training_data_s_curve, nldr_df = PHATE_s_curve, nldr_df_test = PHATE_s_curve, num_bins = num_bins_s_curve, shape_val = shape_value_s_curve, x = "PHATE1", y = "PHATE2", col_start = "x")
...
write_rds(eval_data_training, "data/s_curve/s_curve_summary_lg_phate.rds")
