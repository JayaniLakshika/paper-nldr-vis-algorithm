...
trimap_gau <- read_rds("data/five_gau_clusters/trimap_data_five_gau.rds")
gau_scaled_obj <- gen_scaled_data(
  data = trimap_gau)
trimap_gau_scaled <- gau_scaled_obj$scaled_nldr
...
ggplot(error_gau, aes(x = b_non_empty,
  ...
  geom_point() +
  geom_line() +
  geom_vline(xintercept = 64, linetype="solid",
