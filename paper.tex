...
\includegraphics[width=0.6\textwidth,height=\textheight]{paper_files/figure-pdf/fig-p-d-error-in-2d-two-curvy-1.pdf}
...
NLDR layout, simulated \(4\text{-}D\) data having five Gaussian clusters
is used. Figure~\ref{fig-five-gau-projs} a1, b1, c1 show the \gD{}
layouts for (a) tSNE, (b) UMAP, and (c) PaCMAP, respectively. The
default hyper-parameters are used. All three methods show the five
...
b2, c2 show the fitted models in a projection of the \(4\text{-}D\)
space. In this projection the difference between methods can be seen.
These clusters are fully \(4\text{-}D\) in nature, so we would expect
the model to be a \emph{crumpled \gD{} sheet} that stretches in all four
dimensions. This is what is observed for tSNE and UMAP. The curious
detail is that the model for PaCMAP is closer to a \emph{pancake} in
shape! What this means is that there has to be some ordering of points
in the \gD{} PaCMAP layout that induces the flat model, likely that the
points are organised by the global principal components. So the layout
of the model in \(4\text{-}D\) doesn't reflect the dimension of each
cluster. DHC: is there something in the documentation that might help to
...
To illustrate how tSNE organize different number of points within the
structure in the tSNE layout, simulated \(7\text{-}D\) data having
C-shaped structure is used. Figure~\ref{fig-one-dens_clust-error} a1 and
b1 shows \gD{} layouts for tSNE. The default hyper-parameters are used.
The \gD{} layouts are colored according to the high-dimensional model
error. In b1, the large model error points are clustered in the top left
corner, forming a separate cluster with high model errors.

The models are fitted to these layouts.
Figure~\ref{fig-one-dens_clust-error} b1 and b2 show the fitted models
in projections of the \(7\text{-}D\) space. In these projections, the
differences in the distribution of high-dimensional model errors can be
observed. The uniform C-shaped cluster displays randomly distributed
model errors, while the dense C-shaped cluster shows high model errors
at the sparse end. In the dense C-shaped cluster, the high model errors
occur because there are fewer data points positioned within the bins at
the sparse end of the structure.

\begin{figure}[H]

\centering{

\includegraphics[width=1\textwidth,height=0.5\textheight]{paper_files/figure-pdf/fig-one-dens_clust-error-1.pdf}
...
error. The tSNE layouts of the C-shaped structure, with uniform points
(a1) and dense points (b1) are colored according to their
high-dimensional error, along with their selected projections (a2 and
b2), are displayed. Darker colors represent a high error, while lighter
colors indicate a low error. The uniformly distributed points
demonstrate random model errors throughout the structure, whereas the
dense structure shows large model error at the sparse end. Videos of the
