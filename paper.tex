...
(PBMC3k). Figure~\ref{fig-NLDR-variety} (a) is the reproduction of the
published plot. It shows three well-separated clusters with big
separations.
...
original data structure, we visualize the model constructed with UMAP
overlaid on the \(9\text{-}D\) data. The \textbf{?@fig-umap-author}
shows three well-separated clusters. However, as shown in
\textbf{?@fig-pbmc1-sc}, there is no big separation between three
clusters in \(9\text{-}D\). Therefore, the suggested UMAP representation
