...
\item
  ...
  shows three well-separated clusters. The question we need to answer
...
{\centering \includegraphics{paper_files/figure-pdf/fig-nldervisPBMCUMPview-1.pdf}
...
\caption{\label{fig-nldervisPBMCUMAP}Model in the 2D space with UMAP
(\url{https://youtu.be/CNXjWZMyh_M})}

\end{figure}

\begin{itemize}
\item
  You will see three well-separated clusters in high-D as well which
  gives evidence that UMAP preserves the global structure. Furthermore,
  there is non-linearity within the clusters which was not identified by
  the UMAP, which proves that local structure is not preserved.
\item
  Can we find a better parameter choice for UMAP which preserve the
  local structure as well?
\item
  To do that, we are going to fit models for different parameter choice
  and select the one which have the lowest error.
\end{itemize}
...
\begin{itemize}
\item
  Selected the one with n:neighbor:5, min\_dist:0.99, metric:cosine.
\item
...
{\centering \includegraphics{paper_files/figure-pdf/fig-nldervisPBMCUMAPview2-1.pdf}
...
\caption{\label{fig-nldervisPBMCUMAP3}Model in the 2D space with UMAP
(\textless\textgreater)}
...
{\centering \includegraphics[width=1\textwidth,height=\textheight]{paper_files/figure-pdf/fig-nldervisPBMCUMAP3Res-1.pdf}
...
\begin{itemize}
\item
  Why I choose this dataset? Because I need show that whatever NLDR
...
\includegraphics{paper_files/figure-pdf/unnamed-chunk-84-1.pdf}
