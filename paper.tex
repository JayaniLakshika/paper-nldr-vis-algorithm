...
\paragraph{Density of a bin}\label{density-of-a-bin}

Area of a hexagon is defined as \(A = \frac{3\sqrt{3}}{2}l^2\) where
\(l\) is the side length of the hexagon. If we know \(a_1\) and \(a_2\),
\(l\) can be computed (see appendix). Therefore, the density of a
hexagon grid is computed as \(\frac{\sum^{h}_{i=1}n_h}{A}\) and the
proportion is \(\frac{\sum^{h}_{i=1}n_h}{A \times b}\). The base line
...
\(0.12\) to investigate. As the binwidth increases, the proportion of
non-empty bins also increases. There are two peaks at binwidths \(0.09\)
and \(0.12\). The proportion of densities, calculated as a density with
the smallest possible binwidth, decreases and levels off. Binwidths
\(0.09\) and \(0.12\) are favorable choices. Based on the MSE, \(0.09\)
...
{\centering \includegraphics[width=1\textwidth,height=0.7\textheight]{paper_files/figure-pdf/unnamed-chunk-36-1.pdf}

}

\caption{Distribution of edge lengths and wireframes in \gD{} and
\(7\text{-}D\) with Delaunay triangulation, and two choices of long edge
removal: (b) benchmark = 4.5\(a_1\) and (c) benchmark = 2\(a_1\), where
\(a_1\) = 0.09.}
...
\includegraphics[width=1\textwidth,height=0.6\textheight]{paper_files/figure-pdf/fig-two_curvy_clust-mse-1.pdf}

}

\caption{\label{fig-two_curvy_clust-mse}Assessing which of the 5 NLDR
layouts on the two-curvy clusters data is the better representation
using MSE for varying binwidth (\(a_1\)). Colour used for the lines and
points in the left plot and in the scatterplots represents NLDR layout
(a-e). Layout d is universally poor. Layouts a that show two close
clusters are universally suboptimal. Layout a is the best choice.}

\end{figure}%

\begin{figure}[H]

{\centering \includegraphics[width=1\textwidth,height=0.9\textheight]{paper_files/figure-pdf/best-fit-umap-tsne-1.pdf}

}

\caption{Best fit for two-curvy clusters data using UMAP (a1-a4)
