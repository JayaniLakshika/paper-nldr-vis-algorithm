...
(\(b_1, b_2\)), and the lower left position where the grid starts
(\(s_1, s_2\)). The values of \(s_i\) need to be below their respective
minimum variable values, and could be a full bin lower, to allow a
buffer corresponding to a full hexagon width (\(a_1\)) and height
(\(a_2\)) around the data. The values of \(b_i\) are variables to be
computed that define the reduction in size of the data (\(n\) to \(m\)).

Number of bins is set by fixing \(b_1\), which determines the binwidth
accounting the offset \(q_1\) and breaks on \(x_1\), is calculated as

\[
a_1 = \frac{r_1 + q_1}{b_1}.
\]

The computed binwidth then determines the number of vertical bins,
\(b_2\), and vertical binwidth, which are computed based on \(r_2\), the
offset \(q_2\), and the height/width ratio of a regular hexagon,
\(\frac{2}{\sqrt{3}}\) as
...
Points are allocated to the bin that they fall in. XXX How are you doing
this? Do you check the bounds of the hexagon? Or do you use distance to
centroid?
