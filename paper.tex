...
\includegraphics[width=1\textwidth,height=0.3\textheight]{paper_files/figure-pdf/fig-p-d-error-in-2d-two-curvy-1.pdf}
...
\includegraphics[width=1\textwidth,height=0.8\textheight]{paper_files/figure-pdf/fig-predict-two-curvy-1.pdf}
...
(b) predicted data from the \texttt{umap} prediction method, and (c)
predicted data from our method.}
...
long edge removal: (b) benchmark = 4.5\(a_1\) and (c) benchmark =
...
\includegraphics[width=1\textwidth,height=0.6\textheight]{paper_files/figure-pdf/fig-two_non_linear_diff_shaped_close_clusters-mse-1.pdf}
...
{\centering \includegraphics[width=1\textwidth,height=0.9\textheight]{paper_files/figure-pdf/best-fit-umap-tsne-pacamp-1.pdf}
...
\subsubsection{scDEED}\label{scdeed}

Motivated by this challenge, we present a statistical method, scDEED,
for detecting dubious cell embeddings output by a 2D-embedding method.
By calculating a reliability score for every cell embedding based on the
similarity between the cell's 2D-embedding neighbors and pre-embedding
neighbors, scDEED identifies the cell embeddings with low reliability
scores as dubious and those with high reliability scores as trustworthy.
Moreover, by minimizing the number of dubious cell embeddings, scDEED
provides intuitive guidance for optimizing the hyperparameters of an
embedding method.
...
\begin{longtable*}{rrr}
\toprule
n\_neighbors & min.dist & number\_dubious\_cells\\
\midrule
5 & 0.0125 & 14\\
6 & 0.0125 & 7\\
7 & 0.0125 & 5\\
...
\includegraphics[width=1\textwidth,height=1\textheight]{paper_files/figure-pdf/fig-pbmc-mse-umap-1.pdf}

}

\caption{\label{fig-pbmc-mse-umap}Assessing which of the 2 tSNE layouts
with different perplexity (\(30\) (red), \(320\) (blue)) on the PBMC
data is the better representation using MSE for varying binwidth
(\(a_1\)). Colour used for the lines and points in the left plot and in
the scatterplots represents tSNE layout (a, b). Layout b is universally
...
\includegraphics[width=1\textwidth,height=0.8\textheight]{paper_files/figure-pdf/fig-model-pbmc-author-proj-scdeed-1.pdf}
...
for the PBMC3k data (\(n =  5858\) and \(p = 50\)). Layout shows close
