...
from \pD. In MDS, the \kD   layout is constructed by minimizing a stress
function that differences distances between points in \pD   with
...
We define the NLDR model as function
\(g\text{:}~ \mathbb{R}^p \rightarrow \mathbb{R}^k\), with
(hyper-)parameters \(\mathbfit{\theta}\). The parameters,
...
PaCMAP, or MDS, although in theory any function that maps the entire
space \(\mathbb{R}^p\) to a subset of \(\mathbb{R}^k\) is suitable.
...
Define a hexagonal grid across the \kD space, using hexagons with fixed
height (\(h_b\)), width (\(w_b\)). The starting coordinates
...
point belongs to. Let \(h : \mathbb{R}^k \rightarrow \mathbb{R}^k\) be
