...
XXX SHOW THE S-CURVE DATA WITH THE TRUE MODEL OVERLAID HERE, BEFORE THE
NLDR VIEWS.
...
\caption{\label{fig-scurve-sc}XXX Model in 2D, on the UMAP layout, and
...
\[\frac{1}{n}\sum_{h = 1}^{b}\sum_{i = 1}^{n_h}\sum_{j = 1}^{p} (\mathbfit{x}_{hij} - C^{(p)}_{hj})^2\]

where \(n\) is XXX, \(b\) is XXX, \(n_h\) is XXX, \(p\) is XXX,
\(\mathbfit{x}_{hij}\) is XXX. MAKE THIS A NUMBERED EQUATION, SO WE CAN
REFER TO IT LATER
