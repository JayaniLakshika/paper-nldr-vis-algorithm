...
a and c, and two views of the fit in projections from \(9\text{-}D\),
for the PBMC3k data (\(n =  2622\) and \(p = 9\)). Layout a shows
three-well separated clusters with big separation, while layout c shows
close clusters. In \(9\text{-}D\), the data shows close clusters. This
is the reason where b1, b2, and b3 shows connected edges between the
clusters, and also proved by Figure~\ref{fig-pbmc-mse} as the best
layout, which used the same NLDR method. By viewing the fitted model in
\(9\text{-}D\), help to see some unobserved pattern of the data (i)
densed points, and (ii) non-linear clusters.}
