...
PaCMAP provides a reasonable \gD{} representation of MNIST digit 1 data,
as it preserves the non-linear structure present in the \(10\text{-}D\)
data (Figure~\ref{fig-mnist-tri-proj}). Furthermore, shows some quirks
when visualizing the \emph{model-in-the-data-space}
(Figure~\ref{fig-mnist-tri-proj}).
...
\includegraphics[width=1\textwidth,height=0.8\textheight]{paper_files/figure-pdf/fig-model-mnist-1.pdf}

}

\caption{\label{fig-model-mnist}\(10\text{-}D\) model error in PaCMAP
layout of MNIST digit 1 dataset in \(2\text{-}D\). Most low
