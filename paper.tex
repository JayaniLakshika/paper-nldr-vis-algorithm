...
\includegraphics[width=1\textwidth,height=0.5\textheight]{paper_files/figure-pdf/fig-mse-aic-scurve-1.pdf}

}

\caption{\label{fig-mse-aic-scurve}MSE from UMAP applied to S-curve
dataset with diffferent \(b\) choices. What is the effective \(b\) to
create the model? The MSE plot have a steep slope at the beginning,
indicating that a smaller \(b\) causes a larger amount of MSE. Then, the
slope gradually declines or level off, indicating that a higher \(b\)
generates a smaller MSE. Using the elbow method, it was observed that
when the \(b\) is set to \(420\), the lowest MSE occurred.}

\end{figure}%

The best model for the S-curve with UMAP (n\_neighbors: \(15\)) create
with \(420\) of total number of bins. Also, the best model made after
removing edges which has the length greater than \(0.134\). Why this is
the best? Because AIC plot shows that the minimum occur at \(420\).
Visually, the model capture the geometry of the S-curve well.

On the other hand, a bad model for the S-curve with UMAP is made with
\(190\) bins, and removing edges which has the length greater than
\(0.3\). Why this is a bad model? The MSE error is greater than the best
model. Visually, the model is not continuously fitted along with the
S-curve data, and show some disconnect edges.
...
\caption{\label{fig-scurve_sc}Screen shots of the \textbf{langevitour}
of the S-curve, shows the model-in-data space, a video of the tour
animation is available at (\url{https://youtu.be/4RFNENX_fXk}). MSE with
\(0.035\).}
...
\caption{\label{fig-scurve_sc}Screen shots of the \textbf{langevitour}
of the S-curve, shows the model-in-data space, a video of the tour
animation is available at (\url{https://youtu.be/DHPvhkZ0lUM}). MSE with
\(0.090\).}
...
introducing approaches to bind the data and indicate line segments
beyond \gD{}. These improvements would help in creating a more accurate
representation of the data when \gD{} layout is inadequate.
