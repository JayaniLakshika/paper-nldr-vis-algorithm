...
Common choices for \(g\) include tSNE, uMAP, PACMAP, or PHATE, although
in theory any function that maps the entire space \(\mathbb{R}^p\) to a
subset of \(\mathbb{R}^k\) is suitable. Any input requirements for the
data (such as normalisation, or preprocessing through the use of PCA or
similar) is considered part of the function \(g\).
...
\pD, completing the `model-in-data-space' picture. For this, define the
set \(H^k\) to be the set of points that belong to centroid \(k\). That
is, \(H^k = \{ y \; | \; g(y) = (h_x^k, h_y^k)\}\). We use the
\pD Euclidean mean of the points in \(H^k\) to map the centroid
