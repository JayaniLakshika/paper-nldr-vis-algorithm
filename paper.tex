...
{\centering \includegraphics[width=1\textwidth,height=\textheight]{paper_files/figure-pdf/unnamed-chunk-41-1.pdf}
...
\includegraphics[width=1\textwidth,height=1\textheight]{paper_files/figure-pdf/fig-two_curvilinear_clust-mse-1.pdf}

}

\caption{\label{fig-two_curvilinear_clust-mse}Assessing which of the 5
NLDR layouts on the two\_curvilinear\_clust digit 1 data is the better
...
\caption{\label{fig-five-gau-projs}Model in \gD{}, on the layouts from
tSNE, UMAP, and PaCMAP (a1, b1, c1). Two views of the fit in projections
of each layout from \(4\text{-}D\), for the Gaussian cluster data. All
\gD{} models show five well-separated clusters with different gaps.
Layout a1 shows small separations, while layout b2 shows large gaps. On
the other hand, layout b3 shows average gaps with exact positions as in
\(4\text{-}D\). Assessing the different models fit for different layouts
for a specific cluster shows filled-out shape in a1, a2, and b1, b2. On
the other hand, c1 and c2 shows pancake shape. The full video for the
model for layout a, b, and c are available at
...
close clusters. In \(9\text{-}D\), the data shows close clusters. This
is why b1, b2, and b3 shows connected edges between the clusters, as
proven by Figure~\ref{fig-pbmc-mse} as a best layout. Additionally, in
the fitted model for layout b, the two big clusters are not connected,
but two are connected with the small cluster. Viewing the fitted model
in \(9\text{-}D\), helps to see some unobserved patterns of the data:
(i) dense points, and (ii) non-linear clusters. Videos of the
langevitour animations for the models are available at
\url{https://youtu.be/swkOKdjnjq0} and respectively.}
...
\gD{} layout of tSNE (\textbf{?@fig-tsne-best}) is also visible when
...
visible in the \gD{} representation (\textbf{?@fig-tsne-best}). (d) some
long edges exist in the \(10\text{-}D\) space that are not recognized as
long edges in the \gD{} representation. The full video is available at
