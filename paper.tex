...
large number of images, we selected the handwritten digit 1 which has a
non-linear structure in \gD{}. There are \(7877\) images. Also, applied
PCA as a preprocessing step and selected first \(10\) principal
components. The objective is to evaluate whether PaCMAP preserves the
non-linear structure in \gD{}.
...
\(10\text{-}D\), or if it is misleading. The angle of the digit 1 varies
along this structure. Images at the top-left of the \(2\text{-}D\)
layout show the digit 1 angled more to the left, while images at the
bottom-right show the digit 1 angled more to the right.}
