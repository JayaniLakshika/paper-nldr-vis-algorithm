...
\(2\) and \(b_1 = \frac{n}{2}\) are allowed. FIG XXX(b) shows the effect
of different choices of \(b_1\) on the MSE of the fitted model. XXX Are
you showing \(b_1\) or \(b\) here???? \(b\)
...
the bins with density in the lowest 5\% of all bins, for example. FIG
XXXc illustrates the effect on the model representation of removing bins
below different percentages. Generally, we would urge caution in
removing low count bins.
...
that was used to generate the S-curve example data. The fitted model
should come reasonably close to this, if the fitting has been
successful. We don't ever have this, of course
