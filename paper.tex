...
applied to PBMC3k dataset with diffferent (hyper-)parameter choices.
What is the best (hyper-)parameter choice to create the model? The
residual plot have a steep slope at the beginning, indicating that a
smaller number of non-empty bins causes a larger amount of error. Then,
the slope gradually declines or level off, indicating that a higher
number of non-empty bins generates a smaller error. Using the elbow
method, it was observed that when the number of non-empty bins is set to
\(136\), the lowest error occurred with the parameters perplexity:
\(30\).}
...
\begin{center}
\includegraphics[width=1\textwidth,height=\textheight]{paper_files/figure-pdf/unnamed-chunk-60-1.pdf}
\end{center}
