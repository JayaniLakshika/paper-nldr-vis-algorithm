...
$b$ & number of bins in each direction\\
\addlinespace
$a$ & binwidths, distance between centroids in each direction\\
...
full bin lower, to allow a buffer corresponding to a full hexagon width
(\(a_1\)) and height (\(a_2\)) around the data. The values of \(b_i\)
are variables to be computed that define the reduction in size of the
data (\(n\) to \(m\)).
