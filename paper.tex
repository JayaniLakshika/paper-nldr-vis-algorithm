...
width (\(w_b\)) of a regular hexagon.)

\subsubsection{Computing hexagonal grid
configurations}\label{computing-hexagonal-grid-configurations}

To define a hexagonal grid across the \kD{} space, using hexagons with
fixed height (\(h_b\)), width (\(w_b\)), need to compute the how many
hexagons should represented by each axes in \kD{} space.

When \(k=2\), the number of bins along the \(x\) and \(y\) axes, \(b_1\)
and \(b_2\) is computed by accounting \({r_1, r_2}\), offset along the
axes (\(q_1\), \(q_2\)), and height (\(h_b\)), width (\(w_b\)) of a
regular hexagon.

(\(b_1 = \frac{r_1 + q_1}{w_b}\),
\(b_2 = \frac{r_2 + q_2}{0.75 \times h_b}\)): \(0.75 \times h_b\)
acommodate to have the tesselation without spacing.

\subsubsection{Hexagon binning}\label{hexagon-binning}

Define a hexagonal grid across the \kD{} space, using hexagons with
fixed height (\(h_b\)), width (\(w_b\)).

When \(k=2\),

The starting coordinates \((s_1, s_2)\) mark the lower left of the grid.
This is the bottom left bin centroid. By starting from there, points are
generated to fill the grid accounting \(b_1\) and \(b_2\).

Each of the \kD{} points will belong to a hexagonal bin. That is, for
each \(y \in \mathbfit{Y}\), we can (uniquely) identify the hexagon that
the point belongs to. This identification is done finding the nearest
bin centroid for the \kD{} points by considering the \kD{} Euclidean
distance.
...
computes a set of simplices such that no data points of X are contained
in any circumspheres of the simplices.

When \(k = 2\)

Delaunay triangulation of \(C^{(k)}\) is a triangulation of the convex
hull of \(C^{(k)}\) such that the circumcircle of every triangle in the
triangulation contains no other points from \(C^{(k)}\).
...
When \(k=2\). which bin centroids should connected are identified as a
result of triangulation. Therefore, corresponding \pD{} points should
