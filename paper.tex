...
Although there are several implementations of hexagon binning (REF), and
a published paper (REF), surprisingly, none has sufficient detail or
components that produce everything needed for this project. So we
described the process used here. Figure~\ref{fig-hex-param} illustrates
the notation used.
...
minimum data values, \(s_1 = -q\) and \(s_2 = -r_2*q\).
...
Delaunay triangulation \citep{lee1980} (XXXREF what R package are you
using?) is used to connect neighboring centroids. The edges are needed
to preserve neighborhood information when the model is lifted into
\pD{}.
