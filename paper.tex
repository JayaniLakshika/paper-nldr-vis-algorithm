...
Let \(u : \mathbb{R}^{n\times k} \rightarrow \mathbb{R}^{m\times k}\) be
...
points in \(H_h\) to map the centroid
\(C_{h}^{(k)} = (c_{h1}, c_{h2}, \dots, c_{hk})\) to a point in \pD{}.
Let the \pD{} mean be
...
\(\sum_{i = 1}^{n}\sum_{j = 1}^{p} (x_{ij} - C^{(p)}_{hx_ij})^2\).
Additionally, Mean Squared Error (MSE),
\(\frac{1}{n}\sum_{i = 1}^{n}\sum_{j = 1}^{p}(x_{ij} - C^{(p)}_{hx_ij})^2\),
