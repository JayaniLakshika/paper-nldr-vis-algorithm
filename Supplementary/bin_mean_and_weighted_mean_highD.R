...
hexbin_data_object <- extract_hexbin_mean(nldr_df = UMAP_data, num_bins = num_bins_x, shape_val = shape_value)

df_bin_centroids <- hexbin_data_object$hexdf_data

## To generate the grid
hexbin_data_object_n <- extract_hexbin_centroids(UMAP_data, num_bins_x, shape_val)
...
ggplot(data = hex_grid, aes(x = x, y = y)) + geom_polygon(fill = "white", color = "black", aes(group = id)) +
  ...
  geom_point(data = df_bin_centroids, aes(x = x, y = y), color = "black")
...
weight_df <- compute_weights(UMAP_data, hb_object)

df_bin <- weighted_highD_data(.data = df_all, weight_df)
