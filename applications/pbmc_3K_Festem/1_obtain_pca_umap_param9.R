...
UMAP_fit <- umap(training_data_pbmc |> dplyr::select(-ID), n_neighbors = 5, n_components =  2, metric = "cosine", min_dist = 0.9)
...
## Run only once
write_rds(UMAP_pbmc, file = "data/pbmc/pbmc_umap_5_min_dist_0.9.rds")
...
UMAP_pbmc <- read_rds("data/pbmc/pbmc_umap_5_min_dist_0.9.rds")
...
error_sum_df <- tibble::tibble(n_neighbors = 5, min_dist = 0.9, total_error = sum(error_df$total))
