...
plot_TriMAP_2D(TriMAP_zeisel) + #ggtitle("(b)") +
  theme_linedraw() +
...
num_bins_zeisel <- calculate_effective_x_bins(.data = TriMAP_zeisel, x = TriMAP1,
                                              cell_area = 1) ##48

shape_value_zeisel <- calculate_effective_shape_value(.data = TriMAP_zeisel,
                                                      x = TriMAP1, y = TriMAP2) ## 2.053351

## To extract bin centroids
hexbin_data_object_zeisel <- extract_hexbin_centroids(nldr_df = TriMAP_zeisel,
                                                      num_bins = num_bins_zeisel,
                                                      shape_val = shape_value_zeisel, x = TriMAP1, y = TriMAP2)
...
write_rds(hex_full_count_df, "data/zeisel/zeisel_trimap_hex_31.rds")

##########

ggplot(data = hex_full_count_df, aes(x = x, y = y)) +
  geom_polygon(color = "black", aes(group = polygon_id, fill = std_counts)) +
  geom_point(data = TriMAP_zeisel, aes(x = TriMAP1, y = TriMAP2), alpha = 0.5) +
...
TriMAP_zeisel_with_hb_id <- TriMAP_zeisel |>
  dplyr::mutate(hb_id = hexbin_data_object_zeisel$hb_data@cID)

## To generate a data set with high-D and 2D training data
df_all_zeisel <- dplyr::bind_cols(training_data_zeisel |> dplyr::select(-ID), TriMAP_zeisel_with_hb_id)
...
trimesh_zeisel_trimap <- ggplot(df_bin_centroids_zeisel, aes(x = x, y = y)) +
...
trimesh_zeisel_trimap <- trimesh_zeisel_trimap +
  #ggtitle("(a)") +
...
trimesh_gr_zeisel_trimap <- colour_long_edges(.data = distance_zeisel, benchmark_value = benchmark_zeisel,
                                              triangular_object = tr1_object_zeisel, distance_col = distance)

trimesh_gr_zeisel_trimap <- trimesh_gr_zeisel_trimap +
  geom_point(size = 2, colour = "#33a02c") +
  #ggtitle("(b)") +
  xlab(expression(C[x]^{(2)})) + ylab(expression(C[y]^{(2)})) +
  theme_light() +
...
trimesh_removed_zeisel_trimap <- remove_long_edges(.data = distance_zeisel, benchmark_value = benchmark_zeisel,
                                                   triangular_object = tr1_object_zeisel, distance_col = distance)

trimesh_removed_zeisel_trimap <- trimesh_removed_zeisel_trimap +
  geom_point(size = 2, colour = "#33a02c") +
  #ggtitle("(b)") +
  xlab(expression(C[x]^{(2)})) + ylab(expression(C[y]^{(2)})) +
  theme_light() +
...
tour1_zeisel_trimap <- show_langevitour(df_all_zeisel, df_bin_zeisel,
