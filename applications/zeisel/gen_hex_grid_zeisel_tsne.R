...
plot_tSNE_2D(tSNE_zeisel) + #ggtitle("(b)") +
  theme_linedraw() +
...
num_bins_zeisel <- calculate_effective_x_bins(.data = tSNE_zeisel, x = tSNE1,
                                              cell_area = 1) ##70

shape_value_zeisel <- calculate_effective_shape_value(.data = tSNE_zeisel,
                                                      x = tSNE1, y = tSNE2) ## 0.7831401

## To extract bin centroids
hexbin_data_object_zeisel <- extract_hexbin_centroids(nldr_df = tSNE_zeisel,
...
ggplot(data = hex_full_count_df, aes(x = x, y = y)) +
  ...
  geom_point(data = tSNE_zeisel, aes(x = tSNE1, y = tSNE2), alpha = 0.5) +
...
tSNE_zeisel_with_hb_id <- tSNE_zeisel |>
  dplyr::mutate(hb_id = hexbin_data_object_zeisel$hb_data@cID)

## To generate a data set with high-D and 2D training data
df_all_zeisel <- dplyr::bind_cols(training_data_zeisel |> dplyr::select(-ID), tSNE_zeisel_with_hb_id)
...
tour1_zeisel_tsne <- show_langevitour(df_all_zeisel, df_bin_zeisel,
