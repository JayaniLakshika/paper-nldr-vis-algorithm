...
plot_UMAP_2D(UMAP_zeisel) + #ggtitle("(b)") +
  theme_linedraw() +
...
num_bins_zeisel <- calculate_effective_x_bins(.data = UMAP_zeisel, x = UMAP1,
                                         cell_area = 1) ##18
...
write_rds(hex_full_count_df, "data/zeisel/zeisel_umap_hex_18.rds")

##########

ggplot(data = hex_full_count_df, aes(x = x, y = y)) +
  geom_polygon(color = "black", aes(group = polygon_id, fill = std_counts)) +
  geom_point(data = UMAP_zeisel, aes(x = UMAP1, y = UMAP2), alpha = 0.5) +
...
trimesh_zeisel_umap <- ggplot(df_bin_centroids_zeisel, aes(x = x, y = y)) +
...
trimesh_zeisel_umap <- trimesh_zeisel_umap +
  #ggtitle("(a)") +
...
trimesh_gr_zeisel_umap <- colour_long_edges(.data = distance_zeisel, benchmark_value = benchmark_zeisel,
                                triangular_object = tr1_object_zeisel, distance_col = distance)

trimesh_gr_zeisel_umap <- trimesh_gr_zeisel_umap +
  geom_point(size = 2, colour = "#33a02c") +
  #ggtitle("(b)") +
  xlab(expression(C[x]^{(2)})) + ylab(expression(C[y]^{(2)})) +
  theme_light() +
...
trimesh_removed_zeisel_umap <- remove_long_edges(.data = distance_zeisel, benchmark_value = benchmark_zeisel,
                                     triangular_object = tr1_object_zeisel, distance_col = distance)

trimesh_removed_zeisel_umap <- trimesh_removed_zeisel_umap +
  geom_point(size = 2, colour = "#33a02c") +
  #ggtitle("(b)") +
  xlab(expression(C[x]^{(2)})) + ylab(expression(C[y]^{(2)})) +
  theme_light() +
