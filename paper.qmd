...
With the model fit, some quirks can be observed. The simulated $4\text{-}D$ data having five Gaussian clusters is used. tSNE, and UMAP which showed the fitted model as *crumpled \gD{} sheet* that stretches in all four dimensions evident of preserving local structure. The flat model generated by PaCMAP evident the fail to capture the within cluster variation.
