...
The prediction approach involves employing the K-nearest neighbors (KNN) algorithm to identify the nearest hexagonal bin centroid in the 2D space. Subsequently, the coordinates of this centroid are assigned as the low-dimensional predicted values for the test data in 2D space. 

The prediction approach involves employing the K-nearest neighbors (KNN) algorithm to identify the nearest high-D model point (averaged high-D point) for the new high-D data point. Also, the coordinates of the high-D model point serve as the predicted 2D embedings for the new high-D data point. 
It is noteworthy that traditional NLDR methods, such as tSNE, often lack a direct predict function, making our approach valuable for generating predicted values in the absence of such functionalities.
-->  

The prediction approach involves employing the K-nearest neighbors (KNN) algorithm. In this method, which operates as an unsupervised classification problem, the nearest high-D model point (averaged high-D point) is identified for the new high-D data point. Once the nearest high-D model point is identified, its coordinates are used as the predicted 2D embeddings for the new high-D data point. This step is particularly valuable due to the limitations of certain NLDR techniques, like tSNE, which don't provide a straightforward method for predicting 2D embeddings. As a result, our approach presents a solution that can generate predicted 2D embeddings, irrespective of the NLDR technique employed, effectively addressing this gap in functionality. 
...
UMAP_pbmc_with_true_label <- UMAP_pbmc |>
  dplyr::mutate(true_class_label = case_when(
...
UMAP_pbmc_with_true_label <- UMAP_pbmc_with_true_label |>
  dplyr::mutate(true_class_label = if_else((ID %in% c(2619, 1158)), "cluster 3", true_class_label)) |>
  dplyr::mutate(true_class_label = if_else((ID %in% c(948, 285, 158, 268, 731, 1152, 1372, 1651, 2010, 150, 2304, 2546, 487, 2176, 2157, 2451, 264, 63, 1234, 947, 1126, 1699, 1232, 1492, 1488, 2040, 2550, 1313, 361, 372, 1515, 1264, 1307, 1726, 517, 2568, 2274, 591, 1703, 1097, 430, 1556, 2133, 1232, 1194, 1888, 1796, 494, 1194)), "cluster 6", true_class_label))
...
UMAP_pbmc_with_true_label |>
    ggplot(aes(x = UMAP1,
               y = UMAP2, color = true_class_label))+
    geom_point(alpha=0.5) +
    coord_equal() +
    theme(plot.title = element_text(hjust = 0.5, size = 18, face = "bold")) + #ggtitle("(a)") +
  theme_linedraw() +
