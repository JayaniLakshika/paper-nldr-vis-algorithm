...
#| fig-height: 10
#| fig-width: 10
#| fig-cap: "Model in \\gD{}, on the UMAP layout (a), and three views of the fit in projections from $9\\text{-}D$ (b, c, d), for the PBMC3k data ($a_1 = 0.046$, $b = 676 \\  (26, \ 26)$, $m = 92$, benchmark value to remove large edges is $0.08$). The three well-separated clusters observed in the \\gD{} layout of UMAP (@fig-NLDR-variety (a)) is also visible when visualizing the model overlaid on the data space. But the separation between the clusters are small in $9\\text{-}D$. The model shows non-linear clusters and densed points within clusters in $9\\text{-}D$ space, which are some additional patterns not visible in the \\gD{} representation (@fig-NLDR-variety (a)). The full video is available at <https://youtu.be/VqqWuE0Jj6A>."
