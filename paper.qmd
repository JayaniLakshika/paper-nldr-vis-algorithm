...
Then, after performing NLDR, to have regular hexagons, 2D embedding data are scaled accordingly.
...
## If same total number of bins ocurred only select ones with minimum error
duplicate_df_list <- mse_df_pbmc[duplicated(mse_df_pbmc$num_bins), ] |>
...
mse_df_pbmc <- dplyr::bind_rows(mse_df_pbmc[!duplicated(mse_df_pbmc$num_bins), ], duplicate_df)
```
