...
keywords-formatted: [high-dimensional data, data visualization, tour, statistical graphics, exploratory data analysis, unsupervised learning]
...
The model here is similar to a confirmatory factor analysis model [@brown2015], $\widehat{T} + \Epsilon$. The difference between the fitted model and observed values would be considered to be residuals, and are \pD{}. 
...
$$\sqrt{\frac{1}{n}\sum_{h = 1}^{m}\sum_{i = 1}^{n_h}\sum_{j = 1}^{p} (\mathbfit{x}_{hij} - C^{(p)}_{hj})^2}$${#eq-equation1} 
...
Several additional existing metrics have been used to assess the quality of the NLDR layout. A widely used approach is neighborhood preservation, quantified by the average K-ary neighborhood agreement between \pD{} and \kD{} spaces [@john2015]. This is summarized by the RNX curve, which rescales neighborhood preservation over different neighborhood sizes, and its area under the curve (AUC(R_NX)) provides a scalar summary of overall embedding quality across scales. Additional local and global measures include KNN Accuracy, which evaluates neighborhood consistency using leave-one-out classification, and SVM Accuracy, which assesses neighborhood cohesion with a nonlinear decision boundary [@yingfan2021]. Random Triplet Accuracy (RTA) and Centroid Triplet Accuracy quantify global structure preservation by comparing relative distances among sampled points or class centroids [@yingfan2021]. The Shepard diagram [@shepard1962] and its associated Spearman correlation (sc) [@spearman1961] between \pD{} and \kD{} pairwise distances also provide an interpretable diagnostic for assessing the monotonic relationship. Finally, the Global Score (GS) measures how well an embedding retains the overall geometry of the data relative to a PCA baseline, which minimizes reconstruction error [@amid2019]. Since PCA serves as the optimal linear method, we report RGS = 1 − GS in our comparisons to align the direction of the metric with NLDR methods. As our analysis focuses on unlabeled data, we use AUC(R_NX), RTA, r, and RGS for comparison with our proposed measure.

### Prediction into \gD{}

<!-- Does this really need an entire subsection? It's only 3 sentences long. -->

A new benefit of this fitted model is that it allows us to now predict the NLDR value of a new observation, $x'$, for any method. The steps are to determine the closest bin centroid in \pD{}, $C^{(p)}_{h}$ and predict it to be the centroid of this bin in \gD{}, $C^{(2)}_{h}$. 

It is important to note that similar out-of-sample extensions have been explored in existing methods. For example, PCA and autoencoders [@hinton2006] naturally support reverse mappings that enable reconstruction of the original data from the reduced space. Likewise, methods such as UMAP provide a `predict()` function for embedding new data points based on the learned manifold [@tomasz2023], and parametric tSNE [@van2009] learns an explicit mapping using neural networks. However, many NLDR methods (e.g., standard tSNE) are primarily designed for visualization and exploration rather than reconstruction, making out-of-sample prediction challenging.
...
Default values are provided for each of these, but deciding on the best model fit is assisted by examining the RMSE for a range of choices. <!--Choosing these parameters according to RMSE can be automated but it is recommended that the user examine the resulting model representation by overlaying it on the data in \pD{}. The next few subsections describe the calculation of default values, and the effect that different choices have on the model fit.-->
...
Standardised bin counts is computed as $w_h = n_h/n, ~~h=1, \dots m$, and $n$ is the number of observations. Density is computed as $d_h = w_h/A$ where $A$ is the area of the hexagon. As a measure of the denseness of a single grid, we can compute the average bin density, $\bar{d}$. This can be used to compare the model fit for different grids. 

These quantities can be used to assess and compare the models resulting from different binwidths:

- RMSE should decrease when binwidth decreases, as the binning gets closer to observations being in their own bin. But a big drop in RMSE would indicate the lower binwidth is substantially better than the larger one (@fig-param-two-curvy a).
- The proportion of non-empty bins is interesting to examine across different binwidths. A good binning should have just the right amount of bins to neatly cover the shape of the data, and no more or less (@fig-param-two-curvy b). 
- Bins with no observations or a small number might be dropped from the model. This will create the wireframe not extending into sparse areas, or allowing for holes in the data to be better captured. Note that, all observations are used for the RMSE calculation, though. RMSE can be examined relative to dropping a fraction of the low density bins (@fig-param-two-curvy c).
...
By default, when assessing the choice of horizontal bins, $b_1$, the total number of bins is measured by the number of **non-empty** bins ($m$). This more accurately reflects the hexagon grid relative to the RMSE than the full number of bins in the grid. It may also be beneficial to remove low count bins also, in the situation where data is clustered or stringy, where the observed data is sparse. In order to decide if this is necessary, you would examine the distribution of bin counts, or the density which puts the counts on a standard scale. If there is something of a gap at low values, this would suggest a potential value to use as a cutoff. Alternatively, one could choose to remove based on a percentile, the bins with density in the lowest $5\%$ of all bins, for example. @fig-param-two-curvy (c) illustrates the effect on the model representation of removing bins below different percentages. 
...
rmse_two_curvy_lwd <- ggplot(error_rm_two_curvy, 
  ...
  scale_linetype_manual(values = c("longdash", "solid", "dotted"),
                        labels = parse_format()) +
  xlab(expression(paste("std bin count (", w[h], ")"))) +
  ylab("RMSE") +
  #xlim(c(0, 0.14)) +
  ylim(c(0, 0.75)) +
...
```{r}
#| echo: false
#| fig-cap: "Various plots to help tune the model fit, using the binwidth $a_1$ and low density bin removal. In (a) RMSE against $a_1$, RMSE is going to increase as $a_1$ increases, but just before a big increase is a good choice. Here RMSE steadily increases so there is no clear choice - nevertheless, we have chosen three binwidths, $0.03$ (orange dashed), $0.05$ (blue solid), and $0.07$ (green dotted), to compare in the other plots. Proportion of non-empty bins tends to increase with $a_1$ (b). Removing low count bins changes RMSE substantially (c). The relative average bin density tends to increase with $a_1$ (d)."
...
rmse_two_curvy_b + a1_m_two_curvy +
...
Matching points in the \gD{} layout with their positions in \pD{} is useful for assessing the fit. This can be used to examine the fitted model in some subspaces in \pD{}, in particular in association with residual plots. 
...
Comparing the RMSE to obtain the best fit is appropriate if the same NLDR method is used. However, because the RMSE is computed on \pD{} data it measures the fit between model and data so it can also be used to compare the fit of different NLDR methods. A lower RMSE indicates a better NLDR representation.
...
comp_metrics <- ggparcoord(
  data = nldr_metrics,
  columns = 3:7,           # numeric columns to plot
...
While traditional metrics such as AUC(R_NX), RTA, and sc assess how well local or global structures are preserved, they can be influenced by hyper-parameter choices. In contrast, RMSE provides an interpretable, quantitative link between \pD{} and \gD{} that reflects how well the observed nonlinear structure is captured by the embedding. Therefore, RMSE should be viewed as a complementary diagnostic used alongside measures like RGS, RNX curves, and Shepard diagrams to evaluate both the overall quality and faithfulness of the representation. Together, these metrics allow analysts to distinguish visually appealing but misleading layouts from those that genuinely preserve the high-dimensional structure.

@fig-comp-metric illustrates how these different evaluation metrics behave across methods. The lines correspond to various NLDR representations, and each axis shows a normalized score for one evaluation criterion. Consistent patterns across multiple metrics indicate robustness, while strong divergence highlights trade-offs between preserving local structure (e.g., AUC(R_NX)) and global geometry (e.g., RGS). This multi-metric view underscores that no single metric fully captures accuracy of NLDR layouts, but together they reveal which method provides the most balanced and truthful representation of the high-dimensional data.

```{r}
#| label: fig-comp-metric
#| fig-cap: "Comparison of scaled evaluation metrics across NLDR representations. Each line represents a different layout evaluated using multiple metrics RTA, AUC(R_NX), sc, RGS, and RMSE. The variability across metrics highlights that no single measure universally captures embedding quality, underscoring the importance of considering multiple diagnostics when assessing representation faithfulness."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 10
#| fig-width: 10
...
<!--one_c_shaped_dens_structure/03_gen_model_with_tSNE.R-->
```{r}
...
error_plot_pbmc <- plot_rmse(error_pbmc) +
...
error_plot_mnist <- plot_rmse(error_mnist) +
...
Existing metrics for evaluating NLDR layout, such as AUC(R_NX), RTA, sc, and RGS, provide valuable but distinct numerical insights into the preservation of structure present in \pD{} data. Visualizing the model-in-the-data space offers a more comprehensive and interpretable diagnostic, allowing analysts to see the model fits everywhere, or better in some subspaces, or completely mismatches the data. This approach links the numerical summary with a visual understanding of how well the layout fits the data, helping analysts more easily decide whether the representation is reliable. 
...
#| label: tbl-fig-param
#| tbl-pos: H
#| tbl-cap: "NLDR methods and hyper-parameters used for each figure."
...
kable(fig_param_df, 
  ...
  kable_styling(position = "center", 
                ...
                font_size = 12) |>
  row_spec(0, bold = TRUE) |>
  column_spec(1:2, width = c("1.5cm", "2cm", "10cm"))
```
