...
By visualizing the model of different layouts in $4\text{-}D$ reveal different characteristics of each NLDR method. When examining the model of a specific cluster, which appears more like a filled-out shape in tSNE, and UMAP, while the model has a pancake shape in PaCMAP. The reason to have filled-out shaped clusters in tSNE and UMAP is that the ability of capture the local structure. On the other hand, the reason for the pancake shape in PaCMAP is the fail to capture the local structure.   
