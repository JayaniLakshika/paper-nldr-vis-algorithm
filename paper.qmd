...
Traditional metrics like AUC(R_NX), RTA, and sc focus on how well local or global structures are preserved, but their results can vary a lot depending on the method’s settings. In contrast, RMSE gives a more direct and easy-to-understand link between the \pD{} data space and the \gD{} space, showing how well the NLDR method capture the high-dimensional data structure.

@fig-comp-metric compares how these evaluation metrics behave across methods. Each line represents a different NLDR layout, and each axis shows a normalized score for one evaluation criterion. The variation across metrics highlights that they capture different aspects of structure preservation, and none behave exactly like RMSE. While RMSE provides a unique perspective on how well the high-dimensional structure is represented, only the RTA and sc metrics show a somewhat similar ordering across methods.

```{r}
#| label: fig-comp-metric
#| fig-cap: "Comparison of scaled evaluation metrics across  6 NLDR layouts on the 2NC7 data. Each line represents a different NLDR layout evaluated using multiple normalized metrics: RTA, AUC(R_NX), sc, RGS, and RMSE. The variation across metrics shows that each captures different aspects of structure preservation, and none behave exactly like RMSE. RTA, and sc show a somewhat similar ordering across methods."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 10
#| fig-width: 10
