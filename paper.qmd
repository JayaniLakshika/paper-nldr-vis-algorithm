...
The procedure for the new observation: (i) identify the closest bin centroid in \pD{}, $\mathbfit{c}^{(p)}_{h*}$, and (ii) assign the corresponding centroid in \gD{}, $\mathbfit{c}^{(2)}_{h*}$, as the predicted embedding. This is not entirely different from how UMAP provides predictions:  
...
#| fig-cap: "Comparison of predictions made my UMAP and by our method, relative to the training data. Dark points are the predicted test set, and lighter points are the training sample used to create the layouts. Panel (a) shows predictions obtained using the UMAP `predict()` function. Panels (b)–(f) show predictions obtained using our method applied to (b) UMAP, (c) tSNE, (d) PHATE, (e) TriMAP, and (f) PaCMAP layouts, respectively."
#| fig-height: 4
#| fig-width: 6
#| out-width: 80%
