...
The best model for the S-curve with UMAP (n_neighbors: $15$) create with $420$ of total number of bins. Also, the best model made after removing edges which has the length greater than $0.134$. Why this is the best? Because AIC plot shows that the minimum occur at $420$. Visually, the model capture the geometry of the S-curve well. 

On the other hand, a bad model for the S-curve with UMAP is made with $190$ bins, and removing edges which has the length greater than $0.3$. Why this is a bad model? The MSE error is greater than the best model. Visually, the model is not continuously fitted along with the S-curve data, and show some disconnect edges.  
