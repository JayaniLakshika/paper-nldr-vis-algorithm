...
colour = type)) +
  geom_point() +
  geom_line() +
   geom_vline(xintercept = 278, linetype="solid",
...
```{r}
#| echo: false
#| fig-cap: "Absolute error from UMAP applied to training PBMC3k dataset with diffferent parameter choices. What is the best parameter choice to create the model? The residual plot have a steep slope at the beginning, indicating that a smaller number of non-empty bins causes a larger amount of error. Then, the slope gradually declines or level off, indicating that a higher number of non-empty bins generates a smaller error. Using the elbow method, it was observed that when the number of non-empty bins is set to 278, the lowest error occurred with the parameters n_neighbors: 5 and min_dist: 0.3."
...
umap_pbmc <- read_rds("data/pbmc3k/pbmc_umap_5_min_dist_0.3.rds")
umap_pbmc_scaled <- as.data.frame(do.call(cbind, gen_scaled_data(data = umap_pbmc, 
                                                                 x = "UMAP1", y = "UMAP2"))) |>
  dplyr::mutate(cell_label = umap_pbmc$cell_label) |>
  dplyr::rename(c("UMAP1" = "scaled_UMAP1", 
...
```{r}
#| echo: false
#| fig-cap: "(a) 2D layout from UMAP (n_neighbors = 5, min_dist = 0.3) applied for the PBMC3k dataset. Is this a best representation of the original data?"
#| label: fig-umapsuggest
#| fig-pos: H
...
trimesh_removed_pbmc_umap <- trimesh_removed_pbmc_umap +
  geom_point(data = umap_pbmc_scaled, aes(x = UMAP1, y = UMAP2), alpha = 0.05) +
  theme_linedraw() +
  theme(plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
        axis.title.x = element_blank(), axis.title.y = element_blank(),
        axis.text.x = element_blank(), axis.ticks.x = element_blank(),
