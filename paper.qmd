...
@fig-comp-metric compares how these evaluation metrics behave across methods. Each line represents a different NLDR layout, and each axis shows a normalized score for one evaluation criterion. The variation across metrics highlights that they capture different aspects of structure preservation, and none behave exactly like RMSE. While RMSE provides a unique perspective on how well the high-dimensional structure is represented, only the RTA metric shows a somewhat similar ordering across methods.

```{r}
#| label: fig-comp-metric
#| fig-cap: "Comparison of scaled evaluation metrics across NLDR representations. Each line represents a different NLDR layout evaluated using multiple normalized metrics: RTA, AUC(R_NX), sc, RGS, and RMSE. The variation across metrics shows that each captures different aspects of structure preservation, and none behave exactly like RMSE. RTA shows a somewhat similar ordering across methods."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 10
#| fig-width: 10
