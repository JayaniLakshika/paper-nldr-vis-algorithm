...
Area of a hexagon is defined as $A = \frac{3\sqrt{3}}{2}l^2$ where $l$ is the side length of the hexagon. If we know $a_1$ and $a_2$, $l$ can be computed (see appendix). Therefore, the density of a hexagon grid is computed as $\frac{\sum^{h}_{i=1}n_h}{A}$ and the proportion is $\frac{\sum^{h}_{i=1}n_h}{A \times b}$. The base line proportion is defined as the  
...
wrap_plots(distance_points,distance_hist, ncol = 2) / wrap_plots(trimesh_two_curvy2, trimesh_two_curvy_removed2, trimesh_two_curvy_removed1,
...
#| fig-cap: "Best fit for two-curvy clusters data using UMAP (a1-a4) ($a_1$ = 0.09) and tSNE (b1-b4) ($a_1$ = 0.16) with the same number of non-empty bins ($m$ = 39). Both models show twist in $7\\text{-}D$ (a3, a4, b3, b4). Also, both NLDR methods are unable to accurately capture the width of the curvilinear clusters."
#| fig-width: 10
#| fig-height: 20
#| out-height: 90%

nldr_two_curvy_original2 + tsne_two_curvy_clust_plt +
