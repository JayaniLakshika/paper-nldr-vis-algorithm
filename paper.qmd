...
hex_param_vis <- ggplot(data = hex_grid, aes(x = x, y = y)) + 
    ...
    annotate("text", x=0.5, y=-0.45, label= expression(r[1]), color = "black") +
    geom_segment(x = -0.25, y = 0 - 0.05, xend = -0.25, yend = r2 - 0.05,
                 arrow = arrow(length = unit(0.03, "npc"), ends = "both"), color = "black")+ 
    annotate("text", x=-0.35, y=1, label= expression(r[2]), color = "black") +
    ...
    annotate("text", x=0.2, y=-0.25, label= expression(a[1]), color = "black") +
    ...
    annotate("text", x=-0.15, y=0.45, label= expression(a[2]), color = "black") +
    annotate("text", x=-0.18, y=-0.25, label= expression(group("(", list(s[1], s[2]), ")")), color = "black") ## starting point
```

```{r}
#| label: fig-hex-param
#| fig-cap: "Illustration of the components of the hexagon grid with notation."
#| out-height: 30%
#| fig-width: 3
#| fig-height: 4.5
#| fig-pos: H
...
::: {#fig-scurve-sc layout-ncol="3" fig-pos="H"}
...
Projections of the $7\text{-}D$ S-curve data (purple points) overlaid with the model (black grid with  green points indicating bin means) constructed from the UMAP layout, with XXX bins (using the **langevitour** software). The model closely fits the shape of this data. A video of the full tour is available at (<https://youtu.be/G1m4q9k--v4>).
:::
