...
a1_m_scurve <- ggplot(error_scurve,
  ...
  scale_x_continuous(breaks = sort(unique(round(error_scurve$a1, 2)))[c(1, 3, 5, 6, 8, 10:12)]) +
  labs(x = expression(paste("binwidth (", a[1], ")")), y = expression(paste("non-empty bins (", m, ")"))) +
  ggtitle("(b)") +
  theme_minimal() +
  theme(aspect.ratio = 0.75,
        panel.border = element_rect(fill = 'transparent'),
...
benchmark_label_df <- tibble(
  y = benchmark_scurve - 0.03, 
  x = 1.3, 
...
# and plot them on same aspect ratio
tsne_two_curvilinear_clust <- read_rds("data/two_nonlinear_clusters/two_nonlinear_clusters_tsne_perplexity_30.rds")
...
umap_two_curvilinear_clust <- read_rds("data/two_nonlinear_clusters/two_nonlinear_clusters_umap_n-neigbors_15_min-dist_0.1.rds")
...
phate_two_curvilinear_clust <- read_rds("data/two_nonlinear_clusters/two_nonlinear_clusters_phate_knn_5.rds")
...
trimap_two_curvilinear_clust <- read_rds("data/two_nonlinear_clusters/two_nonlinear_clusters_trimap_n-inliers_12_n-outliers_4_n-random_3.rds")
...
pacmap_two_curvilinear_clust <- read_rds("data/two_nonlinear_clusters/two_nonlinear_clusters_pacmap_n-neighbors_10_init_random_MN-ratio_0.5_FP-ratio_2.rds")
...
error_two_curvilinear_clust_umap <- read_rds("data/two_nonlinear_clusters/error_two_nonlinear_clusters_umap.rds")
error_two_curvilinear_clust_tsne <- read_rds("data/two_nonlinear_clusters/error_two_nonlinear_clusters_tsne.rds")
error_two_curvilinear_clust_phate <- read_rds("data/two_nonlinear_clusters/error_two_nonlinear_clusters_phate.rds")
error_two_curvilinear_clust_trimap <- read_rds("data/two_nonlinear_clusters/error_two_nonlinear_clusters_trimap.rds")
error_two_curvilinear_clust_pacmap <- read_rds("data/two_nonlinear_clusters/error_two_nonlinear_clusters_pacmap.rds")
...
#| fig-width: 15
#| fig-pos: H
#| fig-cap: "Model in \\gD{}, on the layouts from tSNE, UMAP, and PaCMAP (a1, b1, c1). Two views of the fit in projections of each layout from $4\\text{-}D$, for the Gaussian cluster data. All \\gD{} models show five well-separated clusters with different gaps. Layout a1 shows small separations, while layout b2 shows large gaps. On the other hand, layout b3 shows average gaps with exact positions as in $4\\text{-}D$. Assessing the different models fit for different layouts for a specific cluster shows filled-out shape in a1, a2, and b1, b2. On the other hand, c1 and c2 shows pancake shape.  The full video for the model for layout a, b, and c are available at <https://youtu.be/5Y1hE4i7N2k>, <https://youtu.be/5Y1hE4i7N2k>, and <https://youtu.be/5Y1hE4i7N2k> respectively."
...
```{r}
#| echo: false
#| fig-cap: "Model in \\gD{}, on the layouts, and two views of the fit in projections from $9\\text{-}D$, for the PBMC3k data. The \\gD{} model shows three well-separated clusters with large gaps in a1 ($a_1 = 0.05$, $b = 676 \\  (26, \ 26)$, $m = 92$, benchmark value to remove large edges is $0.08$), while a2 ($a_1 = 0.10$, $b = 195 \\  (13, \ 15)$, $m = 90$, benchmark value to remove large edges is $0.166$) shows small separation between the clusters. Because of the small separation, the three clusters are connected in \\gD{} and appear as long edges in $9\\text{-}D$. The full video for the model for layout a is available at <https://youtu.be/5Y1hE4i7N2k>. The full video for the model for layout c is available at <https://youtu.be/5Y1hE4i7N2k>."
...
tsne_minst_scaled_temp <- tsne_minst_scaled |>
  mutate(cluster = if_else((tSNE1 <= 0.92) & (tSNE1 >= 0.75) & (tSNE2 <= 0.62) & (tSNE2 >= 0.44), "small_clust", "big_clust"))
...
highd_error_2d_mnist <- error_df |>
  ...
  geom_rect(aes(xmin = 0.75, xmax = 0.85, ymin = 0.8, ymax = 0.95),
               fill = "transparent", color = "#377eb8", linewidth = 0.7)  +
...
#| fig-height: 10
#| fig-width: 10
#| fig-cap: "Model in \\gD{}, on the tSNE layout (a), and three views (b, c, d) of the fit in projections from $10\\text{-}D$, for the digit 1 of MNIST data ($a_1 = 0.07$, $b = 456 \\  (19, \ 24)$, $m = 226$, and benchmark value to remove large edges is $0.114$). (b) The non-linear structure observed in the \\gD{} layout of tSNE (@fig-tsne-best) is also visible when visualizing the model overlaid on the data space. This indicates that tSNE accurately captures the non-linear structure of the $10\\text{-}D$ data. (c) the model shows a twisted pattern within the non-linear structure in $10\\text{-}D$ space, which is an additional pattern not visible in the \\gD{} representation (@fig-tsne-best). (d) some long edges exist in the $10\\text{-}D$ space that are not recognized as long edges in the \\gD{} representation. The full video is available at <https://youtu.be/zcg_GXBmqjA>."
...
<!--proj for twisted pattern-->
```{r}
#| label: mnist-model-proj-twisted
## fourth projection
projection <- cbind(
  c(-0.03265,-0.43663,0.02143,0.07384,0.26237,0.20909,0.32398,-0.34487,0.03723,-0.04418),
  c(0.52862,-0.14177,0.20045,-0.37753,-0.09552,0.08851,0.07168,0.09255,-0.15653,-0.00030))
...
## fifth projection
projection <- cbind(
  c(-0.36688,-0.10781,-0.15474,-0.23005,-0.20194,-0.10841,-0.32443,0.04531,-0.36514,-0.14546),
  c(0.34256,-0.16215,-0.18747,-0.08515,-0.28689,-0.32807,0.28372,0.20894,-0.16990,0.09181))

projection_scaled <- projection * 2.3
...
```{r}
#| label: fig-mnist-tri-proj-twisted
#| fig-pos: H
#| fig-height: 10
#| fig-width: 10
#| fig-cap: "twisted pattern"

free(trimesh_removed_mnist) + free(trimesh_removed_mnist) + mnist_proj_tsne_model4 + mnist_proj_tsne_model5 +
  plot_layout(ncol = 2)
```
...
#| fig-cap: "(a) The $10\\text{-}D$ model error for tSNE layout in $\\gD{}$ of the MNIST digit 1 dataset. Most low model errors are distributed along the big non-linear clutser, while most large model errors are concentrated along the small cluster. (b) There are certain images that exhibit large errors due to their deviation from the non-linear cluster, which makes them anomalies. The images associated with large model errors shows different patterns of digit 1."
#| fig-pos: H
#| fig-width: 10
#| fig-height: 5
#| out-height: 80%
