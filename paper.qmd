...
In this paper, we present a novel method that aims to determine the most effective way to represent high-D data by selecting the best method and hyperparameter choice. Our approach involves dividing the data set into two parts: a training set to construct the model and a test set to generate predictive values and residuals. To implement our approach, we first use a 2D embedding data set as the initial point. The output of our algorithm is a tour that displays the model and original data in the high-dimensional space. A flow chart of the proposed algorithm is shown in @fig-meth. Our algorithm comprises two main phases: (1) generate the model in the 2D space, and (2) generate the model in the high-D space. These two phases are described in detail in this section using UMAP 2D embedding of the S-curve data set, which has seven dimensions, including four noise dimensions.
...
hex_full_count_df_loop2 <- hex_full_count_df_loop2 |>
  dplyr::mutate(type = if_else(std_counts < 0.12, "low", "high"))
...
#| echo: false
#| warning: false
#| fig-cap: "Goodness of fit statistics from different NLDR techniques applied to training S-curve dataset with different benchmark values to remove the low-density heaxgons. What is the effective benchmark value to remove the low-density heaxgons?"
...
#| echo: false
#| warning: false
#| fig-cap: "Goodness of fit statistics from different NLDR techniques applied to training S-curve dataset with different benchmark values to remove the long edges. What is the effective benchmark value to remove the long edges?"
...
$$
\text{MSE} = \sum_{j = 1}^{n} \frac{\sqrt{\sum_{i = 1}^{p} (x_{ij} - C_{x_ij})^2}}{n}
$$ {#eq-equation9}
...
In the process of selecting the most suitable NLDR technique to represent high-D data, the Akaike Information Criterion (AIC) stands as a crucial tool for insightful comparisons. Once the effective number of bins has been determined for each method, AIC values can be compared to identify the best NLDR technique with the lowest AIC value. For example, as shown in @fig-diagnosticpltScurve (a), TriMAP is the most suitable NLDR technique to represent the S-curve data (see table 1). -->
...
<!--By visualizing the models with original data in high-D space, we are able to understand how various techniques capture the clustering data structure. The tSNE model exhibits five well-separated clusters, effectively preserving both local and global structures. UMAP, on the other hand, also presents five distinct clusters but with a more flattened surface appearance. The PHATE model shows five clusters resembling triangles but struggles to capture local structures. TriMAP and PaCMAP both display five well-separated clusters with a flat surface appearance, capturing within-cluster variation to varying extents. As we can see, even though all NLDR techniques have different representations in 2D, all the NLDR techniques preserve the global structure, but some NLDR techniques fail to capture the local structure. According to the visualizations, we can see that tSNE preserve both local and global structure than other NLDR techniques that is also evident by the AIC plot shown in Figure 4.-->   
...
<!--In all NLDR techniques, except for PHATE, there are five well separated clusters. In tSNE, the five clusters are close to each other. However, in PHATE, two clusters are really close to each other and other three clusters are far away from them. In PaCMAP, one cluster is at the center and other four positioned in four directions. In TriMAP also, there are two clusters close to each other and other three are well-separated from each other. In UMAP, four clusters are positioned parallel to each and remaining one far away from them.-->

<!--The model with tSNE shows five well-separated clusters. Also, within cluster variation is captured by giving proof that with clustering tSNE preserve local and global structures of high-D data.

The model with UMAP also shows five well-separated clusters. But now, the clusters are more flat surfaces.

The model with PHATE also shows five well-separated clusters. The five clusters are more like triangles and couldn't able to capture local structure at all.

In the model with TriMAP shows five well-separated clusters. Also, within cluster variation is captured but much like flat surfaces.

In the model with PaCMAP shows five well-separated clusters. Also, within cluster variation is captured but much like flat surfaces.-->
...
# error_df <- read_csv("data/pbmc/pbmc_3k_festem/error_pbmc_umap.csv")
# 
# error_df |>
#   knitr::kable() 
```

| n_neighbors | min_dist | metric     | error                 |
|-------------|----------|------------|-----------------------|
| **30**      | **0.3**  | **cosine** | **38634.66719286174** |
| 5           | 0.99     | euclidean  | 35938.98594054407     |
| 5           | 0.99     | cosine     | 35597.01981261912     |
| 15          | 0.99     | euclidean  | 35534.75778935427     |
| 30          | 0.99     | euclidean  | 35664.68763936665     |
| 1022        | 0.99     | cosine     | 37022.0125249776      |
| 1022        | 0.3      | cosine     | 39746.347504639125    |
| 800         | 0.3      | euclidean  | 38385.25485420897     |
| 100         | 0.99     | euclidean  | 36528.57424747017     |
| 230         | 0.99     | cosine     | 37864.26802578186     |
| 354         | 0.99     | euclidean  | 37530.86598361975     |
...
#tSNE_pbmc <- read_rds("data/pbmc/pbmc_tsne_30.rds")
UMAP_pbmc <- read_rds("data/pbmc/pbmc_umap_15_min_dist_0.99_metric_euclidean.rds")
# PHATE_pbmc <- read_rds("data/pbmc/pbmc_phate.rds")
# TriMAP_pbmc <- read_rds("data/pbmc/pbmc_trimap.rds")
# PaCMAP_pbmc <- read_rds("data/pbmc/pbmc_pacmap.rds")
...
```{r}
#| echo: false
#| fig-cap: "(a) 2D layout from UMAP (n_neighbors = 15, min_dist = 0.99, metric = euclidean) applied for the PBMC3k dataset. Is this a best representation of the original data?, (b)  Model in the 2D space with UMAP (<>)"
...
tSNE_pbmc <- read_rds("data/pbmc/pbmc_3k_festem/pbmc_tsne_5.rds")
...
#| warning: false
#| echo: false
num_bins_pbmc <- calculate_effective_x_bins(.data = tSNE_pbmc, x = tSNE1,
                                            cell_area = 1) ##23

num_bins_pbmc <- 23

shape_value_pbmc <- calculate_effective_shape_value(.data = tSNE_pbmc,
                                                    x = tSNE1, y = tSNE2) ## 0.8772751
shape_value_pbmc <- 0.8772751

## To extract bin centroids
hexbin_data_object_pbmc <- extract_hexbin_centroids(nldr_df = tSNE_pbmc,
                                                    num_bins = num_bins_pbmc,
                                                    shape_val = shape_value_pbmc, x = tSNE1, y = tSNE2)

df_bin_centroids_pbmc <- hexbin_data_object_pbmc$hexdf_data


tSNE_pbmc_with_hb_id <- tSNE_pbmc |>
  dplyr::mutate(hb_id = hexbin_data_object_pbmc$hb_data@cID)

## To generate a data set with high-D and 2D training data
df_all_pbmc <- dplyr::bind_cols(training_data_pbmc |> dplyr::select(-ID), tSNE_pbmc_with_hb_id)
...
trimesh_removed_pbmc_tsne <- remove_long_edges(.data = distance_pbmc, benchmark_value = benchmark_pbmc,
                                               triangular_object = tr1_object_pbmc, distance_col = distance)
...
```{r}
#| echo: false
#| fig-cap: "(a) 2D layout from tSNE (perplexity = 5) applied for the PBMC3k dataset. Is this a best representation of the original data?, (b)  Model in the 2D space with tSNE (<>)"
#| label: fig-nldervisPBMCtSNE1
#| out-width: 100%
#| fig-pos: h
...
tSNE_pbmc <- read_rds("data/pbmc/pbmc_3k_festem/pbmc_tsne_2.rds")
...
```{r}
#| echo: false
#| fig-cap: "(a) 2D layout from tSNE (perplexity = 2) applied for the PBMC3k dataset. Is this a best representation of the original data?, (b)  Model in the 2D space with tSNE (<>)"
#| label: fig-nldervisPBMCtSNE2
#| out-width: 100%
#| fig-pos: h
...
plot_list2_harps + trimesh_removed_tsne_harps +
  plot_layout(guides='collect', ncol = 2) &
  theme(legend.position='bottom', plot.tag = element_text(size = 8))
