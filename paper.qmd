...
error_df <- error_df |>
  dplyr::mutate(type = paste0(n_neighbors, "_", min_dist))
...
The MNIST dataset consists of grayscale images of handwritten digits [@lecun2010]. @Yingfan2021 used this dataset to demonstrate how the PaCMAP preserves local structure. We select the 2D embedding of PaCMAP for the handwritten digit 1. As shown in @fig-pacmapauthor, we observe a nonlinear structure present in 2D space. To answer the following questions,
...
imge_error_sample1 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
              axis.title.x = element_blank(), axis.title.y = element_blank(),
              axis.text.x = element_blank(), axis.ticks.x = element_blank(),
              axis.text.y = element_blank(), axis.ticks.y = element_blank(),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(7679, 3568, 1312, 3311, 1097)) 

theme_set(theme_light())
imge_error_sample2 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(3552, 7853, 6489, 7689, 6690)) 

theme_set(theme_light())
imge_error_sample3 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(1380, 6057, 2347, 5946, 3355)) 

theme_set(theme_light())
imge_error_sample4 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(4175, 3997, 5378, 387, 1854)) 

theme_set(theme_light())
imge_error_sample5 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(614, 3079, 1762, 5239, 3723)) 

theme_set(theme_light())
imge_error_sample6 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(5748, 728, 7419, 7794, 6233)) 

theme_set(theme_light())
imge_error_sample7 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(33, 2485, 5998, 318, 1761)) 

theme_set(theme_light())
imge_error_sample8 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(5690, 165, 517, 6935, 2682)) 

theme_set(theme_light())
imge_error_sample9 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(4962, 2264, 5563, 6369, 559)) 

theme_set(theme_light())
imge_error_sample10 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(5188, 4849, 2666, 3448, 3055)) 

theme_set(theme_light())
imge_error_sample11 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(3120, 6869, 6345, 4470, 7147)) 

theme_set(theme_light())
imge_error_sample12 <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ instance, ncol = 5) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
```{r}
#| echo: false
#| fig-cap: "Images of the handwritten digit 1 are distributed along the left-to-right of first embedding axis of the PaCMAP nonlinear structure."
#| label: fig-sampleimg
#| out-width: 100%
#| fig-pos: H

imge_error_sample1 + imge_error_sample2 + imge_error_sample3 + imge_error_sample4 + imge_error_sample5 + imge_error_sample6 + imge_error_sample7 + imge_error_sample8 + imge_error_sample9 + imge_error_sample10 + imge_error_sample11 + imge_error_sample12 +
  plot_annotation(tag_levels = 'a') +
...
```{r}
#| echo: false
#| fig-cap: "(a) Model in the 2D space overlaid on PaCMAP data, and (b) high-D model error in model space."
