...
To illustrate the method and how to use it to choose from a selection of layouts, we generated \sD{} simulated data, which we call the "2NC7" data. It has two separated nonlinear clusters, one forming a \gD{} curved shape, and the other a \tD{} curved shape, each consisting of $1000$ observations. The first four variables, $X_1, X_2, X_3, X_4$ hold this cluster structure, and the remaining three $X_5, X_6, X_7$ are purely noise. Because, no error was simulated in the first four variables, we would consider $T=(X_1, X_2, X_3, X_4)$ to be **the geometric structure (true model)** that we hope to capture. In practice, all variables might have some small error, and structure to detect might be in any or all variables. The 2NC7 data is simple but complex enough illustrate the new methodology. We can also consider the two separated clusters as *global structure*, and the nonlinear surfaces to be relatively *local structure*. @fig-NLDR-two-curvy uses an **ideal NLDR layout** of 2NC7, which reveals the two clusters with moderate separation (global), and flattens the curvilinear forms while (hopefully) preserving the proximity of points (local).

```{r}
#| label: fig-NLDR-two-curvy
#| echo: false
#| fig-cap: "Key steps for constructing the model on the (ideal) tSNE layout ($k=2$) of 2NC7: (a) data, (b) hexagon bins, (c) bin centroids, and (d) triangulated centroids. The 2NC7 data is shown."
#| fig-width: 12
#| fig-height: 4
#| out-width: 100%
