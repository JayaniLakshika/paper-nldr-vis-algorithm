...
The UMAP algorithm constructs a fuzzy topological representation of the high-dimensional data and then optimizes the low-dimensional embedding to be as similar to this representation as possible [@Leland2018].

The UMAP algorithm constructs a weighted k-nearest neighbors graph based on the local similarity of data points in the original high-dimensional space. The weighted graph is defined by a conditional probability distribution $P_{ij}$, which represents the probability of observing data point $x_i$ given data point $x_j$ as a neighbor:

$$
P_{ij} = \frac{\exp(-\lVert x_i - x_j \rVert^2)}{\sum_{k \neq i} \exp(-\lVert x_i - x_k \rVert^2)}
$$

Similarly, a fuzzy topological representation of the data is constructed in the low-dimensional space with conditional probability distribution $Q_{ij}$:

$$
Q_{ij} = \frac{\exp(-\lVert y_i - y_j \rVert^2)}{\sum_{k \neq i} \exp(-\lVert y_i - y_k \rVert^2)}
$$

where $y_i$ and $y_j$ are the corresponding low-dimensional representations of data points $x_i$ and $x_j$.

UMAP minimizes the cross-entropy between the high-dimensional fuzzy topological representation $P_{ij}$ and the low-dimensional representation $Q_{ij}$ to find an optimal low-dimensional embedding. The loss function for UMAP is defined as:

$$
L = \sum_{i} \sum_{j} P_{ij} \log \frac{P_{ij}}{Q_{ij}}
$$

The algorithm uses stochastic gradient descent to optimize the positions of the data points in the low-dimensional space to minimize this loss function.

In R, the `umap` package is often used to implement UMAP and generate results.

The "n_neighbors" parameter in UMAP determines the number of nearest neighbors to consider for each data point during the construction of the fuzzy topological representation. Higher values of "n_neighbors" capture more global structure, while lower values emphasize local structure. Choosing an appropriate value for "n_neighbors" is crucial, as it affects the balance between local and global structure in the resulting low-dimensional representation.
...
tSNE_data <- Fit_tSNE(training_data, opt_perplexity = calculate_effective_perplexity(training_data), with_seed = 20230531)
...
#(knn: 5)
# PHATE_data <- Fit_PHATE(training_data, knn = 5, with_seed = 20230531)
# write_csv(PHATE_data, paste0(here::here(), "/data/phate_data_s_curve.csv"))

PHATE_data <- read_csv(paste0(here::here(), "/data/phate_data_s_curve.csv"))
...
TriMAP_data <- read_csv(paste0(here::here(), "/data/trimap_data_s_curve.csv"))
...
PaCMAP_data <- read_csv(paste0(here::here(), "/data/pacmap_data_s_curve.csv"))
...
In this step, the algorithm proceeds to triangulate the centroids (see @fig-meth (Step 3)) of the hex bins using the `tripack` package in R [@article54]. Since we are working with the centroids of regular hexagonal bins, the resulting mesh will predominantly comprise equal-sized regular triangles. However, the triangulation also helps span any gaps that may exist between clusters of points, allowing for a more complete and interconnected representation of the data.
...
a1 <- colour_long_edges(.data = distance, benchmark_value = 13, 
                  triangular_object = tr1_object, distance_col = distance) + 
    geom_point(size = 2, colour = "#33a02c") + 
    #ggtitle("(b)") + 
    xlab(expression(C[x]^{(2)})) + ylab(expression(C[y]^{(2)})) +
...
UMAP_data_with_hb_id_selected <- UMAP_data_with_hb_id |> dplyr::filter(hb_id %in% c(14, 177)) |>
  dplyr::mutate(select_point = if_else(hb_id == 14, "data1", "data2"))
...
a2 <- ggplot(data = hex_full_count_df, aes(x = x, y = y)) + 
  ...
  geom_point(data = df_bin_centroids_all |> dplyr::filter(hexID %in% c(14, 177)), aes(x = x, y = y), colour = "#33a02c") +
  scale_fill_viridis_c(direction = -1, na.value = "#ffffff", option = "C") +
  theme_light() +
  theme(legend.position = "none", plot.title = element_text(size = 5, hjust = 0.5, vjust = -0.5),
          axis.title.x = element_blank(), axis.title.y = element_blank(),
...
### Summaries of the model {#sec-diagnostics}

#### Predicted values and residuals

In our prediction process, we employ the K-nearest neighbors (KNN) algorithm to predict the hex bin IDs. Specifically, for each new high-dimensional data point, the algorithm assigns the first neighboring hex bin ID as the predicted hex bin ID.

<!-- move to goodness of fit stat-->
...
Here, $b$ signifies the total number of bins, $p$ denotes the number of dimensions in the high-dimensional (high-D) data, and $n$ represents the number of observations.
...
predict_umap_plot <- plot_UMAP_2D(UMAP_data) +
...
<!--predict hexid for the test data -->

```{r}
#| echo: false

pred_df_training_object <- predict_hex_id(training_data = training_data, nldr_df = UMAP_data, nldr_df_test = UMAP_data, num_bins = num_bins_x, shape_val = shape_value)
...
eval_df_training <- generate_eval_df(data = data, prediction_df = pred_df_training, df_bin_centroids = centroid_df_training, df_bin = avg_df_training, num_bins = num_bins_x)

pred_df_test_object <- predict_hex_id(training_data = training_data, nldr_df = UMAP_data, nldr_df_test = predict_UMAP_df, num_bins = num_bins_x, shape_val = shape_value)
...
eval_df_test <- generate_eval_df(data = data, prediction_df = pred_df_test, df_bin_centroids = centroid_df_test, df_bin = avg_df_test, num_bins = num_bins_x)
```
...
predict_hex_ID_plot <- ggplot(data = hex_grid, aes(x = x, y = y)) + geom_polygon(fill = "white", color = "black", aes(group = id)) +
...
<!--To draw predict values in hexbin grid-->
...
predict_umap_plot  + predict_hex_ID_plot +
  plot_layout(guides='collect', ncol = 3) &
  theme(legend.position='bottom', plot.tag = element_text(size = 8))
```
...
```{r}
#| echo: false
#num_bins_vec <- append(sample(1:sqrt(750), 10), num_bins_x)

#num_bins_vec <- sample(2:1000, 40)
num_bins_vec <- 1:10
...
aic_plot <- ggplot(MSE_df |> dplyr::filter(data_type == "training"), aes(x = number_of_bins,
  ...
  annotate("text", x= (NROW(full_grid_with_hexbin_id) - 10), y=-15000, label=paste0("effective number of bins = ",as.character(NROW(full_grid_with_hexbin_id))), angle=90, size = 2.5) + 
  theme(legend.position = "none", legend.title=element_blank(), plot.title=element_text(size=8), axis.text = element_text(size = 5),
          axis.title = element_text(size = 7),
...
mse_plot <- ggplot(MSE_df, aes(x = number_of_bins,
  ...
  #geom_smooth(method = "loess", se = FALSE) +
  geom_vline(xintercept = NROW(full_grid_with_hexbin_id)) +
  annotate("text", x= (NROW(full_grid_with_hexbin_id) - 10), y=0.05, label=paste0("effective number of bins = ", as.character(NROW(full_grid_with_hexbin_id))), angle=90, size = 2.5) +
...
In this analysis, we're looking at how well our model is doing in two aspects. First, we're checking the distances in our lower-dimensional space (see @fig-diag (a)). We want to see if the distances between different types of edges are similar. If they are, it means our model is doing a good job of keeping the relationships consistent. If there are big differences, it could mean our model is struggling with certain types of edges.

Next, we're examining how our model handles the relationship between high-dimensional and low-dimensional distances for each type of edge (see @fig-diag (b)). If our data points nicely line up around the expected line, it tells us that our model is preserving the original structure well. If they're scattered, it could mean our model is having trouble with specific data points. This two-part analysis helps us really understand how our model fits the data.

```{r}
#| echo: false

sorted_distance_df <- distance[order(distance$distance),] ## Sort the distances

sorted_distance_df <- cbind(sorted_distance_df, rbind(NA, apply(sorted_distance_df, 2, diff))) ## Calculate differences between unique distance
sorted_distance_df <- sorted_distance_df[, c(3,6)]
sorted_distance_df[is.na(sorted_distance_df)] <- 0.0000 ## To replace missing values with zero
names(sorted_distance_df)[2] <- "difference"

sorted_distance_df <- sorted_distance_df %>% 
  mutate(Edge_type = ifelse(distance >= benchmark, "Long edge", "Short edge"))

diagnostic_plot1 <- ggplot(sorted_distance_df, aes(x = distance, y = difference, color = Edge_type)) +
  geom_point(alpha = 0.5) +
  geom_rug(col="#e31a1c",alpha=0.1, linewidth=1.5) +
  #ggtitle("Visualization of difference between \n 2D distances by edge type.") +
...
```{r}
#| echo: false
#| fig-cap: The two diagnostic plot assess the effectiveness of the high-D representation algorithm  performed for the **s_curve_noise_umap** data set$\colon$ (a) visualizing the differences between low-D distances and (b) visualizing the relationship between high-D ($d^{(p)}$) and low-D distances ($d^{(2)}$). 
#| label: fig-diag
#| out-width: 100%
...
Our algorithm for high-dimensional (high-D) representation is available as an open-source R package named quollr. This package provides analysts with the capability to construct a low-dimensional manifold and seamlessly visualize it in the high-D space alongside the original data.-->

### Simulated data examples {#sec-simpleex}
...
#| warning: false
#| echo: false 
tSNE_data_gau <- Fit_tSNE(training_data_5 |> dplyr::select(-ID, -cluster), with_seed = 20230531)
```
...
data_pca_with_label <- readRDS(file = paste0(here::here(), "/data/pbmc_with_label.rds"))
...
var_explained_df <- readRDS(file = paste0(here::here(), "/data/pbmc_var_explained.rds"))

pbmc_var_explain <- var_explained_df %>%
    ggplot(aes(x = PC,y = var_explained, group = 1))+
    geom_point(size=1)+
    geom_line() +
    theme_minimal() +
...
UMAP_fit_pbmc <- umap(data_pca, n_neighbors = 15, n_components =  2)

UMAP_data_pbmc <- UMAP_fit_pbmc$layout |>
  as.data.frame()

names(UMAP_data_pbmc)[1:(ncol(UMAP_data_pbmc))] <- paste0(rep("UMAP",(ncol(UMAP_data_pbmc))), 1:(ncol(UMAP_data_pbmc)))

UMAP_data_pbmc <- UMAP_data_pbmc |>
  mutate(ID = row_number())

plot_pbmc <- UMAP_data_pbmc %>%
    ggplot(aes(x = UMAP1,
               y = UMAP2, color = data_pca_with_label$cluster_label))+
    geom_point(alpha=0.5) +
    coord_equal() +
...
num_bins_pbmc <- calculate_effective_x_bins(.data = UMAP_data_pbmc, x = UMAP1,
                                        cell_area = 1)
shape_val_pbmc <- calculate_effective_shape_value(.data = UMAP_data_pbmc,
                                              x = UMAP1, y = UMAP2)

## To extract bin centroids
hexbin_data_object_pbmc <- extract_hexbin_centroids(nldr_df = UMAP_data_pbmc, num_bins = num_bins_pbmc, shape_val = shape_val_pbmc)
...
UMAP_data_with_hb_id_pbmc <- UMAP_data_pbmc |> 
  dplyr::mutate(hb_id = hexbin_data_object_pbmc$hb_data@cID)
        
## To generate a data set with high-D and 2D training data
data_pca_n <- data_pca
names(data_pca_n) <- paste0(rep("x", 7), 1:7)
df_all_pbmc <- dplyr::bind_cols(data_pca_n, UMAP_data_with_hb_id_pbmc)
        
## Averaged on high-D
df_bin_pbmc <- avg_highD_data(.data = df_all_pbmc)
...
names(df_all_pbmc)[1:7] <- paste0(rep("PC_", 7), 1:7)
names(df_bin_pbmc) <- c("hb_id", paste0(rep("PC_", 7), 1:7))

# tour_pbmc <- show_langevitour(df_all_pbmc, df_bin_pbmc, df_bin_centroids_pbmc, benchmark_value = benchmark_pbmc, distance = distance_pbmc, distance_col = distance, col_start = "PC")

df_1 <- df_all_pbmc |> 
  dplyr::select(tidyselect::starts_with("PC")) |>
  dplyr::mutate(type = data_pca_with_label$cluster_label) ## original dataset
...
df_exe <- dplyr::bind_rows(df_b_1, df_1)
...
trimesh_pbmc <- ggplot(df_bin_centroids_pbmc, aes(x = x, y = y)) +
  geom_trimesh() +
  coord_equal() 

trimesh_pbmc <- trimesh_pbmc +
  #ggtitle("(b)") +
  xlab(expression(C[x]^{(2)})) + ylab(expression(C[y]^{(2)})) +
  theme(axis.text = element_text(size = 5),
          axis.title = element_text(size = 7))

trimesh_gr_pbmc <- colour_long_edges(.data = distance_pbmc, benchmark_value = benchmark_pbmc,
                 triangular_object = tr1_object_pbmc, distance_col = distance)

trimesh_gr_pbmc <- trimesh_gr_pbmc +
  #ggtitle("(c)") +
  xlab(expression(C[x]^{(2)})) + ylab(expression(C[y]^{(2)})) +
  theme(axis.text = element_text(size = 5),
          axis.title = element_text(size = 7), #change legend key width
...
tour_pbmc <- langevitour::langevitour(df_exe[1:(length(df_exe)-1)], lineFrom = distance_df_small_edges$from, lineTo = distance_df_small_edges$to, group = df_exe$type, levelColors = c("#33a02c", "#b2df8a", "#fb9a99", "#ff7f00", "#e31a1c", "#1f78b4", "black", "#b15928", "#6a3d9a", "#cab2d6"), pointSize = 3)
```
...
#| label: fig-pbmc
#| fig-pos: H
#| fig-cap: Visualization and refinement of low-dimensional representation for pbmc data$\colon$ (a) 2D layout of UMAP (n-neigbors = 15), (b) triangular mesh, and (c) triangular mesh colored by edge type. Edges with lengths less than the benchmark value of $2$ are classified as small, while edges with lengths greater than or equal to this value are labeled as long. The combination of UMAP, hexagonal binning, and triangulation provides the effectiveness of our approach in visualizing and refining the low-dimensional representation of the pbmc data.


plot_pbmc + trimesh_pbmc + trimesh_gr_pbmc + #A = 1, b = (19, 27), s = 1.255103
  plot_annotation(tag_levels = 'a') +
  plot_layout(guides='collect') &
  theme(legend.position='bottom', plot.tag = element_text(size = 8))
```
...
::: {#fig-pbmc_sc layout-ncol="3"}
![](figures/pbmc_a.png){#fig-pbmc_sc1}

![](figures/pbmc_b.png){#fig-pbmc_sc2}

![](figures/pbmc_c.png){#fig-pbmc_sc3}

Screen shots of the **langevitour** with the low-dimensional view applied to pbmc data set, a video of the tour animation is available at (\url{https://drive.google.com/file/d/1F7Xb2EIpfUruCxYihPw2rKro4FB5Y2uL/view}). Combination of the **langevitour** and low-dimensional representation of the pbmc data provides the effectiveness of our approach.
:::

### MNIST

The MNIST dataset is a widely recognized benchmark in the realm of computer vision and machine learning, which is a collection of 28x28 pixel grayscale images of handwritten digits (0 through 9), serving as a foundational resource for developing and evaluating various image classification algorithms. With its simplicity and clarity, MNIST provides an ideal platform for researchers and practitioners to experiment with and benchmark their models. 
