...
We can consider this procedure to be a pseudo-inverse of $g$, $\mathbfit{C}^{p} = \tilde{g}(\mathbfit{C}^{(2)})$, $\tilde{g}\text{:}~ \mathbfit{C}^{(2)} \subset \mathbb{R}^2 \mapsto \mathbfit{C}^{p} \subset \mathbb{R}^p$. Then $\mathbfit{\hat{X}}_i (i=1, \dots, n) \in \{\mathbfit{C}^{(p)} = (C^{(p)}_{j1}, C^{(2)}_{j2}), j=1, \dots, m\}$ are considered the fitted values, from which we can also calculate \pD{} residuals.
