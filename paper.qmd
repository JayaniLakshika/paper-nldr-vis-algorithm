...
num_bins_x_two_curvy <- 22

algo_obj_two_curvy <- gen_nldr_vis_algo_obj(
  high_d_data = training_data_two_curvy, 
  nldr_data = tsne_two_curvy, 
  num_x_bins = num_bins_x_two_curvy)

tsne_two_curvy_scaled <- algo_obj_two_curvy$nldr_scaled
distance_two_curvy <- algo_obj_two_curvy$distance_df
tr_from_to_df_two_curvy <- algo_obj_two_curvy$tr_from_to_df
benchmark_two_curvy <- algo_obj_two_curvy$benchmark
df_bin_centroids_two_curvy <- algo_obj_two_curvy$df_bin_centroids
df_bin_two_curvy <- algo_obj_two_curvy$df_bin
hex_grid_with_counts <- algo_obj_two_curvy$hex_grid_with_counts
hex_grid_nonempty <- algo_obj_two_curvy$hex_grid_nonempty
bin_width <- algo_obj_two_curvy$a1
...
hex_grid_two_curvy <- ggplot(
  ...
  interior_annotation("b", sc_ltr_pos, cex = 1.5)
...
hex_grid_nonempty_two_curvy <- ggplot(
  ...
  geom_point(data = df_bin_centroids_two_curvy, 
             ...
             size = 1) +
  xlim(sc_xlims) + ylim(sc_ylims) +
  interior_annotation("c", sc_ltr_pos, cex = 1.5) 
```
...
trimesh_removed_two_curvy <- ggplot() +
  ...
  coord_equal() +
  xlim(sc_xlims) + ylim(sc_ylims) +
  interior_annotation("d", sc_ltr_pos, cex = 1.5)
...
langevitour::langevitour(df_model_data_two_curvy_filtered[1:(length(df_model_data_two_curvy_filtered)-1)],
                         lineFrom = distance_df_small_edges_two_curvy$from,
                         lineTo = distance_df_small_edges_two_curvy$to,
                         group = factor(df_model_data_two_curvy_filtered$type,
                                        c("data", "model")),
                         levelColors = c(clr_choice, "#000000"))
```
...
proj_obj1 <- get_projection(projection = model_prj1, 
               ...
               distance_df_small_edges = distance_df_small_edges_two_curvy, 
...
algo_obj_two_curvy2 <- gen_nldr_vis_algo_obj(
  high_d_data = training_data_two_curvy, 
  nldr_data = tsne_two_curvy, 
  num_x_bins = num_bins_x_two_curvy)

distance_two_curvy2 <- algo_obj_two_curvy2$distance_df
tr_from_to_df_two_curvy2 <- algo_obj_two_curvy2$tr_from_to_df
benchmark_two_curvy2 <- algo_obj_two_curvy2$benchmark
df_bin_centroids_two_curvy2 <- algo_obj_two_curvy2$df_bin_centroids
df_bin_two_curvy2 <- algo_obj_two_curvy2$df_bin
hex_grid_with_counts_two_curvy2 <- algo_obj_two_curvy2$hex_grid_with_counts

distance_df_small_edges_two_curvy2 <- distance_two_curvy2 |>
  filter(distance < benchmark_two_curvy2) 

tr_from_to_df_two_curvy2 <- inner_join(
...
distance_two_curvy3 <- algo_obj_two_curvy3$distance_df
tr_from_to_df_two_curvy3 <- algo_obj_two_curvy3$tr_from_to_df
benchmark_two_curvy3 <- algo_obj_two_curvy3$benchmark
df_bin_centroids_two_curvy3 <- algo_obj_two_curvy3$df_bin_centroids
df_bin_two_curvy3 <- algo_obj_two_curvy3$df_bin
hex_grid_with_counts_two_curvy3 <- algo_obj_two_curvy3$hex_grid_with_counts

distance_df_small_edges_two_curvy3 <- distance_two_curvy3 |>
  filter(distance < benchmark_two_curvy3) 

tr_from_to_df_two_curvy3 <- inner_join(
  tr_from_to_df_two_curvy3, distance_df_small_edges_two_curvy3, 
  by = c("from", "to"))
...
mse_two_curvy_b <- ggplot(error_two_curvy_umap, 
  ...
  geom_point(size = 1) +
  scale_x_continuous(breaks = sort(unique(round(error_two_curvy_umap$a1, 2)))[c(2, 5, 8, 10, 12, 14, 15, 16, 17)]) +
...
#| fig-cap: "The $4\\text{-}D$ model error in \\gD{} layout. Color indicates square root of total error, dark colour indicating high error and light indicates low error. Most large errors are distributed near the sparse end of the non-linear cluster with dense corner."
#| out-width: 100%
#| fig-pos: H

error_plot_two_curvy + error_plot_two_curvy_hist +
  plot_layout(ncol=2)
```
...
```{r}
#| echo: false
#| fig-cap: "Various plots to help assess best hexagon bin parameters (a, b, d) and thresholds to remove low-density bins (c). Both (a) and (c) show MSE, against binwidth ($a_1$) and threshold. A good benchmark value for these parameters is when the MSE drops and then flattens out. Three binwidth choices were made: $0.03$, $0.06$, and $0.09$ to investigate. As the binwidth increases, the proportion of non-empty bins also increases (b). The relative proportion density decreases and levels off (d). Binwidth $0.06$ is chosen as the initial best binwidth for further analysis. There is no need to remove the low-density hexagons because as shown in (b), there is no considerable drop in MSE."
...
nldr_two_non_linear_diff_shaped_close_clusters6 <- tsne_two_non_linear_diff_shaped_close_clusters |>
  ggplot(aes(x = tSNE1,
             y = tSNE2))+
  geom_point(alpha=0.1, size=1, colour='#984ea3') +
...
error_two_non_linear_diff_shaped_close_clusters <- bind_rows(error_two_curvy_umap, 
...
error_plot_two_non_linear_diff_shaped_close_clusters <-
  plot_mse(error_two_non_linear_diff_shaped_close_clusters) +
  scale_x_continuous(breaks =
    sort(unique(error_two_non_linear_diff_shaped_close_clusters$a1))[
      seq(1, length(
        unique(error_two_non_linear_diff_shaped_close_clusters$a1)), 
        by = 5)]) +
  scale_color_manual(
...
error_plot_two_non_linear_diff_shaped_close_clusters + wrap_plots(nldr_two_non_linear_diff_shaped_close_clusters1, nldr_two_non_linear_diff_shaped_close_clusters2, nldr_two_non_linear_diff_shaped_close_clusters3, 
                                    nldr_two_non_linear_diff_shaped_close_clusters4, nldr_two_non_linear_diff_shaped_close_clusters5,
                                    nldr_two_non_linear_diff_shaped_close_clusters6, ncol = 2, 
                                    widths = c(50, 50))
```
...
df_bin_two_curvy4 <- avg_highd_data(data = df_all_two_curvy4)
...
## First projection
projection <- cbind(
  c(-0.08707,-0.06493,0.07017,-0.07349),
  c(0.03398,0.03735,0.09272,0.01871))
...
error_df_gau_abs <- augment(
  highd_data = training_data_gau,
  model_2d = df_bin_centroids_gau1,
  model_highd = df_bin_gau1)
...
distance_df_small_edges <- distance_gau1 |>
  dplyr::filter(distance < 0.1) |>
  mutate(ID = row_number()) |>
  filter(!(ID %in% c(150, 110))) |>
  dplyr::select(-ID) #benchmark_gau1

# write_rds(distance_df_small_edges, "data/five_gau_clusters/tsne_wireframe.rds")
# write_rds(df_b, "data/five_gau_clusters/tsne_model.rds")
```
...
proj_obj1 <- get_projection(projection = projection, 
               proj_scale = 1, 
               scaled_data = scaled_gau_data, 
               scaled_data_model = scaled_gau_data_model, 
...
distance_df_small_edges_gau1 <- distance_gau1 |>
  filter(distance < 0.05) #|> benchmark_gau1
  #mutate(ID = row_number()) #|>
  #filter(!(ID %in% c(112)))
...
## Set the maximum difference as the criteria
distance_df_small_edges <- distance_gau1 |>
  dplyr::filter(distance < 0.05) #benchmark_gau1
...
# write_rds(distance_df_small_edges, "data/five_gau_clusters/umap_wireframe.rds")
# write_rds(df_b, "data/five_gau_clusters/umap_model.rds")
```
...
distance_df_small_edges_gau1 <- distance_gau1 |>
  filter(distance < 0.06) #|> #benchmark_gau1
  #mutate(ID = row_number()) #|>
  #filter(!(ID %in% c(97)))
...
# write_rds(df_b, "data/five_gau_clusters/pacmap_model.rds")
...
## Set the maximum difference as the criteria
distance_df_small_edges <- distance_gau1 |>
  dplyr::filter(distance < 0.06) #benchmark_gau1 #benchmark_gau1

# write_rds(distance_df_small_edges, "data/five_gau_clusters/pacmap_wireframe.rds")
# write_rds(df_b, "data/five_gau_clusters/pacmap_model.rds")
```
...
algo_obj_dens_clust <- gen_nldr_vis_algo_obj(
  high_d_data = one_c_shaped_data, 
  nldr_data = tsne_one_c_shaped, 
  num_x_bins = num_bins_x_one_c_shaped)

tsne_one_c_shaped_scaled <- algo_obj_dens_clust$nldr_scaled
distance_dens_clust <- algo_obj_dens_clust$distance_df
tr_from_to_df_dens_clust <- algo_obj_dens_clust$tr_from_to_df
benchmark_dens_clust <- algo_obj_dens_clust$benchmark
df_bin_centroids_dens_clust <- algo_obj_dens_clust$df_bin_centroids
df_bin_dens_clust <- algo_obj_dens_clust$df_bin
hex_grid_with_counts_dens_clust <- algo_obj_dens_clust$hex_grid_with_counts
```
...
error_plot_one_curvy_hist_selected <- ggplot(error_df_one_curvy_abs, 
      aes(x = sqrt_row_wise_total_error, 
          y = density,
          colour = factor(error_cat_n, levels = c("deselected", "selected")))) +
...
error_plot_one_curvy_hist <- ggplot(error_df_one_curvy_abs,
        aes(x = sqrt_row_wise_total_error, 
            y = density,
            colour = factor(error_cat_n2, levels = c("deselected", "selected")))) +
  geom_point(alpha=0.7) +
  scale_color_manual(values=c('#d9d9d9', '#800026')) +
  xlab(expression(e[hj])) +
  ylab("") +
...
distance_df_small_edges_c_shaped_structure <- distance_dens_clust |>
  filter(distance < benchmark_dens_clust)

tr_from_to_df_dens_clust <- inner_join(
  tr_from_to_df_dens_clust, distance_df_small_edges_c_shaped_structure,
  by = c("from", "to"))
...
five_c_shaped_proj_tsne_model1 <- plot_proj(
    proj_obj = proj_obj1,
    point_param = c(1.5, 0.5), # size, alpha, color
    line_param = c(0.8, 0.4, "#000000"), # linewidth, alpha
...
five_c_shaped_proj_tsne_model1_selected <- plot_proj(
    proj_obj = proj_obj1, 
    point_param = c(1.5, 0.5), # size, alpha, color
    line_param = c(0.8, 0.4, "#000000"), # linewidth, alpha
    plot_limits = c(-0.35, 0.35), 
...
algo_obj_pbmc <- gen_nldr_vis_algo_obj(
  high_d_data = training_data_pbmc, 
  nldr_data = umap_pbmc, 
  num_x_bins = num_bins_x_pbmc)

umap_pbmc_scaled <- algo_obj_pbmc$nldr_scaled
distance_pbmc <- algo_obj_pbmc$distance_df
tr_from_to_df_pbmc <- algo_obj_pbmc$tr_from_to_df
benchmark_pbmc <- algo_obj_pbmc$benchmark
df_bin_centroids_pbmc <- algo_obj_pbmc$df_bin_centroids
df_bin_pbmc <- algo_obj_pbmc$df_bin

distance_df_small_edges_pbmc <- distance_pbmc |>
  filter(distance < benchmark_pbmc)#benchmark_pbmc
...
proj_obj1 <- get_projection(projection = projection, 
               proj_scale = 1.23, 
               scaled_data = scaled_pbmc_data, 
               scaled_data_model = scaled_pbmc_data_model, 
               distance_df_small_edges = distance_df_small_edges_pbmc, 
               axis_param = list(limits = 1,
                                 axis_scaled = 1, 
                                 axis_pos_x = -0.48, 
                                 axis_pos_y = -0.48, 
...
pbmc_proj_umap_model1 <- plot_proj(
  ...
  cex = 2, 
  axis_text_size = 4,
  is_color = TRUE) + scale_color_manual(values = c("#8dd3c7", "#fdb462", "#bebada"))
...
pbmc_scaled_obj <- gen_scaled_data(
  data = tsne_pbmc)
tsne_pbmc_scaled_best <- pbmc_scaled_obj$scaled_nldr
...
algo_obj_pbmc <- gen_nldr_vis_algo_obj(
  high_d_data = training_data_pbmc, 
  nldr_data = tsne_pbmc, 
  num_x_bins = num_bins_x_tsne_pbmc)

tsne_pbmc_scaled_best <- algo_obj_pbmc$nldr_scaled
distance_pbmc <- algo_obj_pbmc$distance_df
tr_from_to_df_pbmc <- algo_obj_pbmc$tr_from_to_df
benchmark_pbmc <- algo_obj_pbmc$benchmark
df_bin_centroids_pbmc <- algo_obj_pbmc$df_bin_centroids
...
#   mutate(ID = row_number()) |>
#   filter(!(ID %in% c(19, 20, 23, 24, 29, 33, 48,
#                      183, 181, 178, 176))) #19
...
error_plot_mnist <- plot_mse(error_mnist) +
  scale_x_continuous(breaks = sort(
    unique(error_mnist$a1))[append(seq(1, length(unique(error_mnist$a1)), 
                                       by = 5), 24)]) +
  scale_color_manual(
    values=c('#e41a1c','#377eb8',"#ff7f00", 
             '#984ea3', '#999999', '#a65628')) +
  theme(aspect.ratio=1.5)
...
By visualizing the model generated for tSNE in $10\text{-}D$ helps to assess the \gD{} layout. As shown in @fig-mnist-tri-proj (a1), the model provides the evidence for the non-linear structure of the digit 1 data in $10\text{-}D$. The model shows some quirks. The model's twisted pattern provides evidence for the $10\text{-}D$ data structure, which is not observed by \gD{} layout (@fig-mnist-tri-proj (a2)). 
...
algo_obj_mnist <- gen_nldr_vis_algo_obj(
  high_d_data = training_data_mnist, 
  nldr_data = tsne_mnist2, 
  num_x_bins = num_bins_x_mnist)

tsne_minst_scaled <- algo_obj_mnist$nldr_scaled
distance_mnist <- algo_obj_mnist$distance_df
tr_from_to_df_mnist <- algo_obj_mnist$tr_from_to_df
benchmark_mnist <- algo_obj_mnist$benchmark
df_bin_centroids_mnist <- algo_obj_mnist$df_bin_centroids
df_bin_mnist <- algo_obj_mnist$df_bin

sc_ltr_pos_mnist <- c(0.96, 0.96)

distance_df_small_edges_mnist <- distance_mnist |>
  filter(distance < benchmark_mnist) #benchmark_mnist

tr_from_to_df_mnist <- right_join(
  tr_from_to_df_mnist, distance_df_small_edges_mnist, 
  by = c("from", "to"))

# tr_from_to_df_mnist <- tr_from_to_df_mnist |>
#   mutate(ID = row_number()) |>
#   filter(!(ID %in% c(1205, 1629, 1170)))
...
proj_obj1 <- get_projection(projection = projection, 
               proj_scale = 2.5, 
               scaled_data = scaled_mnist_data, 
               scaled_data_model = scaled_mnist_data_model, 
               distance_df_small_edges = distance_df_small_edges_mnist, 
               axis_param = list(limits = 1, 
                                 axis_scaled = 1, 
                                 axis_pos_x = -0.92, 
                                 axis_pos_y = -0.92, 
...
#| fig-width: 15
#| out-width: 100%
#| fig-cap: "The tSNE layout of the MNIST digit 1 data shows a big non-linear cluster (grey) and a small cluster (orange) located very close to the one corner of the big cluster in \\gD{} (a). The brushing feature in the linked plots helps in visualizing the closeness of the small cluster to the big cluster. Videos of the linked plots are available at <>."
...
img_right_top <- c(868, 1465, 1787)

pixels_gathered_within <-  mnist_data |>
  filter(instance %in% img_right_top)

right_top_img <- plot_digit_img(pixels_gathered_within, palette = "Reds 2", title_text = "(a)", hjust = 0.31) 

img_middle <- c(130, 2091, 7795)

pixels_gathered_within <-  mnist_data |>
  filter(instance %in% img_middle)

middle_img <- plot_digit_img(pixels_gathered_within, palette = "Greens 3", title_text = "(b)", hjust = 0.31) 

img_right_bottom <- c(2035, 7708, 7710)

pixels_gathered_within <-  mnist_data |>
  filter(instance %in% img_right_bottom)

right_bottom_img <- plot_digit_img(pixels_gathered_within, palette = "Purples 2", title_text = "(c)", hjust = 0.31) 

img_error_outside <- c(999, 3650, 4916)

pixels_gathered_outside <-  mnist_data |>
  filter(instance %in% img_error_outside)

imge_error_sample_outside <- plot_digit_img(pixels_gathered_within, palette = "Grays", title_text = "(d)", hjust = 0.31) 

##### inside
img_error_inside <- c(3179, 7363, 7650, 5241, 5173, 6981, 7228, 6673, 4636)

pixels_gathered_inside <-  mnist_data |>
  filter(instance %in% img_error_inside)

imge_error_sample_inside <- plot_digit_img(pixels_gathered_inside, palette = "Grays", title_text = "(e)", hjust = 0, ncol = 9)
...
top_layout <- right_top_img / middle_img / right_bottom_img / imge_error_sample_outside
full_layout <- highd_error_2d_mnist + top_layout

# Add the new full-width bottom row
final_layout <- full_layout / imge_error_sample_inside + plot_layout(heights = c(1, 0.2))
...
Our algorithm can also be useful in practice to decide on a reasonable NLDR layout that accurately represents the data, as described with a single-cell gene expression data, **PBMC3k**. The published results of UMAP with default (hyper-)parameter setting by @chen2023 were evaluated. The data showed three clusters, two of which were close. But the published layout showed three well-separated clusters. Therefore, by changing the (hyper-)parameter, we found a better layout with UMAP, which is more representative the data. Additionally, the model discovers non-uniform data distribution and non-linear structures within the clusters that are not visible in the UMAP layout, demonstrating the ability of our model in uncovering hidden data characteristics. Also, as explained in Appendix, with recently introduced approach, scDEED also proved the layout suggested by our layout is better than the published one.  
...
These `R` packages were used for the work: `tidyverse` (@hadley2019), `Rtsne` (@jesse2015), `umap` (@tomasz2023), `patchwork` (@thomas2024), `colorspace` (@achim2020), `langevitour` (@harisson2024), `conflicted` (@hadley2023), `reticulate` (@kevin2024), `kableExtra` (@hao2024). These `python` packages were used for the work: `trimap` (@amid2019) and `pacmap` (@yingfan2021). The article was created with `R` packages `rticles` (@jjallaire2024), `knitr` (@yihui2014), and `rmarkdown` (@yihui2020). The project's GitHub repository ([https://github.com/JayaniLakshika/paper-nldr-vis-algorithm](https://github.com/JayaniLakshika/paper-nldr-vis-algorithm)) contains all materials required to reproduce this article.
