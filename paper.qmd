...
true_model_df <- read_rds("data/two_curvy_diff_clust/two_curvy_diff_clust_true_model.rds")
wireframe_true_model <- read_rds("data/two_curvy_diff_clust/two_curvy_diff_clust_true_model_connections.rds")
...
error_two_curvy_diff_clust_umap <- error_two_curvy_diff_clust_umap |>
  group_by(a1) |>
  filter(bin1 == min(bin1)) |>
  ungroup() |>
  mutate(prop_dens = 1/(b*side_length^2)) 

base_line_prop_dens <- error_two_curvy_diff_clust_umap |>
  filter(a1 == min(a1)) |>
  pull(prop_dens)
...
mse_two_curvy_b <- ggplot(error_two_curvy_diff_clust_umap, 
                     aes(x = a1, 
                         y = MSE)) +
  ...
  geom_point(size = 1) +
  scale_x_continuous(breaks = sort(unique(round(error_two_curvy_diff_clust_umap$a1, 2)))[c(1, 4, 8, 10, 11, 12, 13, 14)]) +
...
prop_dens_a1 <- ggplot(error_two_curvy_diff_clust_umap,
                     aes(x = a1,
                         y = prop_comp)) +
  geom_vline(xintercept = 0.09,
             linetype="solid",
...
a1_m_two_curvy <- ggplot(error_two_curvy_diff_clust_umap,
                     aes(x = a1,
                         y = prop_bins)) +
  geom_vline(xintercept = 0.09,
             linetype="solid",
...
predict_umap_two_curvy <- read_rds(file = "data/two_curvy_diff_clust/two_curvy_diff_clust_umap_predict_true.rds")
...
error_rm_two_curvy <- read_rds("data/two_curvy_diff_clust/error_rm_lwd_diff_bin.rds") |>
...
# Read a variety of different NLDR representations of two_curvy_diff_clust
# and plot them on same aspect ratio
tsne_two_curvy_diff_clust <- read_rds("data/two_curvy_diff_clust/two_curvy_diff_clust_tsne_perplexity_30.rds")

nldr_two_curvy_diff_clust1 <- tsne_two_curvy_diff_clust |>
  ggplot(aes(x = tSNE1,
             y = tSNE2))+
  geom_point(alpha=0.1, size=1, colour='#a65628') +
...
nldr_two_curvy_diff_clust2 <- umap_two_curvy_diff_clust |>
  ggplot(aes(x = UMAP1,
             y = UMAP2)) +
...
nldr_two_curvy_diff_clust3 <- phate_two_curvy_diff_clust |>
  ggplot(aes(x = PHATE1,
             y = PHATE2))+
  geom_point(alpha=0.1, size=1, colour='#ff7f00') +
  interior_annotation("c") +
...
nldr_two_curvy_diff_clust4 <- trimap_two_curvy_diff_clust |>
  ggplot(aes(x = TriMAP1,
             y = TriMAP2))+
  geom_point(alpha=0.1, size=1, colour='#4daf4a') +
  interior_annotation("d") +
...
nldr_two_curvy_diff_clust5 <- pacmap_two_curvy_diff_clust |>
  ggplot(aes(x = PaCMAP1,
             y = PaCMAP2))+
  geom_point(alpha=0.1, size=1, colour='#e41a1c') +
  interior_annotation("e") +
...
```{r}
#| label: combine-data-two_curvy_diff_clust
...
```{r}
#| label: error-comp-two_curvy_diff_clust
...
#| label: fig-two_curvy_diff_clust-mse
#| fig-pos: H
#| out-height: 60%

free(error_plot_two_curvy_diff_clust) + wrap_plots(nldr_two_curvy_diff_clust1, nldr_two_curvy_diff_clust2, nldr_two_curvy_diff_clust3, 
                                    nldr_two_curvy_diff_clust4, nldr_two_curvy_diff_clust5, ncol = 2)
```
...
tsne_two_curvy_diff_clust_plt <- tsne_two_curvy_scaled |>
  ggplot(aes(x = tSNE1,
             y = tSNE2))+
  geom_point(alpha=0.1, size=1, colour='#a65628') +
  interior_annotation("b1", sc_ltr_pos) +
...
nldr_two_curvy_original2 + tsne_two_curvy_diff_clust_plt +
