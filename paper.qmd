...
umap_pbmc <- read_rds("data/pbmc3k/pbmc_umap_3_min_dist_0.6.rds")
nldr2 <- umap_pbmc |>
  ggplot(aes(x = emb1,
             y = emb2))+
  geom_point(alpha=0.1, size=1, colour=clr_choice) +
...
df_model_data_n <- bind_rows(data_gau, df_b)
...
df_model_data_pbmc_n <- bind_rows(df_b_pbmc, data_pbmc)
...
error_pbmc <- error_pbmc |>
  mutate(method = factor(method,
                         levels = c("UMAP_3_min_dist_0.6", "UMAP_30_min_dist_0.3", "UMAP_5_min_dist_0.01","tsne_5", "tsne_30", "phate_5", "trimap_12_4_3", "pacmap")))
```
...
umap_pbmc2 <- read_rds("data/pbmc3k/pbmc_umap_3_min_dist_0.6.rds")
pbmc_scaled_obj <- gen_scaled_data(
  data = umap_pbmc2)
...
umap_pbmc_scaled_best_with_cluster <- umap_pbmc_scaled_best |>
...
trimesh_removed_pbmc_best <- ggplot() + 
  geom_segment(data = tr_from_to_df_pbmc, 
               ...
               linewidth = 1) +
  geom_point(
    data = umap_pbmc_scaled_best_with_cluster,
    aes(
      x = emb1,
      y = emb2,
      color = cluster
...
```{r}
#| label: prep-pbmc-umap-model-proj-best
...
```{r}
#| label: langevitour-pbmc-umap-best-proj
#| eval: false
...
df_model_data_mnist_n <- bind_rows(data_mnist_cluster, df_b_mnist)
