...
#  scale_x_continuous(breaks = sort(unique(round(error_two_curvy_tsne$a1, 2)))[c(1, 4, 7, 11, 13, 14, 15, 16)]) +
  ylim(c(0, 0.75)) +
  labs(x = "relative average bin density", y = "HBE") +
  #ylim(c(0, 0.75)) +
...
It is worthwhile to consider what are desirable aspects of a hexbin result, that maps to summarising the \pD{} fit well. The binning should capture the underlying data distribution closely, with minimum number of necessary bins. An ideal binning might be indicated by a more uniform distribution of bin counts, or having few relatively empty bins. To help with this assessment average standardized bin count ($\bar{w} = \sum{w_h}/m$), average density ($\bar{d} = \sum{d_h}/m$) and proportion of non-empty bins ($m/b$), are also computed. @fig-param-two-curvy shows some choices of plots of these quantities for a single NLDR layout, with three choices of $a_1$ indicated. Some expectations and reasoning for these plots is:
...
rmse_two_curvy_lwd <- ggplot(error_rm_two_curvy, 
  ...
  scale_linetype_manual(values = c("longdash", "solid", "dotted"),
                        labels = parse_format()) +
  xlab(expression(paste("low density bin cutoff based on ", d[h]))) +
  ylab("HBE") +
  #xlim(c(0, 0.14)) +
  ylim(c(0, 0.75)) +
...
nldr_metrics <- inner_join(nldr_metrics, error_two_curvy_filtered, by = c("Method" = "method")) |>
  rename("sc" = "r") |>
  #mutate(sc = 1 - sc) |>
  mutate(RGS = 1-RGS, `AUC(R_NX)` = 1 - `AUC(R_NX)`) |>
  rename(SC = sc)

comp_metrics <- ggparcoord(
  data = nldr_metrics,
  columns = c(4,3,5:7),           # numeric columns to plot
...
XXX For HBE what binwidth was used? Is what I said correct.

<!-- Current metrics like AUC(R_NX), RTA, and SC focus on how well local or global structures are preserved, but their suggestions best layout can be very different. In contrast, HBE gives a more direct and easy-to-understand link between the \pD{} data space and the \gD{} space, showing how well the NLDR method capture the high-dimensional data structure.-->

@fig-comp-metric compares the metrics AUC(R_NX), RTA, SC, RGS, along with HBE computed on $a_1=0.05$ for the six layouts shown in @fig-toy-rmse. This is a parallel coordinate plot where the y-axis shows a normalized score to ensure the metrics are on the same scale. Each line corresponds to one layout. The metrics AUC(R_NX) and RGS (XXX which direction are these normally read? when is lowest best? when is highest? all need ot be lowest is best to match HBE) have been reversed so that they most align with HBE, and the lower the value the better the layout. There is some agreement between the metrics. All, except AUC(R_NX) agree that layout d is worst. All agree that layout f is best or very close to best. Layout a is best according to HBE and AUC(R_NX) but considered to be much less optimal by RTA, SC and RGS. Layout c is considered favorably by RTA, SC and RGS but to be very poor by AUC(R_NX). This illustrates how difficult it is to use the numerical metrics alone to decide on the best layout. When they disagree is causes confusion for the analyst, and thus provides a temptation to choose the nicest looking layout, even though it may be sub-optimal. Because HBE is accompanied with a representation of the layout in \pD{} to compare with the observed data, it can help to add more clarity in making decisions.
...
#| fig-cap: "Comparison of scaled evaluation metrics (RTA, AUC(R_NX), SC, RGS, and HBE using $a_1=0.05$) for the six NLDR layouts computed on the 2NC7 data using a parallel coordinate plot. Color of the line indicates NLDR layout. AUC(R_NX) and RGS are reversed so that lower is best like the other metrics. There is some general agreement, and small disagreements, with AUC(R_NX) being most different. All agree that layout f is good, and almost all that d is bad. Interestingly, there is disagreement on layout a, which is almost the same as layout f."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 6
#| fig-width: 6

comp_metrics + wrap_plots(nldr_two_curvy1, nldr_two_curvy2, nldr_two_curvy3, 
                                    nldr_two_curvy4, nldr_two_curvy5,
                                    nldr_two_curvy6, ncol = 2, 
                                    widths = c(50, 50))
```
