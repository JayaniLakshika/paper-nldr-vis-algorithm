...
<!-- The prediction approach involves performing the K-nearest neighbors (KNN) algorithm for an unsupervised classification problem. First, the nearest \pD{} model point is identified for a given new \pD{} point. Then, the corresponding \kD{} centroid mapping for the identified \pD{} model point is determined. Finally, the coordinates of this \kD{} centroid are used as the predicted \kD{} embedding for the new \pD{} data point. . -->

We define the function $z: \mathbb{R}^{n\times p} \rightarrow \mathbb{R}^{m\times p}$.

For the $i^{th}$ point, $z(i) = \arg\min_{j = 1, \dots, b} \sqrt{\sum_{v=1}^{P}(x_{iv} - C^{(2)}_{jv})^2}$, and assign it to $H_h = \{i| z(i) = h\}$. Then, the fitted values for $i$ are defined as $C_{h}^{(2)} = (c_{h1}, c_{h2})$. 

#### Error calculation

To assess how well our method captures and represents the underlying structure of the \pD{} data, residuals are important. Residuals are computed by taking the squared \pD{} Euclidean distance, $\sum_{i = 1}^{n}\sum_{j = 1}^{p} (x_{ij} - C^{(p)}_{hj})^2$. Additionally, Mean Squared Error (MSE), $\frac{1}{n}\sum_{i = 1}^{n}\sum_{j = 1}^{p}(x_{ij} - C^{(p)}_{hj})^2$, is computed based on residuals.  
...
The S-curve example demonstrates how the model accurately fits the points. Viewing the wireframe with a tour reveals how well the model represents the underlying geometry of the S-curve, highlighting its ability to preserve both local and global structures in high-dimensional space. 

As demonstrated in the simulation case study, all observed NLDR methods preserve the global structure, while tSNE is the only method that preserves the local structure. Our approach helps in determining this and illustrates how different methods have specific quirks.

Even though three distant well-separated clusters have shown in the UMAP layout of **pbmc** suggested by @chen2023, the model evident that the clusters are close to each other in high dimension. Also, viewing the model in the data space shows data patches which shows the data are not uniformly distributed within the clusters and non-linear structures within clusters. These summaries can't be observed in UMAP layout. Therefore, model in high dimension is useful to obtain some quirks like these.

On the other hand, the error or unexplained variance is important in evaluating how well the model fit the data. By observing error for different number of bins, tSNE with perplexity 30 selects as a reasonable representation for **pbmc**. Connecting closest clusters by line segments in the fitted model provides evidence of preserving the neighborhood.   

We have shown in the **digit: 1**, how the model accurately represents the non-linear structure. Also, provides some unobserved information in \gD{}. The twisted pattern which squeezed the structure in some projections and long line segments which detect anomalies.  
...
The future enhancements involve introducing approaches to bind the data and indicate line segments beyond \gD{}. This would help in creating a more accurate representation of the data when \gD{} layout is inadequate.
