...
nldr_two_curvy_original <- ggplot(tsne_two_curvy_scaled, aes(x = tSNE1, y = tSNE2)) +
...
two_curvy_proj_true <- plot_proj(
  projected_df = projected_df, 
  model_df = true_model_df_proj, 
  axes = axes, 
  circle = circle, 
  point_param = c(0.5, 0.5, "#636363"), # size, alpha, color
  line_param = c(0.5, 0.5, "#2b8cbe"), # linewidth, alpha
  plot_limits = c(-0.6, 0.6), 
  title = "a", 
  cex = 1.5, 
  axis_text_size = 4, 
...
projected_true_model <- as.matrix(scaled_two_curvy_data_true_model) %*% projection
...
```{r}
#| label: best-fit-tsne
#| fig-cap: "We start with a tSNE layout (a) of non-linear clusters data ($n =  2000$ and $p = 4$), from which the \\gD{} wireframe is constructed (b) and then lifted to $4\\text{-}D$. A \\gD{} projection of the model fit from a tour through the $4\\text{-}D$ space is presented in c. Additionally, a \\gD{} projection of the fitted and true models (d) from the same tour demonstrates that the model accurately captures the non-linear structure of the data, although it does not fully represent the width of that structure. This highlights a key characteristic of tSNE, which compresses the data when applying tSNE. Furthermore, the sparse space observed in the middle of the tSNE layout is a result of (hyper-)parameter choice. The default is used. Video of the langevitour animation is available at <>."
#| fig-width: 10
#| fig-height: 10
...
error_plot_one_curvy_hist_selected <- ggplot(error_df_one_curvy_abs, 
      aes(x = sqrt_row_wise_total_error, 
          y = density,
          colour = factor(error_cat_n, levels = c("low", "high")))) +
...
error_plot_one_curvy_hist <- ggplot(error_df_one_curvy_abs,
        aes(x = sqrt_row_wise_total_error, 
            y = density,
            colour = factor(error_cat_n2, levels = c("low", "high")))) +
  geom_point(alpha=0.7) +
  scale_color_manual(values=c('#d9d9d9', '#800026')) +
  xlab(expression(e[hj])) +
  ylab("") +
...
projected_df <- projected_df |>
  mutate(cluster = factor(error_df_one_curvy_abs$error_cat_n2,
                          levels=c("low", "high")))
...
projected_df <- projected_df |>
  select(-cluster) |>
  mutate(cluster = error_df_one_curvy_abs$error_cat_n)
...
pbmc_proj_umap_model1 <- plot_proj(
  ...
  axes = axes, 
  circle = circle, 
  point_param = c(1.5, 0.2, "#a65628"), # size, alpha, color
...
algo_obj_pbmc <- gen_nldr_vis_algo_obj(
  high_d_data = training_data_pbmc, 
  nldr_data = umap_pbmc2, 
  num_x_bins = num_bins_x_pbmc)

umap_pbmc_scaled_best <- algo_obj_pbmc$nldr_scaled
distance_pbmc <- algo_obj_pbmc$distance_df
tr_from_to_df_pbmc <- algo_obj_pbmc$tr_from_to_df
benchmark_pbmc <- algo_obj_pbmc$benchmark
df_bin_centroids_pbmc <- algo_obj_pbmc$df_bin_centroids
...
pbmc_proj_umap_model1_best <- plot_proj(
    projected_df = projected_df, 
    model_df = model_df, 
    axes = axes, 
    circle = circle, 
    point_param = c(1.5, 0.2, "#999999"), # size, alpha, color
    plot_limits = c(-0.65, 0.65), 
    title = "b2", 
    cex = 2, 
    axis_text_size = 5,
...
pbmc_proj_umap_model2_best <- plot_proj(
    projected_df = projected_df, 
    model_df = model_df, 
    axes = axes, 
    circle = circle, 
...
trimesh_removed_mnist <- ggplot() + 
  ...
  interior_annotation("a", c(0.08, 0.93), cex = 1.5) +
  theme(legend.position = "none") 
...
projected_df <- proj_obj1$projected_df |>
  mutate(cluster = tsne_minst_scaled$cluster)
