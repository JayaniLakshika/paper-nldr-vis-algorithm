...
The paper is organized as follows. @sec-background provides a summary of the literature on NLDR and high-dimensional data visualization methods. <!--@sec-method--> Sections [-@sec-NLDRmodel], [-@sec-algorithm], [-@sec-highd], [-@sec-summary] contain the details of the new methodology, illustrated using a simulated data example. @sec-prediction describes a way to predict the layout positions of new observations. @sec-interactive illustrates interactive graphics to view the fitted model in high dimensions linked to the NLDR layout, to diagnose a fit. A workflow for choosing from a selection of layouts is provided in @sec-bestfit. Two applications illustrating the use of the new methodology for bioinformatics and image classification are in @sec-applications. Limitations and future directions are provided in @sec-discussion. 
...
Hexagon binning is a good solution for \gD{} [@dan2023].  Although there are several implementations available (e.g., @carr1987), none have components made available to produce what is needed for this project. So we have re-implemented the method and describe the process (@fig-hex-param). 

The \gD{} hexagon grid is uniquely defined the hexagon ids, $h (= 1, \dots, b)$, where $b$ is the number of bins, and their \gD{} centroids, $C_{h}^{(2)} = (c_{h1}, c_{h2})$. The number of bins in each direction is denoted as $(b_1, b_2)$, with  $b = b_1 \times b_2$ being the total number of bins. We expect the user to provide just $b_1$, and we calculate $b_2$ using the NLDR ratio to compute the grid. 

To ensure that the grid covers the range of data values, a buffer parameter ($q$) is set as a proportion of the range. By default,  $q=0.1$. The buffer should extend a full hexagon width ($a_1$) and height ($a_2$) beyond the data, in all directions. The lower left position where the grid starts is defined as $(s_1, s_2)$, and corresponds to the centroid of the lowest left hexagon, $C_{1}^{(2)} = (c_{11}, c_{12})$. This must be smaller than the minimum data value. Because it is one buffer unit, $q$ below the minimum data values, $s_1 = -q$ and $s_2 = -qr_2$. 
...
By default, the bin centroid is used to represent a hexagon center (@fig-NLDR-two-curvy (c)), but any measure of center, such as a mean or weighted mean of the points within each hexagon, could be used. The hexagonal bin, $h$, for each observation is needed to construct the model representation in high dimensions.  
...
NLDR methods are primarily designed for visualization and exploration rather than reconstruction, and most do not explicitly support out-of-sample prediction. Among the methods considered here, only UMAP provides a `predict()` function for embedding new observations based on a learned representation [@tomasz2023]. Other approaches, such as PCA, neural network autoencoders [@hinton2006], and parametric tSNE [@van2009], also provide mechanisms for mapping new data into a low-dimensional space.

Within the framework developed here, a simple approach can be used to approximate the embedding of a new observation, in a manner consistent with the fitted model. The procedure is to (i) identify the closest bin centroid in \pD{}, $C^{(p)}_{h}$, and (ii) assign the corresponding centroid in \gD{}, $C^{(2)}_{h}$, as the predicted embedding. This treats the fitted wireframe as a piecewise representation of the underlying structure.
