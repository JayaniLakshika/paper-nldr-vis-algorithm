...
projection <- cbind(
    c(0.24288,-0.03628,-0.03302,0.12182,-0.04150,0.02344,0.05910),
    c(0.05019,0.07165,-0.20131,-0.16200,-0.05343,0.06854,-0.00551))

projected <- as.matrix(training_data_scurve |> select(-ID)) %*% projection
...
projected_true <- as.matrix(true_model_df |> select(-ID)) %*% projection
...
projection <- cbind(
    c(0.03685,-0.02239,0.08047,0.22415,-0.10295,0.04529,0.10381),
    c(-0.16908,0.11691,0.04886,0.10189,-0.00203,-0.09763,-0.13207))
...
```{r}
#| label: s-curve-true-model-proj3
#| eval: false

projection <- cbind(
    c(0.11484,0.06997,-0.13638,0.01096,0.01533,0.16257,-0.13615),
    c(0.07805,0.01652,0.17174,-0.07343,0.16177,0.11112,0.04729))
...
scurve_proj3_true_model <- projected_df |>
  ggplot(
    aes(
      x = proj1, 
      y = proj2)) +
...
```{r}
#| label: bin-counts-scurve-with_buffer
## To initialize number of bins along the x-axis
bin1_vec_scurve <- c(7, 11, 17) #sqrt(NROW(training_data_scurve)/2)
#buffer_vec <- seq(0.05, 0.16, by = 0.01)
buffer_vec <- c(0.05, 0.07, 0.12)

centroids_df_scurve <- data.frame(matrix(nrow = 0, ncol = 0))
...
for (xbins in bin1_vec_scurve) {
  for (buff in buffer_vec) {
    ...
    centroids_df_scurve <- bind_rows(centroids_df_scurve, df_bin_centroids_scurve)
      
  }

}

b7_q_plt <- centroids_df_scurve |>
  filter(bin1 == 7) |>
  filter(buff == 0.12) |>
  ggplot(aes(x = bin_counts)) +
  geom_density() +
  xlab("bin count") +
  theme(
...
```{r}
#| label: fig-bin-counts-scurve-with_buffer
#| fig-cap: "Distribution of bin counts for differnt $b_1$ with specific $q$"
#| out-height: 30%
...
b7_q_plt2 <- centroids_df_scurve |>
  filter(bin1 == 7) |>
  ggplot(aes(x = std_counts,
             colour = as.factor(buff),
             group = as.factor(buff))) +
  geom_density() +
  scale_color_brewer(type = "qual") +
  xlab("standardised bin count") +
  theme(
    axis.text.x = element_text(size = 7),
    axis.ticks.x = element_line(linewidth = 0.5),
    axis.title.x = element_text(size = 7)
...
b11_q_plt2 <- centroids_df_scurve |>
  filter(bin1 == 11) |>
  ggplot(aes(x = std_counts,
             colour = as.factor(buff),
             group = as.factor(buff))) +
...
b17_q_plt2 <- centroids_df_scurve |>
  filter(bin1 == 17) |>
  ggplot(aes(x = std_counts,
             colour = as.factor(buff),
             group = as.factor(buff))) +
...
b7_q_plt2 + b11_q_plt2 + b17_q_plt2
```
...
centroids_df_scurve |>
  filter(bin1 == 7) |>
  ggplot(aes(x = as.factor(hexID), 
           y = std_counts,
           group = as.factor(buff),
           color = as.factor(buff))) + 
  geom_line() +
  scale_color_brewer(type = "qual")
```
...
error_df_scurve_q <- data.frame(matrix(nrow = 0, ncol = 0))
...
for (xbins in bin1_vec_scurve) {
  for (buff in buffer_vec) {
    ...
    error_df_scurve_q <- bind_rows(error_df_scurve_q, error_df)
      
  }

}

ggplot(error_df_scurve_q, 
       aes(x = bin1, 
           y = MSE,
           colour = as.factor(q),
           group = as.factor(q))) + 
...
```{r}
#| label: diff-bins-scurve-buff
## To initialize benchmark values to remove low density hexagons
benchmark_rm_hex_vec <- seq(0, 0.7, by=0.1)
bin1_vec_scurve <- c(7, 11, 17)
error_rm_scurve <- data.frame(matrix(nrow = 0, ncol = 0))
...
for (xbins in bin1_vec_scurve) {
  ...
  for (benchmark_rm_lwd in benchmark_rm_hex_vec) {
    ...
    a1 <- calc_bins_y(bin1 = xbins, r2 = r2, q = 0.07)$a1
    ...
    tr1_object_scurve <- tri_bin_centroids(
      df_bin_centroids_scurve_high_dens, 
      x = "c_x", 
      y = "c_y")
...
mse_scurve_lwd <- ggplot(error_rm_scurve, 
                     aes(x = benchmark_rm_lwd, 
                         y = MSE,
                         colour = as.factor(bin1),
                         group = as.factor(bin1))) + 
  geom_point(
    size = 1
    ) +
  geom_line(
  ...
  # geom_vline(xintercept = benchmark1, linetype="solid", 
  #            color = "#bdbdbd", linewidth=1, alpha = 0.5) +
  scale_x_continuous("bechmark to remove low-density hexagons", 
         transform = "reverse") + 
  ylab("MSE") +
  ggtitle("(c)") +
  theme(aspect.ratio = 0.75,
...
```{r}
#| label: diff-bins-scurve-buff3
#| eval: false
## To initialize benchmark values to remove low density hexagons
benchmark_rm_hex_vec <- seq(0, 0.7, by=0.1)
bin1_vec_scurve <- c(7, 11, 17)
distance_all_scurve <- data.frame(matrix(nrow = 0, ncol = 0))
#buffer_vec <- 0.07
...
for (xbins in bin1_vec_scurve) {
  ...
  #for (benchmark_rm_lwd in benchmark_rm_hex_vec) {
    ...
    a1 <- calc_bins_y(bin1 = xbins, r2 = r2, q = buff)$a1
    ...
    bins <- seq(start_point, max(distance_scurve$distance) + a1, 
                by = a1)
    ...
    distance_all_scurve <- bind_rows(distance_all_scurve, distance_scurve)
...
# dist_summary_scurve <- distance_all_scurve |>
#     count(bin1, q, bin)

distance_hist <- distance_all_scurve |>
  filter(bin1 == 11) |>
  filter(q == 0.07) |>
  ggplot(aes(x = distance)) +
    geom_histogram(
      ...
      binwidth = a1,
      position = "dodge") +
  scale_x_continuous(breaks = bin_labels, labels = round(bin_labels, 3)) +
  scale_color_brewer(type = "qual") +
  #facet_wrap(vars(as.factor(bin1)), scale = "free") +
...
cell_count_scurve + mse_scurve_b +  
mse_scurve_lwd + 
distance_hist + plot_layout(ncol=2)
```
...
names(model_df)[(2 + NCOL(proj_model)):NCOL(model_df)] <- paste0(names(proj_model)[-NCOL(proj_model)], "_to")

five_gau_proj_tsne_model <- projected_df |>
  filter(type == "data") |>
  ggplot(
    aes(
      x = proj1, 
...
five_gau_proj_umap_model <- projected_df |>
  filter(type == "data") |>
  ggplot(
    aes(
      x = proj1, 
...
five_gau_proj_pacmap_model <- projected_df |>
  filter(type == "data") |>
  ggplot(
    aes(
      x = proj1, 
...
# five_gau_proj_pacmap_model <- gen_proj_langevitour(
#   points_df = df_exe,
#   projection = projection,
#   edge_df = distance_df_small_edges |> select(-distance)
# ) +
...
five_gau_proj_tsne_model + five_gau_proj_umap_model + five_gau_proj_pacmap_model +
  plot_layout(guides = "collect", ncol = 3) &
...
To determine whether the UMAP representation with the (hyper-)parameter choice suggested by @chen2023 preserves the original data structure, we visualize the model constructed with UMAP overlaid on the $9\text{-}D$ data. The @fig-umap-author shows three well-separated clusters. However, as shown in @fig-pbmc1-sc, there is no big separation between three clusters in $9\text{-}D$. Therefore, the suggested UMAP representation (@fig-umap-author) does not accurately represent the structure(s) present in PBMC3k dataset. But, when visualizing the model-in-th-data-space some unobserved structures can be seen. Some clusters have non-linear continuity patterns and high-density patches (@fig-pbmc1-sc).  
...
```{r}
#| fig-cap: "MSE from UMAP with diffferent (hyper-)parameter choices and tSNE with default (hyper-)parameter setting applied to PBMC3k dataset for different number of bins along the x-axis ($b_1$). Which is the most reasonable representation of the structures present in the PBMC3k data? The residual plot have a steep slope at the beginning, indicating that a smaller $b_1$ causes a larger amount of error. Then, the slope gradually declines or level off, indicating that a higher number of $b_1$ generates a smaller error. It was observed that for every $b_1$, the lowest error occurred with tSNE using the perplexity: $30$."
#| label: fig-pbmc-mse
#| fig-pos: H
#| out-width: 70%
...
As shown in @fig-tsne-suggest, there are three well-separated clusters, although the separation between the clusters are small. Additionally, non-linear structures can also be observed within the clusters (@fig-tsne-suggest). This demonstrates that tSNE accurately captures the data structure of the PBMC3k dataset, which UMAP did not achieve.

<!--best choice-->

```{r}
#| label: tsne-pbmc-read
tsne_pbmc <- read_rds("data/pbmc3k/pbmc_tsne_30.rds")
pbmc_scaled_obj <- gen_scaled_data(
  data = tsne_pbmc)
...
```{r}
#| echo: false
#| fig-cap: "The $2\\text{-}D$ layout from tSNE applied for the PBMC3k dataset. The default (hyper-)parameter setting is a perplexity $30$. We use our model-in-the-data-space to assess whether this is an accurate representation of structure present in the high-dimensional data, or if it is misleading."
...
proj1_pbmc_tsne <- gen_proj_langevitour(
  points_df = df_exe,
  projection = projection,
  edge_df = distance_df_small_edges |> select(-distance)
) 
...
proj2_pbmc_tsne <- gen_proj_langevitour(
  points_df = df_exe,
  projection = projection,
  edge_df = distance_df_small_edges |> select(-distance)
) 
...
proj3_pbmc_tsne <- gen_proj_langevitour(
  points_df = df_exe,
  projection = projection,
  edge_df = distance_df_small_edges |> select(-distance)
) 
...
```{r}
#| echo: false
#| fig-cap: "Model in $2\\text{-}D$, on the tSNE layout, and three views of the fit in projections from $9\text{-}D$, for the PBMC3k data ($(s_1, \ s_2) = (-0.050, \ -0.058)$, $b = 300 \  (15, \ 20)$, $m = 136$, benchmark value to remove large edges is $0.133$). (The **langevitour** software is used to view the data with a tour, and the full video is available at <https://youtu.be/5Y1hE4i7N2k>)."
#| label: fig-pbmc2-sc
#| fig-pos: H
#| out-height: 100%
#| eval: true

trimesh_removed_pbmc + proj1_pbmc_tsne + 
  proj2_pbmc_tsne + proj3_pbmc_tsne +
  plot_layout(guides='collect', ncol=2) &
  theme(legend.position='none')
```
