...
Each of the \kD{} points will belong to a hexagonal bin. That is, for each $y \in \mathbfit{Y}$, we can (uniquely) identify the hexagon that the point belongs to. Let $h : \mathbb{R}^{n\times k} \rightarrow \mathbb{R}^{m\times k}$ be the function that maps a point in \kD{} space to it's hexagonal centroid ($C^{(k)}$). 
...
ar <- umap_scurve |>
  summarise_at(c("UMAP1", "UMAP2"), 
    function(x) max(x)-min(x)) |>
  mutate(ar = UMAP2/UMAP1) |>
  pull(ar) # This is a bit cumbersome too
hr <- 2/sqrt(3)
umap_scurve_scaled <- umap_scurve |>
  mutate_at(c("UMAP1", "UMAP2"), 
    function(x) (x - min(x))/(max(x)-min(x))) |> # can be no missings
  mutate(UMAP2 = UMAP2 * ceiling(ar/hr) * hr)
  
  #as.data.frame(do.call(cbind, gen_scaled_data(data = umap_scurve, 
  #                                  x = "UMAP1", y = "UMAP2"))) |>
...
#| echo: false
#| fig-cap: "Key steps for constructing the model on the NLDR layout ($k=2$) of the S-curve data: (a) obtain UMAP data, (b) hexagonating UMAP data, (c) obtain hexagonal bin centroids, and (d) triangulate bin centroids."
#| fig-width: 8
#| fig-height: 4
#| out-width: 100%
