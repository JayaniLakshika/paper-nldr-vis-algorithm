...
A buffer is used to ensure that the grid covers the range of data values, and provides flexibility for where the grid positining. It is well-known that where a grid starts can change binning results, even though most documentation focuses on binwidth! The buffer ($q$) is set as a proportion of the range. By default,  $q=0.1$, which is 10% of the range. <!--The buffer should extend a full hexagon width ($a_1$) and height ($a_2$) beyond the data, in all directions.--> The lower left position where the grid starts is set to be $(s_1 = -q, s_2 = -qr_2)$, specifying the centroid of the lower left hexagon, $C_{1}^{(2)} = (c_{11}, c_{12})$. 
...
$$u(i) = \arg\min_{h = 1, \dots, m} \sqrt{(y_{i1} - C^{(2)}_{h1})^2 + (y_{i2} - C^{(2)}_{h2})^2},$$ maps observation $i$ into $H_h = \{i| u(i) = h\}$. 
<!--By default, the bin centroid is used to represent a hexagon center (@fig-NLDR-two-curvy (c)), but any measure of center, such as a mean or weighted mean of the points within each hexagon, could be used.--> Then $\mathbfit{\widehat{Y}}_i = \mathbfit{C}^{(2)}_{u(i)} (i=1, \dots, n) \in \{\mathbfit{C}^{(2)} = (C^{(2)}_{h1}, C^{(2)}_{h2}), h=1, \dots, m\}$ are considered the fitted values, from which we can also calculate \gD{} residuals. The bin ID, $h$, for each observation is needed to construct the model representation in high dimensions. 
...
The next step is to lift the \gD{} model into \pD{} by computing \pD{} vectors that represent bin centroids. We use the \pD{} mean of the points in a given hexagon, $H_h$, denoted $C_{h}^{(p)}$, to map the centroid $C_{h}^{(2)} = (c_{h1}, c_{h2})$ to a point in \pD{}. Let the $j^{th}$ component of the \pD{} mean be

$$C_{hj}^{(p)} = \frac{1}{n_h}\sum_{i \in H_h} x_{ij}, ~~~h = 1, \dots, b;~ j=1, \dots, p; ~ n_h > 0.$$
We can consider this procedure to be a pseudo-inverse of $g$, $\mathbfit{C}^{p} = \tilde{g}(\mathbfit{C}^{(2)})$, $\tilde{g}\text{:}~ \mathbfit{C}^{(2)} \subset \mathbb{R}^2 \mapsto \mathbfit{C}^{p} \subset \mathbb{R}^p$. Then $\mathbfit{\hat{X}}_i (i=1, \dots, n) \in \{\mathbfit{C}^{(p)} = (C^{(p)}_{h1}, \dots C^{(p)}_{hp}), h=1, \dots, m\}$ are considered the fitted values, from which we can also calculate \pD{} residuals.
