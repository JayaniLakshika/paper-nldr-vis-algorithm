...
The \gD{} hexagon grid is uniquely defined by the hexagon ids, $h (= 1, \dots, b)$, and their \gD{} centroids, $\mathbfit{c}_{h*}^{(2)} = [c_{h1}, c_{h2}]$. The algorithm inputs $b_1$, which determines $b_2$ using the NLDR ratio, producing $b = b_1 \times b_2$, the total number of bins in the grid. 

A buffer is used to ensure that the grid covers the range of data values, and provides flexibility for where the grid positining. It is well-known that where a grid starts can change binning results, even though most documentation focuses on binwidth! The buffer ($q$) is set as a proportion of the range. By default,  $q=0.1$, which is 10% of the range. <!--The buffer should extend a full hexagon width ($a_1$) and height ($a_2$) beyond the data, in all directions.--> The lower left position where the grid starts is set to be $(s_1 = -q, s_2 = -qr_2)$, specifying the centroid of the lower left hexagon, $\mathbfit{c}_{1*}^{(2)} = [c_{11}, c_{12}]$. 
...
and then collected into sets $H_h = \{i| u_i = h\}$ $(h=1, \dots, b)$. 
<!--By default, the bin centroid is used to represent a hexagon center (@fig-NLDR-two-curvy (c)), but any measure of center, such as a mean or weighted mean of the points within each hexagon, could be used.--> Then $\mathbfit{\widehat{y}}_{i*} = \mathbfit{c}^{(2)}_{u_i*}$ $(i=1, \dots, n)$ are considered the fitted values, from which we can also calculate \gD{} residuals. The bin ID, $u_i$, for each observation is also needed to construct the model representation in high dimensions. 
...
Then $\mathbfit{\hat{x}}_{i*} = \mathbfit{c}^{(p)}_{u_i*}$ $(i=1, \dots, n)$ are considered the fitted values, from which we can also calculate \pD{} residuals.
...
We treat the embedding as defining a fitted representation in \pD{}. Each observation $\mathbfit{x}_{i*}$ has an associated bin $u_i$ with a centroid that serves as its fitted value, $\mathbfit{\hat{x}}_{i*} = \mathbfit{c}^{(p)}_{u_i*}$. The discrepancy between observed and fitted values can then be interpreted as a residual.
