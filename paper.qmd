...
Projections of the $7\text{-}D$ S-curve data (purple points) overlaid with the model (black grid with green points indicating bin means) constructed from the UMAP layout, with 60 non-empty bins (using the **langevitour** software). The model closely fits the shape of this data. A video of the full tour is available at (<https://youtu.be/G1m4q9k--v4>).
:::
...
#| fig-pos: H
#| fig-cap: "Hexbin plots from different number of bins for the UMAP applied to S-curve data: (a) $b = 91$ (7, 13), (b) $b = 190$ (10, 19), and (c) $b = 364$ (14, 26). The hexbins are colored based on the density of points, with darker colors indicating higher density and yellow color representing lower density within each bin. What is the number of bins that would be effective?"
#| fig-width: 6
#| fig-height: 3
