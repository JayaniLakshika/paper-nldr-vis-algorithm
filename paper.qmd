...
The values $(s_1, \ s_2)$ define the position of the centroid of the bottom left hexagon. By default, this is at $s_1 = -q, s_2 = -qr_2$, where $q$ is the buffer sound the data. The choice of these values can have some effect on the distribution of bin counts. FIG XXX(a) illustrates this. The distribution of bin counts for $s_1$ varying between $-0.1-0.0$ is shown. Generally, a more uniform distribution among these possibilities would indicate that the bins are reliably capturing the underlying distribution of observations. 
...
hex_grid_coloured_scurve1_dens <- ggplot(hex_grid_with_counts_s_curve1) +
  geom_histogram(aes(x=std_counts), binwidth = hb_obj_scurve1$centroids$c_x[2]-hb_obj_scurve1$centroids$c_x[1])
```
...
hex_grid_coloured_scurve2_dens <- ggplot(hex_grid_with_counts_s_curve2) +
  geom_histogram(aes(x=std_counts), binwidth = hb_obj_scurve2$centroids$c_x[2]-hb_obj_scurve2$centroids$c_x[1])
```
...
hex_grid_coloured_scurve3_dens <- ggplot(hex_grid_with_counts_s_curve3) +
  geom_histogram(aes(x=std_counts), binwidth = hb_obj_scurve3$centroids$c_x[2]-hb_obj_scurve3$centroids$c_x[1]) 
```
...
The default number of bins $b=b_1\times b_2$ is computed based on the sample size, by setting $b_1=n^{1/3}$, consistent with the Diaconis-Freedman rule [@freedman1981]. The value of $b_2$ is determined analytically by $b_1, q, r_2$. Values of $b_1$ between $2$ and $b_1 = \frac{n}{2}$ are allowed. FIG XXX(b) shows the effect of different choices of $b_1$ on the MSE of the fitted model. XXX Are you showing $b_1$ or $b$ here???? $b$
