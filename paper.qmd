...
The 2D hexagon grid is defined by the number of bins in each direction ($b_1, b_2$), and the lower left position where the grid starts ($s_1, s_2$). The values of $s_i$ need to be below their respective minimum variable values, and could be a full bin lower, to allow a buffer corresponding to a full hexagon width ($a_1$) and height ($a_2$) around the data. The values of $b_i$ are variables to be computed that define the reduction in size of the data ($n$ to $m$).

The value for $b_2$ is computed by fixing $b_1$. Considering the lower bound of the NLDR, $a_1 > 2 \times s_1$ and $a_1 > \frac{1-s_1}{b_1 -1}$. The minimum $a_1$ that satisfies the constraints will be selected. For regular hexagons, $a_2 = \frac{2}{\sqrt(3)}a_1$. Therefore, $a_2$ is computed for the minimum $a_1$. According to the upper bound of the NLDR, $b_2 = \left\lceil\frac{r_2 - s_2}{vs} + 1\right\rceil$, where $vs = \frac{a_2}{2} + \sqrt{{(\frac{a_2}{2})}^2 + {(\frac{a_1}{2})}^2}$.       
...
Points are allocated to the bin that they fall in by considering the nearest nearest centroid. 
