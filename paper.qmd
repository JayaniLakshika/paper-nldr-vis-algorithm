...
```{r}
#| label: test-two-curvy

## First projection
proj_obj1 <- read_rds("data/two_nonlinear/test_two_nonlinear_proj_obj1.rds")
...
two_curvy_proj_test <- projected_df |>
  ggplot(
    aes(
      x = proj1,
      y = proj2)) +
...
predict_highdvis <- read_rds("data/two_nonlinear/test_two_non_linear_diff_shaped_close_clusters_highd_vis_predict.rds")

predict_two_curvy_highdvis <- ggplot(predict_highdvis, 
                                  aes(x = pred_emb_1, y = pred_emb_2)) +
  geom_point(alpha = 0.5, colour = clr_choice, size = 0.5) +
  interior_annotation(label = "b", cex = 1.5) 

predict_umap <- read_rds("data/two_nonlinear/test_two_non_linear_diff_shaped_close_clusters_umap_predict.rds")

predict_two_curvy_umap <- ggplot(predict_umap, 
                                  aes(x = UMAP1, y = UMAP2)) +
  geom_point(alpha = 0.5, colour = clr_choice, size = 0.5) +
  interior_annotation(label = "c", cex = 1.5) 
...
two_curvy_proj_test + 
  predict_two_curvy_highdvis + 
  predict_two_curvy_umap +
  plot_layout(ncol = 3)
```
