...
#  scale_x_continuous(breaks = sort(unique(round(error_two_curvy_tsne$a1, 2)))[c(1, 4, 7, 11, 13, 14, 15, 16)]) +
  ylim(c(0, 0.75)) +
  labs(x = "average bin density", y = "HBE") +
  #ylim(c(0, 0.75)) +
...
error_two_curvy_dens <- bind_rows(error_two_curvy_umap, 
...
error_two_curvy_dens <- error_two_curvy_dens |>
  mutate(d_bar = round(d_bar, 3)) |>
  #mutate(a1 = round(a1, 2)) |>
  filter(b1 >= 5) |>
  group_by(method, d_bar) |>
...
error_two_curvy_tsne <- error_two_curvy_tsne |> filter(b1 <= 44) #sqrt(2000/1.02)

error_two_curvy_umap <- error_two_curvy_umap |> filter(b1 <= 60)

error_two_curvy_phate <- error_two_curvy_phate |> filter(b1 <= 46)

error_two_curvy_trimap <- error_two_curvy_trimap |> filter(b1 <= 72)

error_two_curvy_pacmap <- error_two_curvy_pacmap |> filter(b1 <= 48)
...
error_plot_two_curvy_dh <-
  plot_rmse_dh(error_two_curvy_dens) +
  scale_color_manual(
    values=c("PaCMAP" = '#e41a1c', "PHATE" = '#ff7f00', 
             "TriMAP" = '#4daf4a', "tSNE" = '#a65628', 
             "UMAP" = '#636363', "tSNE2" = '#984ea3')) +
...
nldr_metrics <- inner_join(nldr_metrics, error_two_curvy_filtered, by = c("Method" = "method")) |>
  rename("sc" = "r") |>
  #mutate(sc = 1 - sc) |>
  mutate(`ARNX` = 1 - `AUC(R_NX)`) |>
  rename(SC = sc, HBE = RMSE)

comp_metrics <- ggparcoord(
  data = nldr_metrics,
  columns = c(8,3,5,2,7),           # numeric columns to plot
...
```{r}
#| fig-cap: "Assessing which of the 6 NLDR layouts (a-f) on the 2NC7 data is the better representation using HBE for varying binwidth ($a_1$) and average bin density ($\\bar{d}_h$). Color represents NLDR layout. Layout d is universally poor. Layouts a, b, e that show two close clusters are universally suboptimal. Layout b with little separation performs well at tiny binwidth (where most points are in their own bin) and poorly as binwidth increases. Layout e has small separation with oddly shaped clusters. Layout a is the best choice. Comparison of scaled evaluation metrics (ARNX, RTA, SC, GS, and HBE using $a_1=0.05$) for the six NLDR layouts computed on the 2NC7 data using a parallel coordinate plot. Color of the line indicates NLDR layout. ARNX is reversed so that lower is best. RTA, SC and GS should be reversed, too, but they best match ARNX and HBE without doing so! There is some general agreement, and small disagreements, with ARNX being most different. All agree that layout f is good, and almost all that d is bad. Interestingly, there is disagreement on layout a, which is almost the same as layout f."
#| label: fig-toy-rmse
#| fig-pos: H
#| out-width: 100%
#| fig-height: 6
...
left_column <- error_plot_two_curvy / error_plot_two_curvy_dh +
...
error_plot_pbmc_dh <- plot_rmse_dh(error_pbmc) +
  scale_color_manual(values=c('#999999','#a65628','#e41a1c','#377eb8','#4daf4a','#ff7f00','#984ea3','#f781bf')) +
  ylab("HBE")
...
error_plots_pbmc <- error_plot_pbmc / error_plot_pbmc_dh
...
error_plot_mnist_dh <- plot_rmse_dh(error_mnist) +
  scale_color_manual(
    values=c('#a65628', '#984ea3', '#999999', 
             '#377eb8', '#ff7f00', '#e41a1c')) +
  theme(aspect.ratio=0.685) +
...
left_column <- error_plot_mnist / error_plot_mnist_dh
