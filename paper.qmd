...
- Remove all the extraneous commented text (Done)
- Check how \kD{} and \gD{} are used, and make sure it's sensible (changed in Rendering the model: Done)
- Check spelling is consistently American (Done)
- Check references (Done)
- Add videos - make a list of the videos in the supplementary section, with the number of the figure that it corresponds too. This avoids having captions that are too long and awkward with raw URLs. And it keeps all of this supplementary material in one place.
- Move the notation table to appendix (Done)
- Code clean up: recommend that all of the calculations done with quollr are done with a separate script(s) and put in a scripts directory. Save the resulting data need to make plots and tables. Only code for the plots and tables is directly in the qmd in the final version.
...
<!-- Where are sections 4 & 5 in this description? -->

## Background {#sec-background}

<!-- - Connection between NLDR and MDS-->
Historically, low-dimensional (\kD{}) representations of high-dimensional (\pD{}) data have been computed using multidimensional scaling (MDS) [@kruskal1964], which includes principal components analysis (PCA) [@jolliffe2011] as a special case. (A contemporary comprehensive guide to MDS can be found in @borg2005.)
<!-- Does JASA allow footnotes? If so this is a footnote, otherwise the punctuation has gone a bit rogue - can't have an entire sentence in brackets surely --> The \kD{} representation can be considered to be a layout of points in \kD{}   produced by an embedding procedure that maps the data from \pD{}. In MDS, the \kD{} layout is constructed by minimizing a stress function that differences distances between points in \pD{} with potential distances between points in \kD{}. Various formulations of the stress function result in non-metric scaling [@saeed2018] and isomap [@silva2002]. Challenges in working with high-dimensional data, including visualization, are outlined in @johnstone2009. 

<!-- Non-expert comment: This to me is a leap from MDS to NLDR. Is NLDR an MDS? I don't think so, but I'm missing the link -->
<!-- Suggested rewording: "To better capture specific structures potentially existing in \pD{}, many new methods for NLDR have emerged in recent years." -->
...
<!-- I'm not thrilled about H_h but I guess it can stay because changing it everywhere is a pain -->
...
Line segments that exist in the \gD{} model generate line segments in \pD{} by connecting the \pD{} means of the corresponding \gD{} bin centroids. If additional long edges need to be removed, compute the edges in \pD{} and prune any detected long edges to improve the accuracy. Once pruned, re-plot the \gD{} view to ensure it accurately captures the data. <!-- Do we still do this after the new binning system? -->
...
<!-- In Figure 4, is the "sparse space" in the middle of each cluster (i.e. the holes) or is it the gap between clusters? If it's between clusters, I'd argue tSNE is making this gap smaller than in reality, which I don't think the caption currently conveys. -->

```{r}
#| label: best-fit-tsne
#| fig-cap: "Lifting the fitted model into \\pD{}. Starting from a default tSNE layout (a) of nonlinear clusters data ($n =  2000$ and $p = 4$), from which the \\gD{} wireframe is constructed (a) and then lifted to $4\\text{-}D$ (b, c). The fit is reasonbly tight with the data, although it does not fully spread the full width. This highlights a key characteristic of tSNE, which compresses the data when applying tSNE. Furthermore, the sparse space observed in the middle of the tSNE layout is a result of (hyper-)~parameter choice. Video of the langevitour animation is available at <>."
#| fig-width: 8
#| fig-height: 2
...
<!-- Sparse end with dense corner to me is very hard to understand - sparse and dense are pretty much antonyms -->
```{r}
#| label: fig-p-d-error-in-2d-two-curvy
#| fig-cap: "The $4\\text{-}D$ model error in \\gD{} layout. Color indicates error ($e_{hj}$), dark color indicating high error and light indicates low error. Most large errors are distributed near the sparse end of the nonlinear cluster with dense corner."
#| out-width: 80%
#| fig-pos: H
...
In this paradigm of model fitting, it is now possible to predict a new observation's value in the NLDR representation, independent of the choice of NLDR method. To do this, we first need to determine the closest bin centroid in \pD{}, $C^{(p)}_{h}$, to the new observation, which is,

$$z = \arg\min_{j = 1, \dots, b} \sqrt{\sum_{v=1}^{p}(x_{iv} - C^{(p)}_{jv})^2}.$$

Then, the predicted value is is the centroid of this bin in \gD{}, $C^{(2)}_{z} = (c_{z1}, c_{z2})$.
...
The values $(s_1, \ s_2)$ define the position of the centroid of the bottom left hexagon. By default, this is at $s_1 = -q, s_2 = -qr_2$, where $q$ is the buffer bound the data. The choice of these values can have some effect on the distribution of bin counts which is seen in @fig-bins-two-curvy. <!-- Figure 6 doesn't seem to have any varying s_1 unless I'm missing something --> The distribution of bin counts for $s_1$ varying between $\text{-}0.1\,\text{--}\,0.0$ is shown. Generally, a more uniform distribution among these possibilities would indicate that the bins are reliably capturing the underlying distribution of observations. 
...
The default number of bins $b=b_1\times b_2$ is computed based on the sample size, by setting $b_1=n^{1/3}$, consistent with the Diaconis-Freedman rule [@freedman1981]. The value of $b_2$ is determined analytically by $b_1, q, r_2$ <!-- What is the formula for this? -->. Values of $b_1$ between $2$ and $b_1 = \sqrt{n/r_2}$ are allowed. @fig-param-two-curvy (a) shows the effect of different choices of $b_1$ on the MSE of the fitted model.
...
## Curiosities about NLDR results discovered by examining the model in the data space
...
In this data there are $2622$ single cells and $1000$ gene expressions (variables). Following their pre-processing, UMAP was performed on the first nine principal components. @fig-NLDR-variety shows this data using a variety of methods, and different (hyper-)~parameters. <!-- I think some of these panels aren't umap - if so saying "umap was performed on the first nine principal components" is not necessarily correct. I think its what the publication did but not what is in our figure --> You can see that the result is wildly different depending on the choices. Layout a is a reproduction of the layout published in @chen2024. This layout suggests that the data has three very well separated clusters, each with an odd shape. The question is whether this accurately represents the cluster structure in the data, or whether they should have chosen b or c or d or e or f or g. This is what our new method can help with -- to decide which is the more accurate \gD{} representation of the cluster structure in the \pD{} data. 
...
```{r}
#| label: link-plts-mnist
#| eval: false

df_exe_mnist <- comb_all_data_model(
  highd_data = training_data_mnist,
  nldr_data = tsne_minst_scaled_temp,
  model_highd = df_bin_mnist,
  model_2d = df_bin_centroids_dens_clust)
...
right_top_img <- plot_digit_img(
  pixels_gathered_within, palette = "Blues 2", title_text = "b")
...
middle_img <- plot_digit_img(
  pixels_gathered_within, palette = "Greens 3", title_text = "c")
...
right_bottom_img <- plot_digit_img(
  pixels_gathered_within, palette = "Purples 2", title_text = "d")
...
imge_error_sample_outside <- plot_digit_img(
  pixels_gathered_outside, palette = "Grays", title_text = "e")
...
imge_error_sample_inside <- plot_digit_img(
  pixels_gathered_inside, palette = "Oranges", title_text = "f")
...
kable(links_df, 
  ...
  column_spec(1:3, width = c("1.5cm", "4.5cm", "9cm"))
```
