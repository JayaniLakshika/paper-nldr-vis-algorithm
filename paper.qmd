...
The error is computed by taking the squared \pD{} Euclidean distance, of points from their bin centroid, which we will call hexbin error (HBE):
...
Default values are provided for each of these, but deciding on the best model fit is assisted by examining the HBE for a range of choices. <!--Choosing these parameters according to HBE can be automated but it is recommended that the user examine the resulting model representation by overlaying it on the data in \pD{}. The next few subsections describe the calculation of default values, and the effect that different choices have on the model fit.-->
...
The default number of bins $b=b_1\times b_2$ is computed based on the sample size, by setting $b_1=n^{1/3}$, consistent with the Diaconis-Freedman rule [@freedman1981]. The value of $b_2$ is determined analytically by $b_1, q, r_2$ (@eq-bin2). Values of $b_1$ between $2$ and $b_1 = \sqrt{n/r_2}$ are allowed. @fig-param-two-curvy (a) shows the effect of different choices of $b_1$ on the HBE of the fitted model.
...
- HBE should decrease when binwidth decreases, as the binning gets closer to observations being in their own bin. But a big drop in HBE would indicate the lower binwidth is substantially better than the larger one (@fig-param-two-curvy a).
...
```{r}
#| echo: false
#| fig-cap: "Various plots to help tune the model fit, using the binwidth $a_1$ and low density bin removal. In (a) HBE against $a_1$, HBE is going to increase as $a_1$ increases, but just before a big increase is a good choice. Here HBE steadily increases so there is no clear choice - nevertheless, we have chosen three binwidths, $0.03$ (orange dashed), $0.05$ (blue solid), and $0.07$ (green dotted), to compare in the other plots. Proportion of non-empty bins tends to increase with $a_1$ (b). Removing low count bins changes HBE substantially (c). The relative average bin density tends to increase with $a_1$ (d)."
...
@fig-comp-metric compares how these evaluation metrics behave across methods. Each line represents a different NLDR layout, and each axis shows a normalized score for one evaluation criterion. The variation across metrics highlights that they capture different aspects of structure preservation, and none behave exactly like HBE. While HBE provides a unique perspective on how well the high-dimensional structure is represented, only the RTA and sc metrics show a somewhat similar ordering across methods.

```{r}
#| label: fig-comp-metric
#| fig-cap: "Comparison of scaled evaluation metrics across  6 NLDR layouts on the 2NC7 data. Each line represents a different NLDR layout evaluated using multiple normalized metrics: RTA, AUC(R_NX), sc, RGS, and HBE. The variation across metrics shows that each captures different aspects of structure preservation, and none behave exactly like HBE. RTA, and sc show a somewhat similar ordering across methods."
#| fig-pos: H
#| out-width: 100%
#| fig-height: 10
#| fig-width: 10
