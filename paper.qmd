...
umap_data_with_hb_id_pbmc <- as.data.frame(do.call(cbind, hb_obj_s_curve$data_hb_id))
...
<!--obtain 2D and high-D distances-->
...
## Obtian 2D distances
proxy::dist(umap_pbmc[, -3]) |> 
as.data.frame()

## Obtain high-D distances
proxy::dist(training_data_pbmc[, -16]) |> 
as.data.frame()
```
--->
...
- Learned from the model: There is a non-linear shape in 2D, but if look at the model in high-D, the non-linear continuity is there but there is a twist in the structure which didn't capture by PaCMAP.

- Write about looking at images of digit 1 as well..how in 2D space images are distributed, which images occur high-errors in high-D
