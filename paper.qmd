...
- low count bin removal using a bin density cutoff, based on standardized bin counts ($w_h = n_h/n, ~~h=1, \dots m$). (Note that, density can be also computed and used, as $d_h = w_h/A$ where $A$ is the area of the hexagon.)
...
It is worthwhile to consider what are desirable aspects of a hexbin result, that maps to summarizing the \pD{} fit well. The binning should capture the underlying data distribution closely, with minimum number of necessary bins. An ideal binning might be indicated by a more uniform distribution of bin counts, or having few relatively empty bins. To help with this assessment average standardized bin count ($\bar{w} = \sum{w_h}/m$), average density ($\bar{d} = \sum{d_h}/m$) and proportion of non-empty bins ($m/b$), are also computed. @fig-param-two-curvy shows some choices of plots of these quantities for a single NLDR layout, with three choices of $a_1$ indicated. Some expectations and reasoning for these plots is:
...
error_plot_two_curvy_dh <-
  plot_hbe_dh(error_two_curvy_dens) +
  scale_color_manual(
    values=c("PaCMAP" = '#e41a1c', "PHATE" = '#ff7f00', 
             "TriMAP" = '#4daf4a', "tSNE" = '#a65628', 
             "UMAP" = '#636363', "tSNE2" = '#984ea3')) +
...
error_plot_pbmc_dh <- plot_hbe_dh(error_pbmc) +
  scale_color_manual(values=c('#999999','#a65628','#e41a1c','#377eb8','#4daf4a','#ff7f00','#984ea3','#f781bf')) +
  ylab("HBE")
...
error_plot_mnist_dh <- plot_hbe_dh(error_mnist) +
  scale_color_manual(
    values=c('#a65628', '#984ea3', '#999999', 
             '#377eb8', '#ff7f00', '#e41a1c')) +
  theme(aspect.ratio=0.685) +
