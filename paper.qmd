...
source("scripts/five_gau_clusters/5_gen_model_with_tsne.R")
...
nldr_two_curvy <- tsne_two_curvy_scaled |> 
...
nldr_two_curvy2 <- tsne_two_curvy_scaled |> 
  ggplot(aes(x = tSNE1, y = tSNE2)) + 
  geom_point(alpha=0.5, colour="#636363", size = 0.5) +
  interior_annotation("a", sc_ltr_pos)

nldr_two_curvy_original <- tsne_two_curvy_scaled |> 
...
nldr_two_curvy_original2 <- tsne_two_curvy_scaled |> 
  ggplot(aes(x = tSNE1, y = tSNE2)) + 
  geom_point(alpha=0.5, color = "#636363", size = 0.5) +
  theme(aspect.ratio = 1) +
  #xlim(c(-0.01, 1.1)) + ylim(c(-0.08, 0.57)) +
...
boxplot2 <- ggplot(df_bin_centroids_two_curvy, 
       aes(x=bin_counts)) + 
  geom_histogram()
...
hex_grid_coloured_two_curvy1 <- ggplot() + 
  ...
  interior_annotation("b", sc_ltr_pos) + 
  xlim(c(-0.3, 1.2)) + ylim(c(-0.17, 1.1)) 
...
benchmark_two_curvy <- 2.8 * bin_width
...
## First projection
projection <- cbind(
    c(0.11062,-0.02743,0.02786,-0.01930,0.02374,-0.03493,0.03159),
    c(-0.01110,-0.06886,0.04730,0.09890,-0.00571,-0.00038,0.00169))
...
two_curvy_proj_umap_first_model1_temp <- projected_df |>
  ggplot(
    aes(
      x = proj1, 
      y = proj2)) +
...
To illustrate the method, we use $7\text{-}D$ simulated data, which we call the "non-linear clusters". It is constructed by simulating two clusters, each consisting of $1000$ observations. The C-shaped cluster is generated from $\theta \sim U(-3\pi/2, 0)$, $X_1 = \sin(\theta)$, $X_2 \sim U(0, 2)$ (adding thickness to the C), $X_3 = \text{sign}(\theta) \times (\cos(\theta) - 1)$. The other cluster is from $X_1 \sim U(0, 2)$, $X_2 \sim U(0, 3)$, $\gamma \sim U(0, 0.5)$, and $X_3 = -(X_1^3 + X_2) + \gamma$. The remaining variables $X_4, X_5, X_6, X_7$ are all uniform error, with small variance. We would consider $T=(X_1, X_2, X_3)$ to be the geometric structure (true model) that we hope to capture.
...
## Second projection
projection <- cbind(
    c(-0.03372,0.07726,0.03948,-0.00096,-0.02976,-0.07968,0.03194),
    c(0.07080,0.00316,-0.00382,0.05069,-0.07334,-0.02506,-0.05749))
...
#| fig-cap: "Two projections from $7\\text{-}D$, for the two non-linear clusters data are shown. The sample obtained from the true model is shown, with the sample shown in grey and the true model in blue lines. In the C-shaped cluster, the data points are distributed randomly along the cluster, while the data is more concentrated more in one corner in the other non-linear cluster. Video of the langevitour animations are available at <>." 
#| fig-pos: H
#| fig-width: 10
#| fig-height: 5
...
boxplot1 <- ggplot(df_bin_centroids2, 
                   aes(x=bin_counts)) + 
    geom_histogram()
...
boxplot3 <- ggplot(df_bin_centroids3, 
                   aes(x=bin_counts)) + 
    geom_histogram()
...
plot_predict_tsne_two_curvy <- ggplot(
  data = true_pred_df, 
  aes(x = pred_UMAP_1,
      y = pred_UMAP_2)) +
  geom_point(alpha = 0.5) +
...
error_two_non_linear_diff_shaped_close_clusters_umap <- read_rds("data/two_non_linear_diff_shaped_close_clusters/error_two_non_linear_diff_shaped_close_clusters_tsne.rds")
...
mse_two_curvy_b <- ggplot(error_two_non_linear_diff_shaped_close_clusters_umap, 
  ...
  geom_point(size = 1) +
  scale_x_continuous(breaks = sort(unique(round(error_two_non_linear_diff_shaped_close_clusters_umap$a1, 2)))[c(2, 5, 8, 10, 12, 14, 15, 16, 17)]) +
...
#| fig-cap: "The $7\\text{-}D$ model error in \\gD{} layout. Color indicates square root of total error, dark colour indicating high error and light indicates low error. Most large errors are distributed near the sparse end of the non-linear cluster with dense corner."
#| out-width: 60%
#| fig-pos: H
...
To illustrate how tSNE organize different number of points within the structure in the tSNE layout, simulated $7\text{-}D$ data having C-shaped structure is used. @fig-one-dens_clust-error a1 and b1 shows \gD{} layouts for tSNE. The default hyper-parameters are used. The \gD{} layouts are colored according to the high-dimensional model error. In b1, the large model error points are clustered in the top left corner, forming a separate cluster with high model errors.   

The models are fitted to these layouts. @fig-one-dens_clust-error b1 and b2 show the fitted models in projections of the $7\text{-}D$ space. In these projections, the differences in the distribution of \pD{} model errors can be observed. The uniform C-shaped structure displays randomly distributed model errors, while the dense C-shaped structure shows high model errors at the sparse end. In the dense C-shaped structure, the high model errors occur because there are fewer data points positioned within the bins at the sparse end of the structure.
...
#| fig-pos: H
#| out-height: 50%
#| fig-cap: "The sparse end of the structure exhibits a high model error, while the dense end shows a low model error. The tSNE layouts of the C-shaped structure, with uniform points (a1) and dense points (b1) are colored according to their high-dimensional error, along with their selected projections (a2 and b2), are displayed. Darker colors represent a high error, while lighter colors indicate a low error. The uniformly distributed points demonstrate random model errors throughout the structure, whereas the dense structure shows large model error at the sparse end. Videos of the langevitour animations are available at <>."

generate_error_plots_one_c_shaped()
```
...
trimesh_removed_mnist <- ggplot() + 
  ...
  interior_annotation("a1", sc_ltr_pos_mnist) +
  theme(legend.position = "none") 
...
#| fig-width: 12
#| out-height: 100%
#| fig-cap: "\\gD{} wireframe layout shows that there is a big non-linear cluster (grey) and a small cluster (orange) located very close to the one corner of the big cluster. The MNIST digit 1 data ($n =  7877$ and $p = 10$) has a non-linear structure in $10\\text{-}D$. Two \\gD{} projection from a tour on $10\\text{-}D$ reveal that the closeness of the clusters in $10\\text{-}D$ and the twisted pattern of the model fit with tSNE ($a_1 = 0.041$, $b = 659/585 \\text{ } (30, \\text{ }48)$). Video of the langevitour animation is available at <>."
...
right_top_img <- pixels_gathered_within |>
  ...
  theme(strip.background = element_blank(),
        ...
        plot.title = element_text(hjust = 0.31))
...
##### inside
img_error_inside <- c(7228, 7363, 7650)
...
imge_error_sample_inside <- pixels_gathered_inside |>
  ggplot(aes(x, y, fill = value)) +
  geom_tile() +
  facet_wrap(~ instance, ncol = 4) +
  ggtitle("(b)") +
...
#| fig-cap: "The $10\\text{-}D$ model error in \\gD{} layout of the MNIST digit 1 data shows a pattern. Most low model errors are distributed along the big non-linear cluster, while most large model errors are distributed along the small cluster. The images associated with large model errors shows different patterns of digit 1, some inside (b) the non-linear structure and others outside (a). Along the non-linear cluster, the angle of digit 1 changes (c-e)."
#| fig-pos: H
#| fig-width: 10
#| fig-height: 5
#| out-width: 80%

highd_error_2d_mnist + (imge_error_sample_outside / imge_error_sample_inside / right_top_img / middle_img / right_bottom_img) 
```
...
It’s important to access the \gD{} layout and the generated model overlaid on data in \pD{} together to understand whether it fits the points everywhere, fits better in some places, or simply mismatches the pattern. Interactivity also helps in understanding the quirks. 

The embedding and tour views are linked together via rectangular brushes; when a brush is active, points will be highlighted in the adjacent view. Because the tour is dynamic, brush events that become active will pause the animation, so that a user can interrogate the current view.  By default, brushing on the embedding view will produce a one-to-one linking with the tour view. For interpreting specific combinations of clusters, the multiple guides on the legend can be selected in order to see their relative orientations.

The interface is constructed as a **shinygadget** specifically designed for interactive data analysis. 
...
Another usability of our approach is how NLDR methods behave with different density of the data structure. The tSNE layout of simulated $7\text{-}D$ C-shaped data with different densities are used. The high \pD{} model errors are scattered to a corner of the \gD{} layout of dense C-shaped structure. In the dense C-shaped structure, the high model errors occur because there are fewer data points positioned within the bins at the sparse end of the structure.  
...
The digit 1 of **Hand-written digits** data shows how the model accurately capture the non-linear structure. Not only that, the \pD{} model error help to find the anomalies of digit 1 images. Also, twisted pattern of the model evident that how tSNE tries to maintain the local structure by positioning the similar patterns of digit 1 close to each other. 
...
evaluation metrics is important for NLDR. We introduced a qualitative approach. But having a quantitative measure also will be useful for various application domains. Second, one could use our approach to create an approach for tuning (hyper-)parameters.  Another direction for future work is to introduce approaches to diagnose the fitted model. Finally, one can think of designing a new NLDR method by following the binning in high-dimensions.
