...
prop_dens_a1 <- ggplot(error_two_curvy_tsne,
  ...
  scale_x_continuous(breaks = sort(unique(round(error_two_curvy_tsne$a1, 2)))[c(1, 4, 7, 11, 13, 14, 15, 16)]) +
  labs(x = expression(paste("binwidth (", a[1], ")")), y = expression(paste("relative ", bar(d[h])))) +
...
Standardised bin counts is computed as $w_h = n_h/n, ~~h=1, \dots m$, and $n$ is the number of observations. Density is computed as $d_h = w_h/A$ where $A$ is the area of the hexagon. As a measure of the denseness of a single grid, we can compute the average bin density, $\bar{d_h}$. This can be used to compare grids. 
...
- Lastly, an ideal distribution of the density of a grid is uniform, in the sense that if each bin captures a similar number of observations, then it has just the right number of bins to neatly cover the shape of the data. To examine this, the average bin density is compared against the range of binwidths. 
...
<!--pbmc3k/14_gen_model_with_UMAP.R-->
```{r}
#| label: hexbin-pbmc
#| 
...
<!--pbmc3k/11_gen_mse_for_diff_methods.R-->
```{r}
...
<!--pbmc3k/15_gen_model_with_tSNE.R-->
```{r}
