...
interior_annotation_left <- function(label) {
  annotation_custom(grid::textGrob(label = label,
      x = unit(0.08, "npc"), y = unit(0.93, "npc"),
      gp = grid::gpar(cex = 1, col="grey70")))
}
```
...
We define the NLDR as a function $g\text{:}~ \mathbb{R}^p \rightarrow \mathbb{R}^k$, with (hyper-)parameters $\mathbfit{\theta}$. The parameters, $\mathbfit{\theta}$, depend on the choice of $g$, and can be considered part of model fitting in the traditional sense. Common choices for $g$ include functions used in tSNE, UMAP, PHATE, TriMAP, PaCMAP, or MDS, although in theory any function that maps the entire space $\mathbb{R}^p$ to a subset of $\mathbb{R}^k$ is suitable. <!--Any input requirements for the data (such as normalization, or preprocessing through the use of PCA or similar) is considered part of the function $g$.-->

With our goal being to make a representation of this 2D layout that can be lifted into high-dimensional space, the layout needs to be augmented to include neighbour information. A simple approach would be to triangulate the points and add edges. A more stable approach is to first hexagonally bin the data, and connect the bin centroids. This process serves reduce some noise in the resulting surface shown in \pD{}. 

Define a hexagonal grid across the \kD{} space, using hexagons with fixed height ($h_b$), width ($w_b$). The starting coordinates $(x_s, y_s)$ mark the lower left of the grid. <!--We deliberately separate out the creation of the hexagonal grid from the mapping of points on the grid.-->
Each of the \kD{} points will belong to a hexagonal bin. That is, for each $y \in \mathbfit{Y}$, we can (uniquely) identify the hexagon that the point belongs to. Let $h : \mathbb{R}^k \rightarrow \mathbb{R}^k$ be the function that maps a point in \kD{} space to it's hexagonal centroid ($C^{(k)}$). <!--It follows that $h \circ g : \mathbb{R}^p \rightarrow \mathbb{R}^k$ maps each point in $\mathbfit{X}$ to a \kD{} centroid.-->


<!--First, a random variable $a$ is generated from a uniform distribution between $-\frac{\pi}{2}$ and $\frac{\pi}{2}$, representing the parameter $a$ is in $T$. Next, three variables $x_1$, $x_2$, and $x_3$ are computed based on $a$ to construct the S-curve data. $x_1$ is calculated as the sine of $a$ (@eq-eqn1), $x_2$ as twice the value of another uniformly distributed random variable between 0 and 1 (@eq-eqn2), and $x_3$ as the cosine of $a$ minus 1, multiplied by the sign of $a$ (@eq-eqn3). Additionally, four noise dimensions are introduced to the dataset to introduce noise variability: $x_4$, $x_5$, $x_6$, and $x_7$, each generated from uniform distributions within specific intervals (@eq-eqn4, @eq-eqn5, and @eq-eqn6 respectively). Finally, these variables are combined to form the S-curve dataset $T$.
...
$$
x_3 = \text{sign}(a) \times (\cos(3 \pi a) - 1)
$$ {#eq-eqn3}
...
umap_scurve_scaled <- as.data.frame(do.call(cbind, gen_scaled_data(data = umap_scurve, 
                                    x = "UMAP1", y = "UMAP2"))) |>
  rename(c("UMAP1" = "scaled_UMAP1", 
                  "UMAP2" = "scaled_UMAP2")) |>
  mutate(ID = 1:NROW(umap_scurve))

nldr_scurve <- umap_scurve_scaled |> 
  ggplot(aes(x = UMAP1, y = UMAP2)) + 
  geom_point(alpha=0.5, colour="#000000", size = 0.5) +
  interior_annotation_left("a")
```
...
```{r}
num_bins_x_scurve <- 6
num_bins_y_scurve <- 14
hex_size_scurve <- 0.13

hb_obj_scurve <- hex_binning(data = umap_scurve_scaled, x = "UMAP1", 
                      y = "UMAP2", num_bins_x = num_bins_x_scurve, 
                      num_bins_y = num_bins_y_scurve, x_start = NA, y_start = NA, 
                      buffer_x = NA, buffer_y = NA, hex_size = hex_size_scurve, 
                      col_start = "UMAP")

## Data set with all possible centroids in the hexagonal grid
all_centroids_df <- as.data.frame(do.call(cbind, hb_obj_scurve$centroids))

## Generate all coordinates of hexagons
hex_grid <- as.data.frame(do.call(cbind, hb_obj_scurve$hex_poly))

## To obtain the standardise counts within hexbins
counts_df <- as.data.frame(do.call(cbind, hb_obj_scurve$std_cts))
df_bin_centroids <- extract_hexbin_centroids(centroids_df = all_centroids_df, 
                                             counts_df = counts_df)

hex_grid_with_counts <- dplyr::left_join(hex_grid, counts_df, by = c("hex_poly_id" = "hb_id"))

hex_grid_scurve <-  ggplot(data = hex_grid_with_counts, aes(x = x, y = y)) +
  geom_polygon(color = "black", aes(group = hex_poly_id), fill = "#ffffff") +
  geom_point(data = umap_scurve_scaled, aes(x = UMAP1, y = UMAP2), alpha = 0.5) +
  interior_annotation_left("b")
```
...
hex_grid_nonempty_scurve <-  ggplot(data = hex_grid_nonempty, aes(x = x, y = y)) +
  geom_polygon(color = "black", aes(group = hex_poly_id), fill = "#ffffff") +
  geom_point(data = df_bin_centroids, aes(x = c_x, y = c_y), color = "#33a02c") +
  interior_annotation_left("c") 
```
...
tr1_object_scurve <- tri_bin_centroids(df_bin_centroids, x = "c_x", y = "c_y")
tr_from_to_df_scurve <- gen_edges(tri_object = tr1_object_scurve)

## Compute 2D distances
distance_scurve <- cal_2d_dist(tr_coord_df = tr_from_to_df_scurve, 
                             start_x = "x_from", start_y = "y_from", 
                             end_x = "x_to", end_y = "y_to", 
                             select_vars = c("from", "to", "distance"))

## To find the benchmark value
benchmark_scurve <- find_lg_benchmark(distance_edges = distance_scurve, distance_col = "distance")

trimesh_removed_scurve <- vis_rmlg_mesh(distance_edges = distance_scurve, 
                                        benchmark_value = benchmark_scurve, 
                                        tr_coord_df = tr_from_to_df_scurve, 
                                        distance_col = "distance") +
  interior_annotation_left("d") 
```

```{r}
#| label: fig-NLDR-scurve
#| echo: false
#| fig-cap: "Key steps for constructing the $2-D$ model for S-curve data: (a) obtain UMAP data, (b) hexagonating UMAP data, (c) obtain hexagonal bin centroids, and (d) triangulate bin centroids."
#| fig-width: 8
#| fig-height: 4
#| out-width: 100%

nldr_scurve + hex_grid_scurve + hex_grid_nonempty_scurve + trimesh_removed_scurve +
  plot_layout(ncol = 4)
```
