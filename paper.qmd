...
- UMAP representation of PBMC3k data with 15 PCs and parameter choice of n:neighbor:30, min_dist:0.1, metric:cosine (author's suggestion) shows three well-separated clusters. The question we need to answer is: Does this UMAP representation preserve the actual data structure, or is something missing or misidentified or misleading?
...
- To fit the model, first we need to decide how many number bins should allocate along the x and y axes. To make an initial choice, we use MSE plot.
...
```{r}
#| echo: false
#| fig-cap: " Model in the 2D space with UMAP (<https://youtu.be/CNXjWZMyh_M>)"
#| label: fig-nldervisPBMCUMAP
#| fig-pos: H

trimesh_removed_pbmc_umap 
```

<!--add screen shots of model in high-D-->

- You will see three well-separated clusters in high-D as well which gives evidence that UMAP preserves the global structure. Furthermore, there is non-linearity within the clusters which was not identified by the UMAP, which proves that local structure is not preserved.

- Can we find a better parameter choice for UMAP which preserve the local structure as well?

- To do that, we are going to fit models for different parameter choice and select the one which have the lowest error.  
...
- Selected the one with n:neighbor:5, min_dist:0.99, metric:cosine.
...
```{r}
#| echo: false
#| fig-cap: "Model in the 2D space with UMAP (<>)"
...
- Why I choose this dataset? Because I need show that whatever NLDR technique that we use, our model can capture the non-linear structure within clusters as well. 
