...
#### Hexagonal grid starting point

Mainly hexagonal grid starts at the lowest x and y values assign for the grid. Another possibility is to shift the grid. That's another interesting parameter to investigate.
...
ggplot(data = hex_full_count_df, aes(x = c_x, y = c_y)) +
  ...
  geom_point(data = shifted_hex_coord_df, aes(x = c_x, y = c_y), color = "red")

ggplot(data = shifted_hex_coord_df, aes(x = x, y = y)) +
...
ggplot(data = shifted_hex_coord_df, aes(x = x, y = y)) +
  ...
  scale_fill_viridis_c(direction = -1, na.value = "#ffffff")
```
Compute aic, mse after shifting and compare before and after shifting errors
And investigate how aic, mse change according to the shifted amount
...
for (i in 1:length(num_bins_vec)) {
  ...
  pred_df_training <- predict_hex_id(df_bin_centroids = centroid_df_training, nldr_df_test = tSNE_data_gau, x = "tSNE1", y = "tSNE2")
...
for (i in 1:length(num_bins_vec)) {
  ...
  pred_df_training <- predict_hex_id(df_bin_centroids = centroid_df_training, nldr_df_test = UMAP_data_gau, x = "UMAP1", y = "UMAP2")
...
for (i in 1:length(num_bins_vec)) {
  ...
  pred_df_training <- predict_hex_id(df_bin_centroids = centroid_df_training, nldr_df_test = PHATE_data_gau, x = "PHATE1", y = "PHATE2")
...
for (i in 1:length(num_bins_vec)) {
  ...
  pred_df_training <- predict_hex_id(df_bin_centroids = centroid_df_training, nldr_df_test = TriMAP_data_gau, x = "TriMAP1", y = "TriMAP2")
...
for (i in 1:length(num_bins_vec)) {
  ...
  pred_df_training <- predict_hex_id(df_bin_centroids = centroid_df_training, nldr_df_test = PaCMAP_data_gau, x = "PaCMAP1", y = "PaCMAP2")
...
num_non_empty_bins_pbmc <- df_bin_centroids_pbmc$hexID |> length()
