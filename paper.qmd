...
training_data_scurve_n <- read_rds("data/s_curve/scurve_simulated_5000.rds")
```
...
projected <- as.matrix(training_data_scurve_n) %*% projection
...
obs_labels <- as.character(1:nrow(training_data_scurve_n))
...
axis_labels <- colnames(training_data_scurve_n)
...
bin1_vec_scurve <- 2:15 #sqrt(NROW(training_data_scurve)/2)
...
umap_15_img <- readPNG("figures/pbmc3k/umap_15.png", native = TRUE) 
pacmap_img <- readPNG("figures/pbmc3k/pacmap.png", native = TRUE) 
trimap_img <- readPNG("figures/pbmc3k/trimap.png", native = TRUE)
umap_5_img <- readPNG("figures/pbmc3k/umap_5.png", native = TRUE)

## Add arrows
arrow_df_pbmc <- tibble(x = c(94, 280, 101, 150, 218, 250, 50),
             xend = c(96, 295, 96, 180, 250, 290, 25),
             y = c(10.06, 9.69, 9.88, 9.855, 9.82, 9.78, 10),
             yend = c(10.14, 9.8, 9.8, 10, 10.3, 10, 9.8))
...
error_plot_pbmc <- ggplot(error_pbmc, 
  ...
  theme(#aspect.ratio = 0.75,
    ...
    inset_element(p = tsne_30_img, 
                  ...
                  right = 0, 
                  top = 0.18) +
    inset_element(p = pacmap_img, 
                  left = 1.7, 
                  bottom = 0.3, 
                  right = 0, 
                  top = 0.5) +
    inset_element(p = umap_5_img, 
                  left = 0.4, 
                  bottom = 0.05, 
                  right = -0.2, 
                  top = 0.22) 
