...
prop_dens_a1 <- ggplot(error_two_curvy_tsne,
  ...
  scale_x_continuous(breaks = sort(unique(round(error_two_curvy_tsne$a1, 2)))[c(1, 4, 7, 11, 13, 14, 15, 16)]) +
  labs(x = expression(paste("binwidth (", a[1], ")")), y = expression(paste("relative ave bin density (", bar(d), ")"))) +
...
<!--mnist/04_gen_mse_for_diff_methods.R-->
```{r}
...
<!--mnist/05_gen_model_with_tSNE.R-->
```{r}
#| label: hexbin-mnist
