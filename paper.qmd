...
4. Examine the model for the chosen layout in \pD{}. The exact choice of binwidth isn't important, but we recommend a reasonably small one, so that wireframe is smoother, and there is a chance to see small details of departures between the fitted model and the data. (See @best-fit-tsne b, c for this data.)
...
It can also be useful to calculate the suite of other metrics to measure the fit, for each layout, which is done in plot (ii). HBE shows most agreement with rARNX, both rank (a) and (f) the best. They differ in how they rate layout (d): HBE would consider it the worst, and rARNX to be not quite as good as (a) and (f). The other three metrics roughly agree with each other, and almost the polar opposite of HBE. This is most likely explained by the tension between local and global structure. Most will prefer bigger distance between clusters in the layout, or at least not penalise this. The problem is that this leads to incorrect perception when the clusters are not widely separated, as is the case for this data (@best-fit-tsne).

The final check is to examine the chosen model fit (or several top-rated fits) in the \pD{} data space @best-fit-tsne (b, c). As with all model fitting, if the data closely hugs the model representation, in all projections shown by the tour, then the choice is further supported. You might find that some elements of the \pD{} structure is captured, but others are not. If this happens, a strategy break the data into subsets and repeat the process of deciding on a layout. Showing multiple layouts for a data set may better illustrate its nature. This might be advisable with 2NC7 also. At first glance layout (b) looks prefable, because the distribution of observations within each cluster is smooher than in either (a) or (f). This likely reflects that the layout better captures the local structure better. 
