...
Let $u : \mathbb{R}^{n\times k} \rightarrow \mathbb{R}^{m\times k}$ be the function that maps a point in \kD{} space to it's bin centroid ($C^{(k)}$). This is the model points in \kD{} space.
...
with corresponding \pD{} vector $\vec{f_i}$. Then, $f(u(x))$ maps a \pD{} point to the \pD{} model estimate, completing the model cycle.
