...
#| fig-cap: "Comparison of fitted models for layouts a and c. Three $2\\text{-}D$ projections are shown with fitted model overlaying the 2NC7 data. Layout a (top row) preserves the true nonlinear structure. In contrast, layout c (bottom row) squeezes the model and fails to capture the full width of the underlying shapes. By visualizing the model-in-data space is important to identify these quirks and layout a provides a more faithful representation than layout c."
#| fig-width: 6
#| fig-height: 4
...
This work can be applied with existing metrics for evaluating NLDR layout, such as ARNX, RTA, sc, and RGS. It provides an additional evaluation metric, and allows any layout to viewed the in the \pD{} data space. This latter aspect can help to disentangle conflicting suggestions by the different metrics.
