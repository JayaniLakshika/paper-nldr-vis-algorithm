...
XXX Model in 2D, on the UMAP layout, and three views of the fit in projections from $7\text{-}D$, for the S-curve data. The model closely fits the shape, but it has breaks which means that it does not adequately capture the surface. (The **langevitour** software is used to view the data with a tour, and the full video is available at (<https://youtu.be/G1m4q9k--v4>). NEED TO CHANGE FIRST PLOT TO BE 2D WIREFRAME.
:::
...
where $n$ is XXX, $b$ is XXX, $n_h$ is XXX, $p$ is XXX, $\mathbfit{x}_{hij}$ is XXX. MAKE THIS A NUMBERED EQUATION, SO WE CAN REFER TO IT LATER
