...
for (xbins in bin1_vec_scurve) {
  ...
  error_df <- glance(
    ...
    mutate(bin1 = xbins,
           ...
           a1 = a1,
           mean_counts = sum(df_bin_centroids_scurve$bin_counts)/NROW(df_bin_centroids_scurve))
...
count_mse_scurve <- ggplot(error_scurve,
                     aes(x = mean_counts,
                         y = MSE)) +
  geom_point(size = 1) +
  geom_line(linewidth = 0.05) +
  ...
  # geom_vline(xintercept = 13, linetype=2,
  #            color = "#bdbdbd", linewidth=1) +
  labs(x = expression(mean(n[h])), y = "MSE") +
  ggtitle("(c)") +
  theme(aspect.ratio = 0.75,
        axis.text.x = element_text(size = 10,
                                   angle = 90),
...
mse_scurve_b + a1_m_scurve +
  count_mse_scurve + distance_hist + 
  plot_layout(ncol=2)
```
