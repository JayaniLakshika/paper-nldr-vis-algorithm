...
The models are fitted to these layouts. @fig-one-dens_clust-error b1 and b2 show the fitted models in projections of the $7\text{-}D$ space. In these projections, the differences in the distribution of high-dimensional model errors can be observed. The uniform C-shaped cluster displays randomly distributed model errors, while the dense C-shaped cluster shows high model errors at the sparse end. In the dense C-shaped cluster, the high model errors occur because there are fewer data points positioned within the bins at the sparse end of the structure.
...
The MSE for different binwidths ($a_1$) using tSNE, UMAP, PHATE, PaCMAP, and TriMAP with default (hyper-)parameter setting (@fig-mnist-mse) were calculated. It is found that tSNE (@fig-mnist-mse (f)) provide the most reasonable representation for the digit 1 dataset, showing universally best. However, \gD{} representation shows a big non-linear cluster and a small cluster with a small gap. 
...
This study makes several important contributions to the field of NLDR. We have developed an algorithm to evaluate the most reasonable NLDR method and (hyper-)parameter choices for creating an accurate \gD{} layout of high-dimensional data. Our objective is to fit a model for the \gD{} layout as a \gD{} wireframe and lifts it into \pD{}, which can be overlaid on the data and visualized using a tour. This approach is defined as *model-in-data-space*. Viewing a model in the data space is an ideal way to examine the fit.

With the model fit, how the NLDR methods organize points in \gD{} layout is examined using the simulated $4\text{-}D$ data having five Gaussian clusters. tSNE, UMAP, and PaCMAP preserved the global structure having well-separated clusters, while tSNE, and UMAP effectively maintains the local structure, showing the fitted model as *crumpled \gD{} sheet* that stretches in all four dimensions. The interesting result is that the model fit of PaCMAP show *pancake* shape.  
...
To extend layouts beyond $k$-D, when 2D is clearly inadequate, can use k-means instead of bin centroids.
