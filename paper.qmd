...
The \gD{} hexagon grid is defined by the number of bins in each direction $(b_1, b_2)$, giving total number of bins as $b = b_1 \times b_2$, and a unique hexagon id, $h = 1, \dots, b$. Each hexagon, $H_h$ is uniquely described by centroid, $C_{h}^{(2)} = (c_{h1}, c_{h2})$.  (Note that, we explain the algorithm for constructing the grid and binning the data here because, surprisingly, there is no readily available implementation that could produce everything needed for this project.)

To make the grid, a buffer parameter ($q$) is set as a proportion of the plot space. By default this is 0.1. A value should be chosen to provide at buffer of a full hexagon width ($a_1$) and height ($a_2$) for the grid to extend beyond the data, in all directions. The lower left position where the grid starts is defined as $(s_1, s_2)$, and corresponds to the centroid of the lowest left hexagon. This must be smaller than the minimum data value.
This is computed as $s_1 = -q - a_1$, and $s_2 = -q \times r_2$ to have buffer around the data space. <!-- This is computed as one buffer unit, $q$ below the minimum data value, by default.--> The user provides the preferred number of bins in the horizontal direction, $b_1$, and the algorithm computes the appropriate number of bins in the vertical direction, $b_2$.

The value for $b_2$ is computed by fixing $b_1$. Considering the upper bound of the first NLDR component, $a_1 > \frac{1+q}{b_1 -3}$. Similarly, for the second NLDR component, $a_2 > \frac{r_2(1 + q)}{(b_2 - 2)}$. Since $a_2 = \frac{\sqrt(3)}{2}a_1$ for regular hexagons, $a_1 > \frac{2r_2(1 + q)}{\sqrt{3}(b_2 - 2)}$. This is a linear optimization problem. Therefore, the optimal solution must occur on a vertex. Therefore, $b_2 = \Big\lceil2 +\frac{2r_2(b_1 - 3)}{\sqrt{3}}\Big\rceil$.
...
mse_scurve_b <- ggplot(error_scurve, 
  ...
  geom_point(size = 1) +
  geom_line() + 
  geom_vline(xintercept = 240, linetype="solid", 
             color = "black", linewidth=0.8, alpha = 0.5) +
  geom_vline(xintercept = 96, linetype=2, 
             color = "black", linewidth=0.8, alpha = 0.5) +
  geom_vline(xintercept = 375, linetype=2, 
             color = "black", linewidth=0.8, alpha = 0.5) +
  ylab("log(MSE)") +
  xlab("b") +
  ggtitle("(a)") +
...
```{r}
#| fig-cap: "Reproduction of plot published in Chen et al. (2023) showing a $2\\text{-}D$ layout from UMAP applied for the PBMC3k dataset. The parameter settings, beyond the defaults, are n_neighbors = 30 and min_dist = 0.3. The question is whether this is an accurate representation of structure in the high-dimensional data."
