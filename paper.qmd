...
df_bin_n <- df_bin |>
  mutate(ID = 1:NROW(df_bin))

tr_from_to_df_n <- left_join(tr_from_to_df, df_bin_n, by = c("from" = "ID")) |>
  rename("from_hb_id" = "hb_id")

tr_from_to_df_n <- left_join(tr_from_to_df_n, df_bin_n, by = c("to" = "ID")) |>
  rename("to_hb_id" = "hb_id")
...
a1 <- ggplot(bin_coord, aes(x = x, y = y)) +
  geom_segment(
    ...
    data = tr_from_to_df_coord_with_group, colour = "#f6e8c3"
  ) +
  geom_point() +
  coord_equal() +
...
- Fit a linear model using response as distance_high_D and predictor as distance_2D
- Filter the model residuals only for short edges
- compute error for each edge connection as (residual for that point)/(total residual for all edges * number of short edges) 
...
error_plot_lg_1 <- ggplot(error_df_lg_1, aes(x = benchmark_rm_lg, y = total_error, color = method)) +
  geom_point() +
  geom_line() +
...
error_plot_lg_2 <- ggplot(error_df_lg_2, aes(x = benchmark_rm_lg, y = total_error, color = method)) +
  geom_point() +
  geom_line() +
  #geom_vline(xintercept = NROW(full_grid_with_hexbin_id)) +
  #annotate("text", x= (NROW(full_grid_with_hexbin_id) - 10), y=-5000, label=paste0("effective number of bins = ", as.character(NROW(full_grid_with_hexbin_id))), angle=90) +
...
error_plot_lg_3 <- ggplot(error_df_lg_3, aes(x = benchmark_rm_lg, y = total_error, color = method)) +
  geom_point() +
  geom_line() +
  #geom_vline(xintercept = NROW(full_grid_with_hexbin_id)) +
  #annotate("text", x= (NROW(full_grid_with_hexbin_id) - 10), y=-5000, label=paste0("effective number of bins = ", as.character(NROW(full_grid_with_hexbin_id))), angle=90) +
...
error_plot_lg_4 <- ggplot(error_df_lg_4, aes(x = benchmark_rm_lg, y = total_error, color = method)) +
  geom_point() +
  geom_line() +
  #geom_vline(xintercept = NROW(full_grid_with_hexbin_id)) +
  #annotate("text", x= (NROW(full_grid_with_hexbin_id) - 10), y=-5000, label=paste0("effective number of bins = ", as.character(NROW(full_grid_with_hexbin_id))), angle=90) +
...
error_plot_lg_5 <- ggplot(error_df_lg_5, aes(x = benchmark_rm_lg, y = total_error, color = method)) +
  geom_point() +
  geom_line() +
  #geom_vline(xintercept = NROW(full_grid_with_hexbin_id)) +
  #annotate("text", x= (NROW(full_grid_with_hexbin_id) - 10), y=-5000, label=paste0("effective number of bins = ", as.character(NROW(full_grid_with_hexbin_id))), angle=90) +
...
error_plot_lg_1 + error_plot_lg_2 + error_plot_lg_3 + error_plot_lg_4 + error_plot_lg_5 +
  plot_annotation(tag_levels = 'a') +
