...
true_model_df <- read_rds("data/two_curvy_clust/two_curvy_clust_true_model.rds")
wireframe_true_model <- read_rds("data/two_curvy_clust/two_curvy_clust_true_model_connections.rds")
...
#| fig-cap: "Two views of the true model (blue lines) in $2\\text{-}D$ projections from $7\\text{-}D$, for the two-curvy clusters data (grey points). The data is spread along the non-linear shaped clusters, and does not vary much from the curves. The **langevitour** software is used to view the data with a tour, and the full video is available at <>." 
#| fig-pos: H
#| fig-width: 10
#| fig-height: 5
...
#| echo: false
#| fig-cap: "Key steps for constructing the model on the UMAP layout ($k=2$): (a) data, (b) hexagon bins, (c) bin centroids, and (d) triangulated centroids. The two-curvy clusters data is shown."
#| fig-width: 12
#| fig-height: 4
#| out-width: 100%
...
distance_hist <- ggplot(
  ...
  aes(x = as.numeric(bin),
      y = prop)) +
  geom_line() +
  scale_x_continuous(breaks = as.numeric(distance_two_curvy_dist$bin),
                     labels = c(
                       expression(2*a[1]),
                       expression(3*a[1]),
                       expression(4*a[1]),
...
benchmark_two_curvy <- 5 * bin_width
...
#| fig-cap: "Model in \\gD{} ($a_1 = 0.12$), on the layout and two views of the fit in projections from $7\\text{-}D$, for the two-curvy clusters data ($n =  2000$ and $p = 7$). One view shows the fitted model, and the other shows the fitted and true models alongside the data. The fitted model accurately represents the shape of the clusters but does not capture the edges of the structure. This illustrates a characteristic of UMAP, which compresses the data when transforming the data into \\gD{}. Video of the langevitour animations is available at <>."
#| fig-width: 10
#| fig-height: 10

nldr_two_curvy_original + 
...
error_two_curvy_clust_umap <- read_rds("data/two_curvy_clust/error_two_curvy_clust_umap.rds")

## Find the minimum MSE when have duplicate a1
error_two_curvy_clust_umap <- error_two_curvy_clust_umap |>
  group_by(a1) |>
  filter(bin1 == min(bin1)) |>
  ungroup() |>
  mutate(prop_dens = b_non_empty/side_length) 

base_line_prop_dens <- error_two_curvy_clust_umap |>
  filter(a1 == min(a1)) |>
  pull(prop_dens)
...
mse_two_curvy_b <- ggplot(error_two_curvy_clust_umap, 
                     aes(x = a1, 
                         y = MSE)) +
  ...
  geom_vline(xintercept = 0.05,
             ...
             linewidth=1) +
  geom_vline(xintercept = 0.12, linetype=2,
             color = "#1f78b4", linewidth=1) +
  geom_line(linewidth = 0.5) + 
  geom_point(size = 1) +
  scale_x_continuous(breaks = sort(unique(round(error_two_curvy_clust_umap$a1, 2)))[c(1, 4, 8, 10, 11, 12, 13, 14)]) +
...
prop_dens_a1 <- ggplot(error_two_curvy_clust_umap,
                     aes(x = a1,
                         y = prop_comp)) +
  geom_vline(xintercept = 0.09,
             linetype="solid",
...
a1_m_two_curvy <- ggplot(error_two_curvy_clust_umap,
                     aes(x = a1,
                         y = prop_bins)) +
  geom_vline(xintercept = 0.09,
             linetype="solid",
...
predict_umap_two_curvy <- read_rds(file = "data/two_curvy_clust/two_curvy_clust_umap_predict_true.rds")
...
#| fig-height: 10
#| out-height: 80%
#| fig-cap: "Comparison of prediction generated using the exiting `umap` R package's prediction method and our method: (a) A view of the true model in projection from $7\\text{-}D$, (b) predicted data from the `umap` prediction method, (c) predicted data from our method, and (d) comparison of predictions from both methods. In plot (d), points representing the same data but predicted by different methods are connected by lines. Both prediction methods work similarly within the clusters, as each method predicts the data within the cluster with slight differences."

proj_plot_two_curvy_true + plot_predict_umap_fun_two_curvy +plot_predict_umap_two_curvy +
  plot_predict_umap_connect_two_curvy +
  plot_layout(ncol = 2)
```
...
The values $(s_1, \ s_2)$ define the position of the centroid of the bottom left hexagon. By default, this is at $s_1 = -q, s_2 = -qr_2$, where $q$ is the buffer sound the data. The choice of these values can have some effect on the distribution of bin counts. @fig-param-two_curvy (a) illustrates this. The distribution of bin counts for $s_1$ varying between $-0.1-0.0$ is shown. Generally, a more uniform distribution among these possibilities would indicate that the bins are reliably capturing the underlying distribution of observations. 
...
#| fig-pos: H
#| fig-cap: "Hexbin density plots of UMAP layout of the two-curvy clusters data, using three different bin inputs: (a) $b = 80 \\text{ } (10, \\text{ }8)$, (b) $b = 130 \\text{ } (13, \\text{ }10)$, and (c) $b = 391 \\text{ } (23, \\text{ }17)$. Color indicates standardized counts, dark indicating high count and light indicates low count. Using the MSE of the model fit in $7\\text{-}D$ helps decide on a useful choice of number of bins."
#| fig-width: 9
#| fig-height: 2
...
error_rm_two_curvy <- read_rds("data/two_curvy_clust/error_rm_lwd_diff_bin.rds") |>
...
```{r}
#| echo: false
#| fig-cap: "Various plots to help assess best hexagon bin parameters and thresholds to remove low-density bins. Both (a) and (c) show MSE, against binwidth ($a_1$) and threshold. A good benchmark value for these parameters is when the MSE drops and then flattens out. Three binwidth choices were made: $0.05$, $0.09$, and $0.12$ to investigate. As the binwidth increases, the proportion of non-empty bins also increases. There are two peaks at binwidths $0.09$ and $0.12$. The proportion of densities, calculated as a density with the smallest possible binwidth, decreases and levels off. Binwidths $0.09$ and $0.12$ are favorable choices. Based on the MSE, $0.09$ was chosen as the initial best binwidth for further analysis. There is no need to remove the low-density hexagons because as shown in (b), there is no considerable drop in MSE."
...
The decision on edge length removal is made based on the distribution of edge lengths. In particular, a gap between values, where there a concentration of small values and then a few larger values, likely suggests a cutoff for edge removal. Because the triangulation is typically done on the hexagon centroids, there are particular discrete edge lengths, based on bin widths. @fig-NLDR-two-curvy (d) illustrates edge length distributions. 
...
```{r}
#| fig-cap: "Distribution of edge lengths and wireframes in \\gD{} and $7\\text{-}D$ with Delaunay triangulation, and two choices of long edge removal: (b) benchmark = 2$a_1$ and (c) benchmark = 5$a_1$, where $a_1$ = 0.09."
#| fig-height: 15
#| fig-width: 15
#| out-height: 70%

free(distance_hist) / wrap_plots(trimesh_two_curvy2, trimesh_two_curvy_removed2, trimesh_two_curvy_removed1,
two_curvy_proj_umap_model_delaunay,
two_curvy_proj_umap_model_benchmark2,
two_curvy_proj_umap_model, ncol = 3) 
```


<!-- (@fig-NLDR-two-curvy (d)), it is necessary to remove edges that connect distant bin centroids in the triangular mesh. These edges only exist in the \gD{} model and do not extend into \pD{}, so their removal does not impact the model in \pD{}. Although there are no specific criteria for determining the benchmark value to remove long edges, the following steps provide an approach to identifying a suitable threshold:

1. Plot the distribution of the \gD{} Euclidean distances (@fig-param-two_curvy (d)).
...
# Read a variety of different NLDR representations of two_curvy_clust
# and plot them on same aspect ratio
tsne_two_curvy_clust <- read_rds("data/two_curvy_clust/two_curvy_clust_tsne_perplexity_30.rds")

nldr_two_curvy_clust1 <- tsne_two_curvy_clust |>
  ggplot(aes(x = tSNE1,
             y = tSNE2))+
  geom_point(alpha=0.1, size=1, colour='#a65628') +
...
nldr_two_curvy_clust2 <- umap_two_curvy_clust |>
  ggplot(aes(x = UMAP1,
             y = UMAP2)) +
...
nldr_two_curvy_clust3 <- phate_two_curvy_clust |>
  ggplot(aes(x = PHATE1,
             y = PHATE2))+
  geom_point(alpha=0.1, size=1, colour='#ff7f00') +
  interior_annotation("c") +
...
nldr_two_curvy_clust4 <- trimap_two_curvy_clust |>
  ggplot(aes(x = TriMAP1,
             y = TriMAP2))+
  geom_point(alpha=0.1, size=1, colour='#4daf4a') +
  interior_annotation("d") +
...
nldr_two_curvy_clust5 <- pacmap_two_curvy_clust |>
  ggplot(aes(x = PaCMAP1,
             y = PaCMAP2))+
  geom_point(alpha=0.1, size=1, colour='#e41a1c') +
  interior_annotation("e") +
...
```{r}
#| label: combine-data-two_curvy_clust

error_two_curvy_clust_umap <- read_rds("data/two_curvy_clust/error_two_curvy_clust_umap.rds")
error_two_curvy_clust_tsne <- read_rds("data/two_curvy_clust/error_two_curvy_clust_tsne.rds")
error_two_curvy_clust_phate <- read_rds("data/two_curvy_clust/error_two_curvy_clust_phate.rds")
error_two_curvy_clust_trimap <- read_rds("data/two_curvy_clust/error_two_curvy_clust_trimap.rds")
error_two_curvy_clust_pacmap <- read_rds("data/two_curvy_clust/error_two_curvy_clust_pacmap.rds")

error_two_curvy_clust <- bind_rows(error_two_curvy_clust_umap, 
                         error_two_curvy_clust_tsne,
                         error_two_curvy_clust_phate,
                         error_two_curvy_clust_trimap,
                         error_two_curvy_clust_pacmap)

error_two_curvy_clust <- error_two_curvy_clust |>
...
```{r}
#| label: error-comp-two_curvy_clust

error_plot_two_curvy_clust <- ggplot(error_two_curvy_clust, 
                           aes(x = a1, 
                               y = MSE, 
                               colour = method)) + 
  geom_point(size = 0.8) +
  ...
  scale_x_continuous(breaks = sort(unique(error_two_curvy_clust$a1))[seq(1, length(unique(error_two_curvy_clust$a1)), by = 5)]) +
...
```{r}
#| fig-cap: "Assessing which of the 5 NLDR layouts on the two_curvy_clust digit 1 data is the better representation using MSE for varying binwidth ($a_1$). Colour used for the lines and points in the left plot and in the scatterplots represents NLDR layout (a-e). Layout c is universally poor. Layouts a that show two close clusters are universally suboptimal. Layout a is the best choice."
#| label: fig-two_curvy_clust-mse
#| fig-pos: H
#| out-height: 60%

free(error_plot_two_curvy_clust) + wrap_plots(nldr_two_curvy_clust1, nldr_two_curvy_clust2, nldr_two_curvy_clust3, 
                                    nldr_two_curvy_clust4, nldr_two_curvy_clust5, ncol = 2)
```
...
tsne_two_curvy_clust_plt <- tsne_two_curvy_scaled |>
  ggplot(aes(x = tSNE1,
             y = tSNE2))+
  geom_point(alpha=0.1, size=1, colour='#a65628') +
  interior_annotation("a", c(0.08, 0.93)) +
...
```{r}
#| label: best-fit-umap-tsne
#| fig-cap: "Best fit using UMAP ($a_1$ = 0.09) and tSNE ($a_1$ = 0.16) with the same number of non-empty bins ($m$ = 39). Both models show twist in $7\\text{-}D$."
#| fig-width: 10
#| fig-height: 20
#| out-height: 90%

nldr_two_curvy_original + tsne_two_curvy_clust_plt +
  trimesh_two_curvy_removed1 + trimesh_two_curvy_removed1_tsne_with_data +
  two_curvy_proj_umap_model +
