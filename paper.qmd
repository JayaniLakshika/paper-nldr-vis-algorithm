...
When $k=2$, the number of bins along the $x$ and $y$ axes, $b_1$ and $b_2$ should computed by accounting ${r_1, r_2}$, offset along the axes, and height ($h_b$), width ($w_b$).
...
Each of the \kD{} points will belong to a hexagonal bin. That is, for each $y \in \mathbfit{Y}$, we can (uniquely) identify the hexagon that the point belongs to. 
