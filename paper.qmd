...
rmse_two_curvy_relcount <- ggplot(error_two_curvy_tsne, 
                     aes(x = d_bar, 
                         y = RMSE)) +
  geom_vline(xintercept = error_two_curvy_tsne$d_bar[15],
           colour = "#d95f02", linetype="dashed", 
           linewidth=1) +
  geom_vline(xintercept = error_two_curvy_tsne$d_bar[13],
             colour = "#1f78b4", 
             linetype="solid",
             linewidth=1) +
  geom_vline(xintercept = error_two_curvy_tsne$d_bar[11], linetype="dotted",
             colour = "#1b9e77", linewidth=1) +
  geom_line(linewidth = 0.5) + 
  geom_point(size = 1) +
#  scale_x_continuous(breaks = sort(unique(round(error_two_curvy_tsne$a1, 2)))[c(1, 4, 7, 11, 13, 14, 15, 16)]) +
  ylim(c(0, 0.75)) +
  labs(x = "relative ave bin density", y = "HBE") +
  #ylim(c(0, 0.75)) +
...
- low count bin removal using a bin density cutoff, based on standardised bin counts ($w_h = n_h/n, ~~h=1, \dots m$). (Note that, density can be also computed and used, as $d_h = w_h/A$ where $A$ is the area of the hexagon.)
...
It is worthwhile to consider what are desirable aspects of a hexbin result, that maps to summarising the \pD{} fit well. The binning should capture the underlying data distribution closely, but simply as possible. An ideal binning might be indicated by a more uniform distribution of bin counts, or having few relatively empty bins. To help with this assessment these quantities are also computed. @fig-param-two-curvy shows some choices of plots of these quantities for a single NLDR layout, with three choices of $a_1$ indicated. Some expectations and reasoning for these plots is:

- HBE will increase as $a_1$ increases, so good choices will be just before a big increase. In @fig-param-two-curvy a HBE changes fairly steadily so there is no easy choice to make.
- HBE can also be examined against average standardised bin count or average density (@fig-param-two-curvy b). This is similar to the comparison with $a_1$ but to use when comparing different NLDR layouts. Different layouts might produce different density of points, which will not be captured well by $a_1$.
- The proportion of non-empty bins is interesting to examine across different binwidths. A good binning should have just the right amount of bins to neatly cover the shape of the data, and no more or less (@fig-param-two-curvy c). 
- Bins with no observations or a small number might be dropped from the model. This will create the wireframe not extending into sparse areas, or allowing for holes in the data to be better captured. Note that, all observations are used for the HBE calculation, though. HBE can be examined relative to dropping a fraction of the low density bins (@fig-param-two-curvy d).
