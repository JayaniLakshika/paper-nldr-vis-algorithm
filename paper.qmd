...
```{r}
result_tsne <- read_rds(here::here('data/pbmc3k/pbmc_scdeed_tsne_results.rds'))
```
...
ggplot(data = result_tsne,
       aes(
         x = as.numeric(perplexity),
...
```{r}
#| label: read-pbmc-tsne-layouts
# Read a variety of different NLDR representations of PBMC
# and plot them on same aspect ratio
tsne_pbmc1 <- read_rds("data/pbmc3k/pbmc_scdeed_tsne_perplexity_30.rds")
tsne_pbmc1 <- as_tibble(tsne_pbmc1)
names(tsne_pbmc1) <- c("tSNE1", "tSNE2")
...
tsne_pbmc2 <- read_rds("data/pbmc3k/pbmc_scdeed_tsne_perplexity_320.rds")
tsne_pbmc2 <- as_tibble(tsne_pbmc2)
names(tsne_pbmc2) <- c("tSNE1", "tSNE2")
...
```{r}
#| label: combine-data-pbmc-tsne

error_pbmc_tsne <- read_rds("data/pbmc3k/error_scdeed_pbmc_tsne_perplexity_30.rds")
error_pbmc_tsne2 <- read_rds("data/pbmc3k/error_scdeed_pbmc_tsne_perplexity_320.rds")

error_pbmc <- bind_rows(error_pbmc_tsne, 
                        error_pbmc_tsne2)
...
```{r}
#| label: error-comp-pbmc-tsne

error_tsne_plot_pbmc <- ggplot(error_pbmc, 
                               aes(x = a1, 
                                   y = MSE, 
                                   colour = method)) + 
  geom_point(size = 0.8) +
  ...
  # geom_vline(xintercept = 15, linetype="solid", 
  #            color = "black", linewidth=0.8, alpha = 0.5) +
  scale_x_continuous(breaks = sort(unique(error_pbmc$a1))[c(1, 5, 9, 12, 15, 17, 19, 21)]) +
...
```{r}
#| label: fig-pbmc-mse-tsne
#| fig-pos: H
#| out-height: 100%

free(error_tsne_plot_pbmc) + wrap_plots(nldr1c, nldr2c, ncol = 2)
```
