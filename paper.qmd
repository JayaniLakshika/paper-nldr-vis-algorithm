...
## Data set with all centroids
all_centroids_df4 <- hb_obj_scurve4$centroids

## Generate all coordinates of hexagons
hex_grid4 <- hb_obj_scurve4$hex_poly

## To obtain the standardise counts within hexbins
counts_df4 <- hb_obj_scurve4$std_cts
df_bin_centroids4 <- extract_hexbin_centroids(
  centroids_df = all_centroids_df4, 
  counts_df = counts_df4) |> 
  filter(drop_empty == FALSE)

hex_grid_with_counts_s_curve4 <- full_join(
  hex_grid4, 
  df_bin_centroids4 |> select(hexID, std_counts), 
  by = c("hex_poly_id" = "hexID"))

hex_grid_with_low_counts <- full_join(hex_grid4, 
...
hex_grid_rm_lwd_scurve1 <-  ggplot(
  data = hex_grid_with_counts_s_curve4, 
  aes(x = x, y = y)) + 
...
df_bin_centroids_low <- df_bin_centroids4 |> 
  filter(std_counts <= benchmark1)

## Check neighboring bins
remove_id <- find_low_dens_hex(df_bin_centroids_all = df_bin_centroids4, 
                               bin1 = effective_bin1_scurve, 
                               df_bin_centroids_low = df_bin_centroids_low)

## Remove the identified bins
df_bin_centroids_scurve_removed <- df_bin_centroids4 |>
  filter(hexID %in%remove_id)

df_bin_centroids_scurve_keep  <- df_bin_centroids4 |>
  filter(!(hexID %in%remove_id))
...
```{r}
#| echo: false
#| fig-cap: "Various plots to help assess best number of bins, low density bin and large edge removal. Both (a) and (c) show MSE, against number of bins and standardised count. A good benchmark value for these parameters is when the MSE drops and then flattens out."
# Distribution of $2\\text{-}D$ Euclidean distances between bin centroids of the triangular mesh generated with S-curve UMAP data.
