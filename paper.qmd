...
We define the NLDR as a function $g\text{:}~ \mathbfit{X} \in \mathbb{R}^{n\times p} \mapsto \mathbfit{Y} \in \mathbb{R}^{n\times 2}$, with hyper-parameters $\mathbfit{\theta}$. These parameters, $\mathbfit{\theta}$, depend on the choice of $g$, and can be considered part of model fitting in the traditional sense. Common choices for $g$ include functions used in tSNE, UMAP, PHATE, TriMAP, PaCMAP, or MDS, although in theory any function that does this mapping is suitable. 
...
$$C_{hj}^{(p)} = \frac{1}{n_h}\sum_{i =1}^{n_h} x_{ij}\cdot\mathbf{1}[i \in H_h], ~~~h = 1, \dots, b;~ j=1, \dots, p; ~ n_h > 0.$$
We can consider this procedure to be a pseudo-inverse of $g$, $\tilde{g}\text{:}~ \mathbfit{C}^{(2)} \in \mathbb{R}^{m\times 2} \mapsto \mathbfit{C}^{(p)} \in \mathbb{R}^{m\times p}$. Then $\mathbfit{\hat{X}}_i (i=1, \dots, n) \in \{\mathbfit{C}^{(p)} = (C^{(p)}_{j1}, C^{(2)}_{j2}), j=1, \dots, m\}$ are considered the fitted values, from which we can also calculate \pD{} residuals.
...
find the nearest neighbors in \pD{}, and interpolate their positions in \gD{} to provide the predicted embedding. This can be considered to be an estimate of the mapping $g$ that maps the new observation ($\mathbfit{x}^*$) into the layout ($\mathbfit{y}^*$), $\hat{g}\text{:}~ \mathbfit{x}^* \in \mathbb{R}^{1\times p} \mapsto \mathbfit{y}^* \in \mathbb{R}^{1\times 2}$.
