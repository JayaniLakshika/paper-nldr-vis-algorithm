---
title: "Beyond Subjectivity: New Visual Tools to Choose a Nonlinear Dimension Reduction"
format: 
    jasa-pdf:
        keep-tex: true
    jasa-html: default
...
rmse_two_curvy_lwd <- ggplot(error_rm_two_curvy, 
  ...
  scale_x_continuous(breaks = sort(unique(error_rm_two_curvy$benchmark_rm_lwd))[c(1, 4, 7, 11, 15, 17, 21)]) +
...
```{r}
#| echo: false
#| fig-cap: "Various plots to help tune the model fit, using the binwidth $a_1$ and low density bin removal. In (a) RMSE against $a_1$, RMSE is going to increase as $a_1$ increases, but just before a big increase is a good choice. Here RMSE steadily increases so there is no clear choice. For comparison in the other displays we have chosen $0.03$ (orange dashed), $0.05$ (blue solid), and $0.08$ (green dotted). Proportion of non-empty bins tends to increase with $a_1$ (b). Removing low count bins (up to bins with 20% of observations) doesn't change RMSE substantially (c). The average bin density ($\\bar{d}$) tends to decrease with $a_1$ (d)."
