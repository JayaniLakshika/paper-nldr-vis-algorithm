...
diff_start_df <- diff_start_df[c(2:5, 12:15, 22:25, 32:35, 42:45, 52:55, 62:65, 72:75, 82:85, 92:95, 101),]
...
min_mse_df <- mse_df |> dplyr::filter(mse == min(mse_df$mse))

hb_obj_s_curve_n <- hex_binning(data = umap_s_curve_scaled, x = "UMAP1", 
                      y = "UMAP2", num_bins_x = num_bins_x_s_curve, 
                      num_bins_y = num_bins_y_s_curve, x_start = min_mse_df$x_start[1], 
                      y_start = min_mse_df$y_start[1], 
                      buffer_x = NA, buffer_y = NA, hex_size = hex_size_s_curve, col_start = "UMAP")
...
```{r}
#| echo: false
#| fig-cap: "(a) Visualization of bin centroids and (b) hexagonal grid, showcasing the configuration when the starting point defined at $(-0.113, -0.098)$ (colored in gray) and the starting point defined at $(-0.075, -0.195)$ (colored in light orange)."
...
```{r}
#| echo: false
#| fig-cap: "Hexbin plots shows the distribution of points before and after changing the starting point of the hexagonal grid generated for UMAP applied to the training S-curve dataset. (a) Hexbin plot when the starting point defined at $(-0.113, -0.098)$, and (b) Hexbin plot after changing the starting point to $(-0.075, -0.195)$."
...
model_object <- fit_highd_model( training_data = training_data_mnist, 
                                 ...
                                 x_start = x_start_mnist, y_start = y_start_mnist, 
                                 buffer_x = NA, buffer_y = NA, 
...
There are certain data points that exhibit high error rates due to their deviation from the usual high-dimensional data structure, which makes them outliers. These outliers can be classified into two types: those that are outliers within the nonlinear structure and those that lie outside of it. The images associated with high model error points within the nonlinear structure display different patterns of the digit 1, as shown in @fig-sampleimgerrorwithin. However, when comparing these images to the ones found outside of the nonlinear structure, it becomes evident that the latter display different patterns of the digit 1 (see @fig-sampleimgerror). @fig-nldervisMNISTPACMAP shows some data points that have high error rates due to their outlier status.
...
pixels_gathered <-  mnist_data |>
  ...
  tidyr::extract(pixel, "pixel", "(\\d+)", convert = TRUE) |>
  mutate(pixel = pixel - 2, x = pixel %% 28, y = 28 - pixel %/% 28) |>
  filter(instance %in% c(
    4699, 4785, 5173, 6671, 2186, 4636, 4882, 773, 2825, 6169, 6948, 5987, 1282, 7715, 622, 2989, 6912,
    4370, 1227, 4042, 2444, 836,
    1654)) 
...
imge_error_sample_within <- pixels_gathered |> 
  ggplot(aes(x, y, fill = value)) +  
  geom_tile() +  
  facet_wrap(~ factor(instance, levels = c(
    4699, 4785, 5173, 6671, 2186, 4636, 4882, 773, 2825, 6169, 6948, 5987, 1282, 7715, 622, 2989, 6912,
    4370, 1227, 4042, 2444, 836,
    1654)), ncol = 12) +
  theme(legend.position = "none", plot.title = element_text(size = 7, hjust = 0.5, vjust = -0.5),
...
```{r}
#| echo: false
#| fig-cap: "Images of handwritten digit 1 which occur large model error within the nonlinear strcuture."
#| label: fig-sampleimgerrorwithin
#| out-width: 100%
#| fig-pos: H

imge_error_sample_within
```
...
```{r}
#| echo: false
#| fig-cap: "Images of handwritten digit 1 which occur large model error outside the nonlinear structure."
