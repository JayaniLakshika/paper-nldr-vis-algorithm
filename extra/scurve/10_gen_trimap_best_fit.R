...
Fit_TriMAP_data(training_data_scurve |> dplyr::select(-ID), tem_dir)

path <- file.path(tem_dir, "df_2_without_class.csv")
path2 <- file.path(tem_dir, "dataset_3_TriMAP_values.csv")

Fit_TriMAP(as.integer(2), as.integer(15), as.integer(3), as.integer(3), path, path2)

TriMAP_data <- read_csv(path2)
TriMAP_data <- TriMAP_data |>
  mutate(ID = training_data_scurve$ID)

ggplot(data = TriMAP_data,
       aes(x = TriMAP1, y = TriMAP2)) +
  geom_point()

TriMAP_data |>
  select(TriMAP1, TriMAP2, ID) |>
  write_rds("data/s_curve/s_curve_trimap_new.rds")


trimap_scurve <- read_rds(file = "data/s_curve/s_curve_trimap_new.rds")

scurve_scaled_obj <- gen_scaled_data(
  data = trimap_scurve)

trimap_scurve_scaled <- scurve_scaled_obj$scaled_nldr
...
vis_rmlg_mesh(
  ...
  tr_coord_df = tr_from_to_df_scurve,
  distance_col = "distance") +
  geom_point(data = trimap_scurve_scaled,
             aes(x = TriMAP1, y = TriMAP2), alpha = 0.2)

## Hexagonal binning to have regular hexagons
hb_obj_scurve <- hex_binning(
  data = trimap_scurve_scaled,
  bin1 = num_bins_x_scurve,
  r2 = r2)

trimap_data_with_hb_id <- hb_obj_scurve$data_hb_id

df_all_scurve <- dplyr::bind_cols(training_data_scurve |> dplyr::select(-ID),
                                  trimap_data_with_hb_id)
...
langevitour::langevitour(df_exe[1:(length(df_exe)-1)],
                         ...
                         group = df_exe$type, pointSize = append(rep(1, NROW(df_b)), rep(0.6, NROW(df))),
...
df <- dplyr::bind_rows(model_df, training_data |> mutate(type = "data"))

langevitour(df |> dplyr::select(-type), lineFrom = point_connect_all$from,
            lineTo = point_connect_all$to,
            group = df$type, pointSize = append(rep(1, NROW(true_model) +  NROW(df_b)), rep(0.5, NROW(training_data))), levelColors = c("#6a3d9a", "#33a02c", "#969696"),
