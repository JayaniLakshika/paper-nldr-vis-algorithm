...
generate_curvilinear_grid_2d <- function(n_grid_x, n_grid_y) {
  # x1 <- seq(0, 0.5, length.out = n_grid_x)
  # x2 <- seq(-0.5, 3, length.out = n_grid_y)

  # Generate an equally spaced sequence of percentiles (quantiles)
  #n_points <- 20
  percentiles <- seq(0, 1, length.out = n_grid_x)

  # Use qbeta to transform the equally spaced percentiles into a beta distribution skewed towards lower values
  x1 <- qbeta(percentiles, shape1 = 2, shape2 = 5) * 2
  x2 <- -(x1^3) #+ stats::runif(NROW(curvilinear_grid), 0, 3)) #+ stats::runif(NROW(curvilinear_grid), 0, 0.5)
  # Use expand.grid to create a grid of (x, y) pairs
  curvilinear_grid <- expand.grid(x1 = x1, x2 = x2)

  #curvilinear_grid$x3 <- -(curvilinear_grid$x1^3 + stats::runif(NROW(curvilinear_grid), 0, 3)) + stats::runif(NROW(curvilinear_grid), 0, 0.5)
  curvilinear_grid$x3 <- -sin(curvilinear_grid$x1 * pi) #+ runif(NROW(curvilinear_grid), -0.5, 0.5)
