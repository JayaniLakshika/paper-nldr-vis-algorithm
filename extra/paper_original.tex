...
deviation from the usual high-dimensional data structure, which makes
them outliers. These outliers can be classified into two types: those
that are outliers within the nonlinear structure and those that lie
outside of it. The images associated with high model error points within
the nonlinear structure display different patterns of the digit 1, as
shown in Figure~\ref{fig-sampleimgerrorwithin}. However, when comparing
these images to the ones found outside of the nonlinear structure, it
becomes evident that the latter display different patterns of the digit
1 (see Figure~\ref{fig-sampleimgerror}).
Figure~\ref{fig-nldervisMNISTPACMAP} shows some data points that have
high error rates due to their outlier status.
