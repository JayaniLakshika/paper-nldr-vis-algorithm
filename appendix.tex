...
was originally published. Comparison of scaled evaluation metrics (ARNX,
RTA, SC, GS, and HBE using \(a_1=0.04\)) for two NLDR layouts of the
...
corresponding scatterplots. Most metrics (ARNX, RTA, GS, and HBE)
consistently indicate that the optimised layout (b) provides a better
representation, while SC slightly favors the published layout.}
...
metrics (ARNX, RTA, SC, GS, and HBE using \(a_1=0.06\)) for the eight
NLDR layouts computed on the PBMC3k data using a parallel coordinate
plot. Color of the line indicates NLDR layout. ARNX is reversed so that
lower is best. RTA, SC, and GS should also be reversed, but they align
well with ARNX and HBE without doing so. There is general agreement
across metrics, with some variation in ranking. All metrics identify
layout f as poor, and layouts a, c, g, and h as suboptimal due to
excessive cluster separation. Layout d performs well only at very fine
binwidths but deteriorates as binwidth increases. The best overall
agreement is between layouts b and e, with layout e ultimately chosen as
the most reasonable representation.}
