...
\caption{\label{fig-comp-metric-pbmc-scdeed}Comparison of scaled
evaluation metrics (ARNX, RTA, SC, GS, and HBE using \(a_1=0.05\)) for
two NLDR layouts of the PBMC3k data---the originally published layout
(a) and the scDEED-optimised layout (b). Each line represents a layout,
with color matching the corresponding scatterplots. Most metrics (ARNX,
...
layout. Overall, the results align with HBE, supporting that the
scDEED-optimised layout offers an improved embedding quality.}
...
metrics (ARNX, RTA, SC, GS, and HBE using \(a_1=0.05\)) for six NLDR
...
generally rated highest. Interestingly, ARNX and GS identify layout a as
best, whereas HBE and RTA prefer layout f --- consistent with its stable
performance across binwidths, suggesting that layout f provides the most
