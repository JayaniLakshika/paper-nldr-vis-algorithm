...
\section{Compare RMSE with existing evaluation
metrics}\label{compare-rmse-with-existing-evaluation-metrics}

Figure~\ref{fig-comp-metric-pbmc} and Figure~\ref{fig-comp-metric-mnist}
compare RMSE with commonly used evaluation metrics such as RTA,
AUC(R\_NX), sc, and RGS across multiple NLDR layouts. These visual
comparisons highlight that RMSE behaves differently from these existing
metrics due to the different settings involved.
...
layout evaluated using RTA, AUC(R\_NX), sc, RGS, and RMSE. The metrics
show different patterns, meaning no single measure tells the full story
of embedding quality. Interestingly, the green layout, rated worst by
AUC(R\_NX) and RGS, is seen as the best by RMSE and sc, while the brown
layout, rated best by AUC(R\_NX), is seen as the worst by RMSE showing
that RMSE captures a different view of how well the data structure is
represented.}
...
represents a layout evaluated using RTA, AUC(R\_NX), sc, RGS, and RMSE.
The metrics show different patterns, meaning no single measure tells the
full story of embedding quality. Interestingly, the brown layout, rated
the best by AUC(R\_NX), sc, and RGS, is seen as the worst by RMSE, while
the green layout, rated best by RTA, is seen as the best by RMSE as
well.}
