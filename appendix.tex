...
\section{Generating the nonlinear clusters
data}\label{generating-the-nonlinear-clusters-data}
...
Note that \citet{xia2023} uses a different subset from the PBMC dataset
used by \citet{chen2024}, shown by us in the main paper example, which
is why this comparison is shown here and not in the main paper. Their
data contains \(31,021\) cells including cell type labels, and the gene
...
(Figure~\ref{fig-pbmc-mse-umap} b) by RMSE plot
(Figure~\ref{fig-pbmc-mse-umap}) from tSNE and UMAP with default
hyper-parameters. The PBMC data (\(n =  5858\)) has three clusters in
\(9\text{-}D\), where three clusters are close. Two \gD{} projection
from a tour on \(9\text{-}D\) of the model fit with
Figure~\ref{fig-pbmc-mse-umap} a (\(a_1 = 0.04\),
\(b = 900/179 (30, 30)\)) shows three-well separated clusters with big
separations. On the other hand, the model fit with
Figure~\ref{fig-pbmc-mse-umap} b (\(a_1 = 0.04\),
\(b = 1110/317 (30, 37)\)) shows three-well separated clusters with
small separations. Therefore, Figure~\ref{fig-pbmc-mse-umap} b is more
