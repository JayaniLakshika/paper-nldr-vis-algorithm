...
*Thank you for this helpful suggestion. We agree that including the NLDR algorithm names, hyperparameter settings, and dataset details in the figure panels will improve clarity and make the results more reproducible and interpretable for readers. We have revised the manuscript accordingly:*

- *Each subfigure now includes labels indicating the algorithm used and its key hyperparameter values.*

- *Figure captions have been expanded to explicitly state the dataset used for generating the results.*

- *For figures with multiple algorithms, the layout now clearly shows which configuration corresponds to each subfigure.*
...
### **1. $k$-Neighborhood Preservation Rates / $R_{NX}$-curves**

**Purpose:** Measures how well local neighborhood structures are preserved by a dimensionality reduction method.

**Setup:**

* You have high-dimensional data $X = \{x_1, \dots, x_n\}$ and a lower-dimensional embedding $Y = \{y_1, \dots, y_n\}$.
* For each point $x_i$, define its set of $k$ nearest neighbors in high-dimensional space as $N_k(x_i)$.
* Similarly, define $N_k(y_i)$ in the embedding space.

**Neighborhood preservation for a single point:**

$$
R_{NX}^{(i)}(k) = \frac{|N_k(x_i) \cap N_k(y_i)|}{k}
$$

* Counts the fraction of neighbors in the high-D space that are still neighbors in the embedding.

**Overall $k$-Neighborhood preservation rate:**

$$
R_{NX}(k) = \frac{1}{n} \sum_{i=1}^{n} R_{NX}^{(i)}(k)
$$

* This gives a single number per neighborhood size $k$.
* Plotting $R_{NX}(k)$ against $k$ gives the **$R_{NX}$-curve**, showing local-to-global structure preservation.

**Reference:** Lee et al., 2015; Lueks et al., 2011.

**Interpretation:**

* $R_{NX}(k) = 1$ → perfect preservation of $k$-neighborhoods.
* $R_{NX}(k) \approx 0$ → neighborhoods are poorly preserved.

### **2. Shepard Diagrams**

**Purpose:** Evaluates how well distances between points in high-dimensional space are preserved in the embedding.

**Setup:**

* For all pairs of points $x_i, x_j$, compute high-dimensional distances $d_X(i,j)$.
* In the embedding, compute distances $d_Y(i,j)$.

**Shepard diagram:**

* Plot each pair $(d_X(i,j), d_Y(i,j))$ as a point in 2D.
* Often a smooth curve is added to visualize the trend.

**Interpretation:**

* Points lying close to the diagonal $d_Y = d_X$ → good preservation of distances.
* Large deviations → distortion.
* Can quantify with **stress** or **Pearson/Spearman correlation** between $d_X$ and $d_Y$.

**Applications:**

* Visualizes global distortions.
* Complements $k$-neighborhood preservation (local structure).
...
## Suggested metric by us

For each bin $h$ with $n_h$ points:

$$
SS_h = \sum_{i=1}^{n_h} \sum_{j=1}^{p} \big(x_{hij} - C^{(p)}_{hj}\big)^2
$$

* $x_{hij}$ = $j^{th}$ variable of the $i^{th}$ point in bin $h$
* $C^{(p)}_{hj}$ = $j^{th}$ coordinate of the bin center $C_h$
* $p$ = number of dimensions

The **total within-bin sum of squares** is:

$$
WBSS = \sum_{h=1}^{m} SS_h
$$

* Measures **how spread out points are around their assigned bin center**.
* Smaller WBSS → points are tightly clustered within bins → embedding preserves local structure better **within the bin**.

### Strengths

* Directly reflects the **fidelity of the embedding to your “bin model”**.
* Easy to compute in high-dimensional space.
* Conceptually similar to your RMSE.

### Limitations

* **Does not capture neighborhood preservation across bins**, i.e., whether neighbors in high-dimensional space remain neighbors in low-dimensional space.
* **Does not reflect global structure preservation**.
