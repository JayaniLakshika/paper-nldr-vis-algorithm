...
*Thank you for this thoughtful comment. We have revised Figures 8, 10, and 12 to include an additional plot showing HBE against the average bin count, enabling a comparison between the two x axis choices. The HBE difference between layouts with high density clusters and those with low density clusters (e.g. Figure 10) becomes much smaller when using the average bin count. This is problematic because layouts with high density clusters often show larger separations between clusters, which is precisely a problem our approach is trying to solve. When we look at the high-dimensional data, the clusters are close (as shown in Figure 11). A good layout should balance global (separation between clusters) vs local structure (details within clusters). Layouts that show exaggerated separations between classes are poorer, and the evaluation process should help an analyst understand this. For this reason, we suggest that HBE comparisons should be made using the same binwidth rather than the same average bin count. Having both plots available, though can be useful for the analyst.*
...
*The aspect ratio ($r_2$) of the NLDR layout is typically preferred to be conserved when plotting, and hence when we bin the data. The range of recommended values for $b_1$ is adapted from the Diaconis-Freedman Rule, where the upper limit takes $r_2$ into account. The range provided is just a recommendation. Practically, 2 is too few bins, and the upper bound is possibly too many. We have revised the text to make this clearer.*
...
*HBE is intended as a diagnostic tool rather than a definitive selection metric. It has the additional benefit of viewing the NLDR model in the data space to examine how it fits high-dimensional structure. A comparison with existing metrics has been included.*
...
*Thank you, and apologize for these errors. We have carefully spell-checked and proof-read the revised manuscript.*
