...
*We thank the reviewer for this insightful question. RMSE is intended as a diagnostic tool rather than a definitive selection metric. Its primary purpose is to provide an interpretable measure of how well high-dimensional structure is captured in the NLDR layout. Traditional measures such as AUC(R_NX), RTA, RGS, and sc evaluate local or global structure preservation, but their values can vary substantially depending on method settings. In contrast, RMSE offers a more connection between the high-dimensional data space and the fitted model in the embedding space, highlighting how faithfully an NLDR method captures the underlying data structure.*

*In the revised manuscript (Section 4), we include a parallel coordinate plot for the 2NC7 data comparing RMSE and existing metrics. The appendix contains parallel coordinate plots for the PBMC3k and MNIST digit 1 datasets. These figures show that RMSE offers a different way of looking at how well the NLDR layouts works, while the other metrics capture related but with different settings.*
...
*We thank the reviewer for raising this important point. While Euclidean distance may not be ideal for highly nonlinear manifolds or complex data shapes, it can still provide a meaningful summary when the high-dimensional means used in RMSE are suitably positioned. In such cases, Euclidean distances effectively capture variation from the fitted model, allowing RMSE to quantify how well the NLDR layout preserves the structure of the high-dimensional data.*
