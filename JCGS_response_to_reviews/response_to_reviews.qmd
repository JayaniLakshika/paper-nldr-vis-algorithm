...
*Thank you for this thoughtful comment. This is a problem when comparing the HBE curves for different layouts because the same binwidth could be correspond to very different average count. This is something that we have discussed at length in developing the methods, and decided that using binwidth as the horizontal variable is more faithful comparison than using average count. We have revised the text in the paper to make this clear, and to note that the software allows for plotting against average count instead of binwidth. The other diagnostics proportion of non-empty bins, relative average bin density and standardised bin count should also be considered in deciding the best fit.*
...
*Thank you for raising this important point. We agree that in Figure 7, the steady increase of HBE with binwidth makes it less obvious why specific values of $a_1$ were selected. Our intent was not to claim that these three binwidths (0.03, 0.05, and 0.07) are universally optimal, but rather to use them as illustrative examples for exploring trade-offs between HBE, bin density, and the proportion of non-empty bins. We chose these values because they represent a range of small-to-moderate binwidths that highlight changes across the diagnostic plots.*
