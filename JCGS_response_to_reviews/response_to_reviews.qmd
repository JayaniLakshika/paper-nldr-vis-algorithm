...
*Thank you for this thoughtful comment. We agree that using only the binwidth to compare NLDR layouts can be misleading when the layouts have different cluster densities. To address this, we have revised Figures 8, 10, and 12 to include an additional plot showing HBE against the average bin count. This allows the comparison to take into account the density within each cluster. For example, in the revised Figure 10, the HBE difference between layouts with high density clusters and those with low density clusters becomes much smaller when using the average bin count. We also noticed that layouts with high density clusters often show larger separations between clusters, while layouts with lower densities show smaller separations. This leads to an important question: should a better layout be the one that show large separations, or small separations between clusters? When we look at the high-dimensional data, the clusters are close (as shown in Figure 11). Therefore, we believe that the \gD{} layout should also shows small separations to best reflect the original data structure. For this reason, we suggest that HBE comparisons should be made using the same binwidth rather than the same average bin count.*
...
*Thank you for raising this important point. The three binwidths (0.03, 0.05, and 0.07) represent small to moderate values, to illustrate exploring trade-offs between HBE, bin count and proportion of non-empty bins when decding which to use. This has been made clearer in the text.*
...
*These references have been added, and the text is more carefully worded in the revision.*
