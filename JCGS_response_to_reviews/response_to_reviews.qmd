...
*Thank you for raising this important issue. We have updated the figures to include a version of the HBE plots with the average count in each bin as the x-axis. This allows a comparison of models as you suggest. For example in the revised Figure 10, as you expected, using this different x-axis the differences between layouts with high density clusters and layouts with low density clusters is much reduced. We can notice here that layouts with high density clusters have large gaps between clusters, while layouts with low density clusters have small gaps between clusters. There is then a key question of whether we should prefer layouts with large gaps or small gaps. We observe by touring the data in high dimensions that in real datasets we have examined the gaps in high-dimensions are often small (illustrated for example in Figure 11). Our argument is therefore that the gaps on the k-D layout should be correspondingly small in order to honestly represent the p-D data, if this is what the tour reveals. For this reason, we are advocating comparing HBE values with the same binwidth rather than the same average points per bin, to encourage what we consider more honest layouts.*
